...
from ml.alphazero_lite import (
    capture_002_003_search_policy_arbitration as search_policy_arbitration,
)
...
def sha256_file(path: Path) -> str:
    ...
    if isinstance(payload, dict) and payload.get("schema") == SCHEMA:
        ...
        if (
            isinstance(artifact_write_summary, dict)
            and "trace_capture_sha256" in artifact_write_summary
        ):
            normalized_payload = copy.deepcopy(payload)
            normalized_payload["artifact_write_summary"]["trace_capture_sha256"] = None
            if (
                "regenerated_shared_drift_sha256"
                in normalized_payload["artifact_write_summary"]
            ):
                normalized_payload["artifact_write_summary"][
                    "regenerated_shared_drift_sha256"
                ] = None
            raw_bytes = (
...
def _source_provenance_type_from_payload(source_payload: dict) -> str:
    provenance_source = (source_payload.get("selected_artifact") or {}).get(
        "provenance_source"
    )
    if provenance_source == "materialized_fixture":
        return "fixture"
    if provenance_source in {
        "selection_manifest.selected_target",
        "selection_manifest.selected_artifact",
    }:
        return "artifact"
    raise ValueError(
        "source shared drift artifact selected_artifact provenance_source is unsupported"
    )
...
def _validated_source_artifact_from_payload(source_payload: dict) -> dict:
    ...
    normalized_paired_summary["probe_mode_failure_paths"] = (
        row_split_followup._validate_probe_mode_failure_paths(paired_summary)
    )
    normalized_paired_summary["probe_mode_selected_moves"] = (
        row_split_followup._validate_probe_mode_selected_moves(
            paired_summary=paired_summary,
            rows=rows,
        )
    )
    normalized_paired_summary["shared_mechanism_supported"] = (
        row_split_followup._validate_shared_mechanism_supported(
            paired_summary=paired_summary
        )
    )
    ...
    selected_artifact = row_split_followup._validate_selected_artifact(
        source_payload.get("selected_artifact")
    )
...
def load_source_shared_drift_artifact(
    path: Path, *, allow_fixture_provenance: bool
) -> dict:
    ...
    if source_provenance_type == "fixture" and not allow_fixture_provenance:
        raise ValueError(
            "source shared drift artifact fixture provenance requires allow_fixture_provenance"
        )
...
def _normalize_trace_points(source_artifact: dict) -> list[dict]:
    ...
    for index, trace_point in enumerate(raw_trace_points):
        if not isinstance(trace_point, dict):
            raise ValueError(
                f"source shared drift artifact trace_points[{index}] must be a dict"
            )
        ...
        if isinstance(selected_move, bool) or not isinstance(selected_move, int):
            raise ValueError(
                f"source shared drift artifact trace_points[{index}] selected_move must be int"
            )
        if selected_move not in legal_moves:
            raise ValueError(
                f"source shared drift artifact trace_points[{index}] selected_move must be legal"
            )
        ...
        if (
            isinstance(simulation, bool)
            or not isinstance(simulation, (int, float))
            or not math.isfinite(simulation)
        ):
            raise ValueError(
                f"source shared drift artifact trace_points[{index}] simulation must be finite numeric"
            )
        if not isinstance(visits, list):
            raise ValueError(
                f"source shared drift artifact trace_points[{index}] visits must be a list"
            )
        if not isinstance(moves, list):
            raise ValueError(
                f"source shared drift artifact trace_points[{index}] moves must be a list"
            )
...
def _validate_trace_points(
    *, trace_points: list[dict], reference_move, full_search_selected_move
) -> list[str]:
    ...
    if full_search_selected_move not in [
        trace_point.get("selected_move") for trace_point in trace_points
    ]:
...
def _rerun_insufficiency_reasons(rerun_payload: dict | None) -> list[str]:
    ...
    return [
        reason
        for reason in rerun_insufficiency_reasons
        if isinstance(reason, str) and reason
    ]


def _normalize_rerun_trace_points(
    trace_points: list[dict], *, reference_move: int
) -> list[dict]:
...
def _trace_section(
    *, trace_origin: str, trace_points: list[dict], insufficiency_reasons: list[str]
) -> dict:
    return {
...
def _compare_field_changes(
    *,
    ...
    if (
        ...
        and all(
            isinstance(item, dict) and isinstance(item.get("move"), int)
...
def _field_change_summary(
    *, extracted_trace_points: list[dict], final_trace_points: list[dict]
) -> dict:
    added_fields = set()
    removed_fields = set()
    changed_fields = set()
    ...
    for extracted_trace_point, final_trace_point in zip(
        reversed(extracted_snapshots), reversed(final_snapshots)
    ):
        _compare_field_changes(
...
def _trace_diff_summary(
    *, extracted_trace_points: list[dict], final_trace_points: list[dict]
) -> dict:
    extracted_selected_move = (
        None
        if not extracted_trace_points
        else extracted_trace_points[-1].get("selected_move")
    )
    final_selected_move = (
        None if not final_trace_points else final_trace_points[-1].get("selected_move")
    )
    extracted_simulations = [
        trace_point.get("simulation") for trace_point in extracted_trace_points
    ]
    final_simulations = [
        trace_point.get("simulation") for trace_point in final_trace_points
    ]
    ...
    return {
        ...
        "trace_point_count_delta": len(final_trace_points)
        - len(extracted_trace_points),
        "extracted_first_simulation": None
        if not extracted_trace_points
        else extracted_trace_points[0].get("simulation"),
        "final_first_simulation": None
        if not final_trace_points
        else final_trace_points[0].get("simulation"),
        "extracted_final_selected_move": extracted_selected_move,
        "final_final_selected_move": final_selected_move,
        "extracted_final_simulation": None
        if not extracted_trace_points
        else extracted_trace_points[-1].get("simulation"),
        "final_final_simulation": None
        if not final_trace_points
        else final_trace_points[-1].get("simulation"),
        "field_change_counts": field_change_summary["field_change_counts"],
        "field_changes": field_change_summary["field_changes"],
    }


def build_trace_capture_artifact(
    source_artifact: dict, *, capture_mode: str, rerun_capture=None
) -> dict:
    ...
    if trace_origin == "insufficient" and capture_mode == "extract_then_rerun":
        if rerun_capture is None:
            ...
            if rerun_trace_points:
                ...
                insufficiency_reasons = rerun_insufficiency_reasons
                trace_origin = (
                    "rerun" if not rerun_insufficiency_reasons else "insufficient"
                )
                if rerun_insufficiency_reasons:
                    upstream_inputs = _null_backed_rerun_context(
                        "rerun_unusable_trace_insufficient"
                    )
            else:
                rerun_trace = _trace_section(
                ...
                upstream_inputs = _null_backed_rerun_context(
                    rerun_payload_reasons[0]
                    if rerun_payload_reasons
                    else "rerun_unusable_missing_trace_points"
                )

    extracted_trace = _trace_section(
        trace_origin="extracted"
        if not _validate_trace_points(
            trace_points=extracted_trace_points,
            reference_move=row.get("reference_move"),
            full_search_selected_move=row.get("full_search_selected_move"),
        )
    ...
    trace_diff_summary["full_search_selected_move"] = row.get(
        "full_search_selected_move"
    )
    trace_diff_summary["final_trace_matches_full_search_selected_move"] = bool(
        trace_points
    ) and trace_points[-1].get("selected_move") == row.get("full_search_selected_move")
    ...
    return {
        ...
        "provenance": {
            ...
            "source_shared_drift_artifact_sha256": source_artifact.get(
                "artifact_sha256"
            ),
...
def build_regenerated_shared_drift_artifact(
    ...
    expected_full_search_selected_move = trace_capture_artifact.get(
        "full_search_selected_move"
    )
    ...
    if (
        ...
        or expected_full_search_selected_move
        >= len(final_trace_point.get("visits") or [])
    ):
        return None
    ...
    if isinstance(probe_mode_traces, dict) and isinstance(
        probe_mode_traces.get("full_search"), dict
    ):
        ...
        probe_mode_traces["full_search"]["selected_move"] = (
            expected_full_search_selected_move
        )
    ...
    if isinstance(paired_summary, dict):
        ...
        if isinstance(selected_moves, dict) and isinstance(
            selected_moves.get(ROW_ID), dict
        ):
    ...
    regenerated["trace_capture_provenance"] = {
        ...
        "trace_capture_artifact_path": (
            (trace_capture_artifact.get("artifact_write_summary") or {}).get(
                "trace_capture_path"
            )
        ),
        ...
        "source_shared_drift_artifact_path": (
            trace_capture_artifact.get("source_shared_drift_artifact") or {}
        ).get("artifact_path"),
    ...
    regenerated["trace_capture_diff_summary"] = copy.deepcopy(
        trace_capture_artifact.get("trace_diff_summary")
    )

    try:
        downstream_payload = (
            selection_score_trace.classify_source_shared_drift_artifact_document(
                regenerated,
    ...
    classification = (downstream_payload.get("classification") or {}).get(
        "classification"
    )
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            canonical_validation_path.write_text(
                json.dumps(regenerated), encoding="utf-8"
            )
            row_split_followup.load_source_shared_drift_artifact(
...
def _default_rerun_result(
    loaded_source_artifact: dict, *, capture_mode: str
) -> dict | None:
    ...
    if not isinstance(artifact_path, str) or not artifact_path:
        return {"insufficiency_reasons": ["deterministic_rerun_inputs_incomplete"]}
    if isinstance(settings.get("simulation_count"), bool) or not isinstance(
        settings.get("simulation_count"), int
    ):
        return {"insufficiency_reasons": ["deterministic_rerun_inputs_incomplete"]}
    if isinstance(settings.get("seed"), bool) or not isinstance(
        settings.get("seed"), int
    ):
        return {"insufficiency_reasons": ["deterministic_rerun_inputs_incomplete"]}
    if isinstance(search_settings.get("c_puct"), bool) or not isinstance(
        search_settings.get("c_puct"), (int, float)
    ):
        return {"insufficiency_reasons": ["deterministic_rerun_inputs_incomplete"]}
...
def _derived_regenerated_path(out_path: Path) -> Path:
    regenerated_path = out_path.with_name(
        "capture_002_trace_rehydrated_shared_drift.json"
    )
    if regenerated_path == out_path:
        return out_path.with_name(
            "capture_002_trace_rehydrated_shared_drift.artifact_2.json"
        )
...
def _stable_trace_capture_sha256(artifact: dict) -> str:
    ...
    return hashlib.sha256(
        (json.dumps(normalized_artifact, indent=2, sort_keys=True) + "\n").encode(
            "utf-8"
        )
    ).hexdigest()
...
def main(argv: list[str] | None = None) -> int:
    ...
    rerun_result = _default_rerun_result(
        loaded_source_artifact, capture_mode=args.capture_mode
    )
    rerun_capture = (
        None
        if rerun_result is None
        else (lambda _source_artifact: copy.deepcopy(rerun_result))
    )
    ...
    artifact["artifact_write_summary"]["regenerated_shared_drift_path"] = str(
        regenerated_path
    )
    ...
    if regenerated is not None:
        artifact["artifact_write_summary"]["trace_capture_sha256"] = (
            _stable_trace_capture_sha256(artifact)
        )
        ...
        artifact["artifact_write_summary"]["regenerated_shared_drift_sha256"] = (
            sha256_file(regenerated_path)
        )
    else:
        ...
        artifact["artifact_write_summary"]["regenerated_shared_drift_skip_reason"] = (
            "trace_capture_not_downstream_ready"
        )
        artifact["artifact_write_summary"]["trace_capture_sha256"] = (
            _stable_trace_capture_sha256(artifact)
        )
