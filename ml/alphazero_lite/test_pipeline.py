...
class PipelineScriptTest(unittest.TestCase):
    ...
    SPECIALIZED_HEADS_WIDE_LOCAL_CONFIG = (
        "aggressive_v3_specialized_heads_wide_local.json"
    )
    ...
    V3_STRONGER_BOOTSTRAP_CONFIRM_LOCAL_CONFIG = (
        "aggressive_v3_stronger_bootstrap_confirm_local.json"
    )
    V3_STRONGER_BOOTSTRAP_CONFIRM_B_LOCAL_CONFIG = (
        "aggressive_v3_stronger_bootstrap_confirm_b_local.json"
    )
    V3_STRONGER_BOOTSTRAP_CONFIRM_C_LOCAL_CONFIG = (
        "aggressive_v3_stronger_bootstrap_confirm_c_local.json"
    )
    V3_STRONGER_BOOTSTRAP_MORE_DATA_LOCAL_CONFIG = (
        "aggressive_v3_stronger_bootstrap_more_data_local.json"
    )
    ...
    V3_SUPERHUMAN_PHASE2_ABLATION_SELFPLAY_ONLY_CONFIG = (
        "aggressive_v3_superhuman_phase2_ablation_selfplay_only.json"
    )
    V3_SUPERHUMAN_PHASE2_ABLATION_PHASE1_ARCH_CONFIG = (
        "aggressive_v3_superhuman_phase2_ablation_phase1_arch.json"
    )
    HYBRID_VALUE_TARGET_LOCAL_CONFIG = "aggressive_v3_hybrid_value_target_local.json"
    PHASE_AWARE_VALUE_TARGET_LOCAL_CONFIG = (
        "aggressive_v3_phase_aware_value_target_local.json"
    )
    TACTICAL_STABLE_FAILURE_FAMILY_LOCAL_CONFIG = (
        "aggressive_v3_tactical_stable_failure_family_local.json"
    )
    TACTICAL_OPENING_CAPTURE_FAMILY_LOCAL_CONFIG = (
        "aggressive_v3_tactical_opening_capture_family_local.json"
    )
    ...
    def find_bash_block(self, markdown: str, *, containing: tuple[str, ...]) -> str:
        commands = [
            block.split("```", maxsplit=1)[0]
            for block in markdown.split("```bash\n")[1:]
        ]
    ...
    def test_tactical_replay_launcher_python_executable_falls_back_to_workspace_venv(
        self,
    ):
    ...
    def load_local_tactical_replay_experiment_module(self):
        ...
        loader = importlib.machinery.SourceFileLoader(
            "run_local_tactical_replay_experiment", str(script_path)
        )
        spec = importlib.util.spec_from_loader(loader.name, loader)
        assert spec is not None
        module = importlib.util.module_from_spec(spec)
    ...
    def write_real_gate_command_output(
        ...
        if script_name == "check_superhuman_regressions":
            ...
            and "--challenger-path" in command
        ):
    ...
    def test_checkpoint_self_play_configs_require_evaluator_cache_size(self):
        ...
        for path, config in self.iter_json_pipeline_configs():
            for step, command in self.iter_step_commands(
                config, command_name="ml/alphazero_lite/self_play.py"
            ):
        ...
        self.assertTrue(
            found_checkpoint_self_play,
            f"No checkpoint-driven self_play config found under {configs_dir}",
        )
            ...
            for step, command in self.iter_step_commands(
                ...
                with self.subTest(config=path.name, step=step["name"]):
                    ...
                    has_hybrid_teacher_puct_semantics = (
                        has_hybrid_teacher_mode and teacher_mode == "puct"
                    )
    ...
    def test_pipeline_skip_step_omits_named_step(self):
        ...
        with tempfile.TemporaryDirectory(prefix="pipeline-skip-step-") as tmp:
                        ...
                        "steps": [
                            {
                                ...
                                "command": [
                                    "python3",
                                    "-c",
                                    "from pathlib import Path; Path('{iter_dir}/first.txt').write_text('ok')",
                                ],
                            },
                            {
            ...
            manifest = json.loads(
                (
                    Path(tmp) / "versions" / f"{run_id}-iter1" / "run_manifest.json"
    ...
    def test_pipeline_writes_environment_report(self):
        ...
        with tempfile.TemporaryDirectory(prefix="pipeline-env-report-") as tmp:
            ...
            environment_report = json.loads(
                (versions_dir / f"{run_id}-iter1" / "environment.json").read_text(
                    encoding="utf-8"
                )
            )
    ...
    def test_environment_report_skips_cuda_counts_when_cuda_is_unavailable(self):
        ...
        unavailable_cuda.device_count.side_effect = AssertionError(
    ...
    def test_superhuman_configs_use_current_baseline(self):
        ...
        self.assertIn(
            ...
            " ".join(phase2_steps["benchmark_contract"]["command"]),
        )
        self.assertIn(
            "model-artifact/current",
            " ".join(phase2_steps["arena_confirm_report"]["command"]),
        )
    ...
    def test_superhuman_configs_tighten_training_recipe(self):
        ...
        phase1_self_play = int(
            self.command_flag_value(phase1_steps["self_play"]["command"], "--games")
        )
        phase1_bootstrap = int(
            self.command_flag_value(
                phase1_steps["mcts_bootstrap_dataset"]["command"], "--games"
            )
        )
        phase1_epochs = int(
            self.command_flag_value(phase1_steps["train"]["command"], "--epochs")
        )

        phase2_self_play = int(
            self.command_flag_value(phase2_steps["self_play"]["command"], "--games")
        )
        phase2_epochs = int(
            self.command_flag_value(phase2_steps["train"]["command"], "--epochs")
        )
        phase2_bootstrap = int(
            self.command_flag_value(
                phase2_steps["mcts_bootstrap_dataset"]["command"], "--games"
            )
        )
        ...
        self.assertEqual(
            "kalah_v3",
            self.command_flag_value(
                phase1_steps["self_play"]["command"], "--input-encoding"
            ),
        )
        self.assertEqual(
            "kalah_v3",
            self.command_flag_value(
                phase1_steps["train"]["command"], "--input-encoding"
            ),
        )
        self.assertEqual(
            "residual_v3",
            self.command_flag_value(phase1_steps["train"]["command"], "--model-type"),
        )
        self.assertEqual(
            "sharpened",
            self.command_flag_value(
                phase1_steps["train"]["command"], "--policy-target-mode"
            ),
        )
        self.assertEqual(
            "sharpened",
            self.command_flag_value(
                phase1_steps["train"]["command"], "--value-target-mode"
            ),
        )
    ...
    def test_superhuman_phase2_ablation_configs_pin_expected_train_knobs(self):
        phase2 = self.load_v2_local_config(self.V3_SUPERHUMAN_PHASE2_CONFIG)
        replay_balanced = self.load_v2_local_config(
            self.V3_SUPERHUMAN_PHASE2_ABLATION_REPLAY_BALANCED_CONFIG
        )
        selfplay_only = self.load_v2_local_config(
            self.V3_SUPERHUMAN_PHASE2_ABLATION_SELFPLAY_ONLY_CONFIG
        )
        phase1_arch = self.load_v2_local_config(
            self.V3_SUPERHUMAN_PHASE2_ABLATION_PHASE1_ARCH_CONFIG
        )
        ...
        self.assertEqual(
            ...
            self.command_flag_value(
                self.config_steps_by_name(replay_balanced)["mcts_bootstrap_dataset"][
                    "command"
                ],
                "--out",
            ),
        ...
        self.assertEqual(
            self.command_flag_value(phase2_train, "--epochs"),
            self.command_flag_value(
                self.config_steps_by_name(selfplay_only)["train"]["command"], "--epochs"
            ),
        )
        self.assertEqual(
            self.command_flag_value(phase2_train, "--hidden-sizes"),
        ...
        self.assertEqual(
            "{versions_dir}/{run_id}-iter1/mcts_bootstrap.jsonl",
            self.command_flag_value(
                self.config_steps_by_name(selfplay_only)["mcts_bootstrap_dataset"][
                    "command"
                ],
                "--out",
            ),
        ...
        self.assertEqual(
            "12",
            self.command_flag_value(
                self.config_steps_by_name(phase1_arch)["train"]["command"], "--epochs"
            ),
        )
        self.assertEqual(
            "128,3",
        ...
        self.assertEqual(
            "{versions_dir}/{run_id}-iter1/mcts_bootstrap.jsonl",
            self.command_flag_value(
                self.config_steps_by_name(phase1_arch)["mcts_bootstrap_dataset"][
                    "command"
                ],
                "--out",
            ),
    ...
    def test_superhuman_phase2_config_adds_scheduled_value_trust_to_self_play(self):
        ...
        self.assertEqual(
            "0.8", self.command_flag_value(rendered_self_play, "--value-trust-opening")
        )
        self.assertEqual(
            "1.0", self.command_flag_value(rendered_self_play, "--value-trust-midgame")
        )
        self.assertEqual(
    ...
    def test_build_step_command_rejects_explicit_value_trust_flags_when_schedule_is_nested(
        self,
    ):
        step = {
            "name": "self_play",
    ...
    def test_build_step_command_rejects_equals_form_value_trust_flags_when_schedule_is_nested(
        self,
    ):
        step = {
            "name": "self_play",
    ...
    def render_config_step(
        ...
        *,
        ...
        repo_root = Path(__file__).resolve().parents[2]
        iter_dir = iter_dir or (
            repo_root / "tmp" / f"{config['run_id']}-iter{iteration}"
        )
        current_path = current_path or config["current_path"]
        parent_model_dir = parent_model_dir or (
            repo_root / Path(config.get("parent_artifact_path", current_path))
        )
        checkpoint_candidate = parent_model_dir / "checkpoint.npz"
        fallback_model = parent_model_dir / "model.npz"
        parent_checkpoint = (
            checkpoint_candidate if checkpoint_candidate.exists() else fallback_model
        )
        return render_command(
            ...
            or config.get("hard_state_validation_path", ""),
        )

    def normalize_rendered_command(
        self, command: list[str], *, config: dict, iteration: int = 1
    ) -> list[str]:
        repo_root = Path(__file__).resolve().parents[2]
    ...
    def bootstrap_shell(self, config: dict) -> str:
        return " ".join(
            self.config_steps_by_name(config)["mcts_bootstrap_dataset"]["command"]
        )
    ...
    def test_tactical_teacher_local_config_keeps_v2_student_family_and_encoding(self):
        ...
        self.assertEqual(
            "residual_v2", self.command_flag_value(train_step, "--model-type")
        )
        self.assertEqual(
            "kalah_v2", self.command_flag_value(train_step, "--input-encoding")
        )
        self.assertEqual(
            "residual_v2", self.command_flag_value(export_step, "--model-type")
        )
        self.assertEqual(
            "kalah_v2", self.command_flag_value(export_step, "--input-encoding")
        )
    ...
    def test_tactical_teacher_local_config_uses_targeted_teacher_generation(self):
        ...
        self.assertIn(
            "ml/alphazero_lite/generate_bootstrap_dataset.py", tactical_bootstrap
        )
    ...
    def test_tactical_teacher_local_config_keeps_existing_mcts1200_evaluation_contract(
        self,
    ):
        base_config = self.load_v2_local_config("aggressive_v2.yaml")
        tactical_config = self.load_v2_local_config(self.TACTICAL_TEACHER_LOCAL_CONFIG)
    ...
    def test_hybrid_teacher_local_config_uses_hybrid_teacher_generation(self):
        ...
        self.assertIn(
            "ml/alphazero_lite/generate_bootstrap_dataset.py", hybrid_bootstrap
        )
        self.assertIn("--position-selection-mode hybrid_teacher", hybrid_bootstrap)
        self.assertNotEqual(base_bootstrap, hybrid_bootstrap)

    def test_hybrid_teacher_local_config_keeps_existing_mcts1200_evaluation_contract(
        self,
    ):
        base_config = self.load_v2_local_config("aggressive_v2.yaml")
        hybrid_config = self.load_v2_local_config(self.HYBRID_TEACHER_LOCAL_CONFIG)
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix="local-promotion-gate-") as tmp:
            ...
            candidate_mcts_report_path.write_text(
                json.dumps(candidate_mcts_report), encoding="utf-8"
            )
            current_mcts_report_path.write_text(
                json.dumps(current_mcts_report), encoding="utf-8"
            )
            forensic_report_path.write_text(
                json.dumps(self.passing_forensic_report()), encoding="utf-8"
            )
    ...
    def test_local_promotion_gate_fails_when_only_some_stub_reports_are_provided(self):
        ...
        with tempfile.TemporaryDirectory(prefix="local-promotion-gate-") as tmp:
            ...
            arena_report_path.write_text(
                json.dumps(
                    {"wins": 66, "losses": 42, "draws": 12, "games_played": 120}
                ),
                encoding="utf-8",
            )
    ...
    def test_local_promotion_gate_fails_when_stub_report_json_is_malformed(self):
        ...
        with tempfile.TemporaryDirectory(prefix="local-promotion-gate-") as tmp:
            ...
            hard_report_path.write_text(
                json.dumps(
                    {"wins": 57, "losses": 34, "draws": 29, "games_played": 120}
                ),
                encoding="utf-8",
            )
            candidate_mcts_report_path.write_text(
    ...
    def test_run_executes_configured_steps_and_marks_manifest_completed(self):
        with tempfile.TemporaryDirectory(prefix="azlite-pipeline-") as tmp:
            ...
            self.assertTrue(
                (out_dir / "aggressive-v1-iter1" / "self_play_done.txt").exists()
            )
            ...
            self.assertTrue(
                all(step["status"] == "completed" for step in manifest["steps"])
            )
    ...
    def test_run_fails_when_arena_gate_score_below_threshold(self):
        with tempfile.TemporaryDirectory(prefix="azlite-pipeline-") as tmp:
            ...
            self.assertTrue(
                any(
                    failure["code"] == "arena_score_below_threshold"
                    for failure in manifest["gate_failures"]
                )
            )
    ...
    def test_run_renders_replay_data_placeholder_with_recent_iterations(self):
        with tempfile.TemporaryDirectory(prefix="azlite-pipeline-") as tmp:
            ...
            self.assertEqual(0, result.returncode, msg=result.stderr)
            replay_capture = (
                out_dir / "aggressive-v1-iter2" / "replay_capture.txt"
            ).read_text(encoding="utf-8")
    ...
    def test_run_appends_nested_value_trust_schedule_to_self_play_command(self):
        with tempfile.TemporaryDirectory(prefix="azlite-pipeline-") as tmp:
            ...
            captured_args = json.loads(
                (out_dir / "aggressive-v1-iter1" / "captured_args.json").read_text(
                    encoding="utf-8"
                )
            )
            self.assertEqual(
    ...
    def test_run_uses_repo_relative_fixed_replay_sources_without_injecting_missing_self_play(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(prefix="azlite-pipeline-") as tmp:
            ...
            self.assertEqual(0, result.returncode, msg=result.stderr)
            replay_capture = (
                (out_dir / "aggressive-v1-iter1" / "replay_capture.txt")
                .read_text(encoding="utf-8")
                .splitlines()
            )
            self.assertEqual(
                [
                    str(
                        repo_root
                        / "ml/alphazero_lite"
                        / "synthetic_endgame_escape_fix.jsonl"
                    )
                ],
                replay_capture[0].split(","),
            )
    ...
    def test_issue_263_hard_state_finetune_config_declares_replay_and_validation_contract(
        self,
    ):
        config = self.load_v2_local_config(self.ISSUE_263_HARD_STATE_FINETUNE_CONFIG)
        steps = self.config_steps_by_name(config)
        ...
        self.assertEqual(
            "aggressive-v3-incumbent-hard-state-finetune", config["run_id"]
        )
        self.assertEqual(
            [
                {
        ...
        self.assertEqual(
            "{replay_data}", self.command_flag_value(train_command, "--data-files")
        )
        self.assertEqual(
            "{replay_weights}",
            self.command_flag_value(train_command, "--replay-weights"),
        )
        self.assertEqual(
            "96,3", self.command_flag_value(train_command, "--hidden-sizes")
        )
    ...
    def test_issue_263_hard_state_finetune_validation_step_renders_top_level_validation_path(
        self,
    ):
        config = self.load_v2_local_config(self.ISSUE_263_HARD_STATE_FINETUNE_CONFIG)
    ...
    def test_tactical_replay_local_config_uses_generated_replay_dataset_and_hard_defaults(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        config_path = (
        ...
        self.assertEqual(
            [
                {
                    "path": "ml/alphazero_lite/tactical_capture_protection.jsonl",
                    "weight": 8,
                }
            ],
            config["fixed_replay_sources"],
        ...
        self.assertEqual(
            "{parent_checkpoint}",
            self.command_flag_value(train_command, "--init-checkpoint"),
        )

        hard_validation_command = steps["hard_state_validation"]["command"]
        self.assertEqual(
            "384",
            self.command_flag_value(hard_validation_command, "--artifact-simulations"),
        )

        arena_confirm_command = steps["arena_confirm_report"]["command"]
        self.assertEqual(
            "384",
            self.command_flag_value(arena_confirm_command, "--challenger-simulations"),
        )

    def test_tactical_balanced_replay_local_config_uses_balanced_artifact_and_preserves_original_config(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        ...
        self.assertEqual(
            "aggressive-v3-tactical-balanced-replay-local", balanced_config["run_id"]
        )
        self.assertEqual(
    ...
    def test_tactical_balanced_replay_runtime_config_preserves_balanced_fixed_replay_source(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-balanced-runtime-config-"
        ) as tmp:
    ...
    def test_tactical_replay_launcher_dry_run_emits_stage_plan_and_paths(self):
        ...
        self.assertEqual(
            str(output_root / "demo-run" / "summary.json"), payload["summary_path"]
        )
        self.assertEqual(
            str(output_root / "demo-run" / "selection"),
            payload["paths"]["selection_dir"],
        )
        self.assertEqual(
        ...
        self.assertEqual(
            {"mine", "label", "train", "select"}, set(payload["commands"].keys())
        )

        train_stage = payload["commands"]["train"]
        self.assertEqual(2, len(train_stage))
        self.assertTrue(
            train_stage[0][1].endswith(
        ...
        self.assertTrue(
            mine_stage[0][1].endswith("ml/alphazero_lite/run_forensic_suite.py")
        )
        generated_mining_input = self.command_flag_value(mine_stage[0], "--out")
        self.assertEqual(
            generated_mining_input, self.command_flag_value(mine_stage[1], "--inputs")
        )
        self.assertNotEqual(
            forensic_suite, self.command_flag_value(mine_stage[1], "--inputs")
        )
        ...
        self.assertEqual(
            {
                ...
                "reference_artifact": str(
                    output_root / "demo-run" / "inputs" / "reference_moves.json"
                ),
                "runtime_config_path": str(
                    output_root / "demo-run" / "inputs" / "runtime_config.json"
                ),
                "exploratory_summary": str(
                    output_root / "demo-run" / "exploratory" / "aggregate_summary.json"
                ),
            },
    ...
    def test_tactical_replay_launcher_materializes_runtime_config_with_run_local_paths(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-runtime-config-"
        ) as tmp:
            output_root = Path(tmp)
            ...
            self.assertEqual(
                str(run_paths["base_dir"] / "versions"), runtime_config["versions_dir"]
            )
            self.assertEqual(supplied_current_path, runtime_config["current_path"])
            self.assertEqual(
                supplied_current_path, runtime_config["parent_artifact_path"]
            )
            self.assertEqual(
                [
                    {"path": str(replay_dataset_path), "weight": 1},
            ...
            train_command = self.config_steps_by_name(runtime_config)["train"][
                "command"
            ]
    ...
    def test_tactical_opening_capture_family_local_config_uses_new_artifact_with_weight_eight(
        self,
    ):
        config = self.load_v2_local_config(
            self.TACTICAL_OPENING_CAPTURE_FAMILY_LOCAL_CONFIG
        )
    ...
    def test_tactical_stable_failure_family_local_config_uses_new_artifact_with_weight_eight(
        self,
    ):
        config = self.load_v2_local_config(
            self.TACTICAL_STABLE_FAILURE_FAMILY_LOCAL_CONFIG
        )
    ...
    def test_runtime_config_keeps_dynamic_replay_weight_one_and_prepends_fixed_opening_family_source(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-opening-capture-family-runtime-config-"
        ) as tmp:
            output_root = Path(tmp)
            run_paths = module.build_run_paths(output_root, "demo-run")
            replay_dataset_path = output_root / "inputs" / "tactical_replay.jsonl"
            fixed_replay_artifact_path = (
                run_paths["inputs_dir"] / "tactical_opening_capture_family_replay.jsonl"
            )
            replay_dataset_path.parent.mkdir(parents=True, exist_ok=True)
            replay_dataset_path.write_text("{}\n", encoding="utf-8")
    ...
    def test_runtime_config_keeps_dynamic_replay_weight_one_and_prepends_fixed_stable_failure_source(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-stable-failure-runtime-config-"
        ) as tmp:
            output_root = Path(tmp)
            run_paths = module.build_run_paths(output_root, "demo-run")
            replay_dataset_path = output_root / "inputs" / "tactical_replay.jsonl"
            fixed_replay_artifact_path = (
                run_paths["inputs_dir"] / "tactical_stable_failure_family_replay.jsonl"
            )
            replay_dataset_path.parent.mkdir(parents=True, exist_ok=True)
            replay_dataset_path.write_text("{}\n", encoding="utf-8")
    ...
    def test_tactical_replay_launcher_builds_opening_capture_family_replay_before_training(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-stage-commands-"
        ) as tmp:
            output_root = Path(tmp)
            run_paths = module.build_run_paths(output_root, "demo-run")
    ...
    def test_tactical_replay_launcher_builds_stable_failure_family_replay_with_custom_reference_override(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-stable-failure-stage-commands-"
        ) as tmp:
            output_root = Path(tmp)
            run_paths = module.build_run_paths(output_root, "demo-run")
            custom_reference_artifact_path = (
                output_root / "shared" / "custom_reference_moves.json"
            )
            commands = module.build_stage_commands(
                root=repo_root,
                python_bin=module.python_executable(repo_root),
    ...
    def test_tactical_stable_failure_dry_run_builds_reference_artifact_before_fixed_replay(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-stable-failure-dry-run-"
        ) as tmp:
            output_root = Path(tmp)
            args = module.parse_args(
                [
        ...
        self.assertTrue(
            train_stage[0][1].endswith("ml/alphazero_lite/build_forensic_references.py")
        )
        self.assertTrue(
            train_stage[1][1].endswith(
                "ml/alphazero_lite/build_tactical_stable_failure_family_replay.py"
            )
        )
    ...
    def test_command_names_by_stage_for_does_not_inject_fixed_replay_into_train_stage(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
    ...
    def test_tactical_balanced_replay_launcher_does_not_rebuild_fixed_replay_artifact(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-balanced-stage-commands-"
        ) as tmp:
            output_root = Path(tmp)
            run_paths = module.build_run_paths(output_root, "balanced-demo-run")
            commands = module.build_stage_commands(
    ...
    def test_tactical_replay_launcher_builds_expected_stage_commands(self):
        ...
        with tempfile.TemporaryDirectory(
            ...
            commands = module.build_stage_commands(
                ...
                base_config_path=repo_root
                / "ml/alphazero_lite/configs/aggressive_v3_tactical_replay_local.json",
                exploratory_summary_path=output_root
                / "external"
        ...
        self.assertTrue(
            commands["mine_command"][1].endswith(
                "ml/alphazero_lite/mine_hard_states.py"
            )
        )
        self.assertTrue(
            commands["prepare_forensic_mining_input_command"][1].endswith(
                "ml/alphazero_lite/run_forensic_suite.py"
            )
        )
        generated_forensic_input = self.command_flag_value(
            commands["prepare_forensic_mining_input_command"], "--out"
        )
        self.assertEqual(
            generated_forensic_input,
            self.command_flag_value(commands["mine_command"], "--inputs"),
        )
        self.assertTrue(
            commands["label_command"][1].endswith(
                "ml/alphazero_lite/label_tactical_states.py"
            )
        )
        self.assertTrue(
            commands["train_command"][1].endswith("ml/alphazero_lite/pipeline.py")
        )
        self.assertEqual(
            str(run_paths["runtime_config_path"]),
            self.command_flag_value(commands["train_command"], "--config"),
        )

        self.assertEqual(
            str(run_paths["inputs_dir"] / "mined_hard_states.jsonl"),
            self.command_flag_value(commands["label_command"], "--input"),
        )
        self.assertEqual(
            "1200",
            self.command_flag_value(commands["label_command"], "--policy-simulations"),
        )
        self.assertEqual(
            "1200",
            self.command_flag_value(commands["label_command"], "--value-simulations"),
        )
        self.assertEqual(
            "42", self.command_flag_value(commands["label_command"], "--seed")
        )
        self.assertEqual(
            "sharpened",
            self.command_flag_value(commands["label_command"], "--policy-target-mode"),
        )
        self.assertEqual(
            "sharpened",
            self.command_flag_value(commands["label_command"], "--value-target-mode"),
        )
        self.assertEqual(
            "kalah_v3",
            self.command_flag_value(commands["label_command"], "--input-encoding"),
        )
        self.assertEqual(
            str(run_paths["inputs_dir"] / "labeled_tactical_states.jsonl"),
            self.command_flag_value(commands["label_command"], "--out-labeled"),
        )
        self.assertEqual(
            str(run_paths["inputs_dir"] / "tactical_replay_train.jsonl"),
            self.command_flag_value(commands["label_command"], "--out-tactical-train"),
        )
        self.assertEqual(
            str(run_paths["inputs_dir"] / "preservation_replay_train.jsonl"),
            self.command_flag_value(
                commands["label_command"], "--out-preservation-train"
            ),
        )

        self.assertTrue(
            commands["build_forensic_references_command"][1].endswith(
                "ml/alphazero_lite/build_forensic_references.py"
            )
        )
        self.assertEqual(
            forensic_suite,
            self.command_flag_value(
                commands["build_forensic_references_command"], "--suite"
            ),
        )
        self.assertEqual(
            str(reference_artifact_path),
            self.command_flag_value(
                commands["build_forensic_references_command"], "--out"
            ),
        )
        self.assertEqual(
            "1200",
            self.command_flag_value(
                commands["build_forensic_references_command"], "--policy-simulations"
            ),
        )
        self.assertEqual(
            "1800",
            self.command_flag_value(
                commands["build_forensic_references_command"], "--value-simulations"
            ),
        )
        self.assertEqual(
            "2040",
            self.command_flag_value(
                commands["build_forensic_references_command"], "--seed"
            ),
        )

        self.assertTrue(
            commands["candidate_forensic_command"][1].endswith(
                "ml/alphazero_lite/run_forensic_suite.py"
            )
        )
        self.assertEqual(
            forensic_suite,
            self.command_flag_value(commands["candidate_forensic_command"], "--suite"),
        )
        self.assertEqual(
            supplied_current_path,
            self.command_flag_value(
                commands["candidate_forensic_command"], "--current-artifact"
            ),
        )
        self.assertEqual(
            "1200",
            self.command_flag_value(
                commands["candidate_forensic_command"], "--mcts-simulations"
            ),
        )
        self.assertEqual(
            "1800",
            self.command_flag_value(
                commands["candidate_forensic_command"], "--teacher-simulations"
            ),
        )
        self.assertEqual(
            "3041",
            self.command_flag_value(commands["candidate_forensic_command"], "--seed"),
        )
        self.assertEqual(
            str(reference_artifact_path),
            self.command_flag_value(
                commands["candidate_forensic_command"], "--reference-artifact"
            ),
        )
        self.assertEqual(
            forensic_suite,
            self.command_flag_value(commands["current_forensic_command"], "--suite"),
        )
        self.assertEqual(
            supplied_current_path,
            self.command_flag_value(
                commands["current_forensic_command"], "--current-artifact"
            ),
        )
        self.assertEqual(
            supplied_current_path,
            self.command_flag_value(
                commands["current_forensic_command"], "--challenger-artifact"
            ),
        )
        self.assertEqual(
            "1200",
            self.command_flag_value(
                commands["current_forensic_command"], "--mcts-simulations"
            ),
        )
        self.assertEqual(
            "1800",
            self.command_flag_value(
                commands["current_forensic_command"], "--teacher-simulations"
            ),
        )
        self.assertEqual(
            "2040",
            self.command_flag_value(commands["current_forensic_command"], "--seed"),
        )
        self.assertEqual(
            str(reference_artifact_path),
            self.command_flag_value(
                commands["current_forensic_command"], "--reference-artifact"
            ),
        )

        self.assertTrue(
            commands["arena_command"][1].endswith("ml/alphazero_lite/arena.py")
        )
        self.assertEqual(
            "120", self.command_flag_value(commands["arena_command"], "--games")
        )
        self.assertEqual(
            "1041", self.command_flag_value(commands["arena_command"], "--seed")
        )
        self.assertEqual(
            "384",
            self.command_flag_value(
                commands["arena_command"], "--challenger-simulations"
            ),
        )
        self.assertEqual(
            "384",
            self.command_flag_value(commands["arena_command"], "--current-simulations"),
        )
        self.assertIn("--reuse-subtree", commands["arena_command"])
        self.assertEqual(
            "deterministic",
            self.command_flag_value(commands["arena_command"], "--root-policy-mode"),
        )
        self.assertEqual(
            "0.1",
            self.command_flag_value(commands["arena_command"], "--tactical-root-bias"),
        )
        arena_output = self.command_flag_value(commands["arena_command"], "--out")
        candidate_mcts_output = self.command_flag_value(
            commands["candidate_mcts_command"], "--out"
        )
        current_mcts_output = self.command_flag_value(
            commands["current_mcts_command"], "--out"
        )
        self.assertEqual(
            "40", self.command_flag_value(commands["candidate_mcts_command"], "--games")
        )
        self.assertEqual(
            "2041",
            self.command_flag_value(commands["candidate_mcts_command"], "--seed"),
        )
        self.assertEqual(
            "384",
            self.command_flag_value(
                commands["candidate_mcts_command"], "--az-base-simulations"
            ),
        )
        self.assertEqual(
            "1200",
            self.command_flag_value(
                commands["candidate_mcts_command"], "--mcts-simulations"
            ),
        )
        ...
        self.assertEqual(
            "deterministic",
            self.command_flag_value(
                commands["candidate_mcts_command"], "--root-policy-mode"
            ),
        )
        self.assertEqual(
            "0.1",
            self.command_flag_value(
                commands["candidate_mcts_command"], "--tactical-root-bias"
            ),
        )
        self.assertEqual(
            "40", self.command_flag_value(commands["current_mcts_command"], "--games")
        )
        self.assertEqual(
            "2041", self.command_flag_value(commands["current_mcts_command"], "--seed")
        )
        self.assertEqual(
            "384",
            self.command_flag_value(
                commands["current_mcts_command"], "--az-base-simulations"
            ),
        )
        self.assertEqual(
            "1200",
            self.command_flag_value(
                commands["current_mcts_command"], "--mcts-simulations"
            ),
        )
        ...
        self.assertEqual(
            "deterministic",
            self.command_flag_value(
                commands["current_mcts_command"], "--root-policy-mode"
            ),
        )
        self.assertEqual(
            "0.1",
            self.command_flag_value(
                commands["current_mcts_command"], "--tactical-root-bias"
            ),
        )
        self.assertEqual(
            arena_output,
            self.command_flag_value(
                commands["aggregate_holdout_command"], "--arena-inputs"
            ),
        )
        self.assertEqual(
            candidate_mcts_output,
            self.command_flag_value(
                commands["aggregate_holdout_command"], "--candidate-mcts-inputs"
            ),
        )
        self.assertEqual(
            current_mcts_output,
            self.command_flag_value(
                commands["aggregate_holdout_command"], "--current-mcts-inputs"
            ),
        )

        arena_holdout_output = self.command_flag_value(
            commands["aggregate_holdout_command"], "--out-arena"
        )
        candidate_mcts_holdout_output = self.command_flag_value(
            commands["aggregate_holdout_command"], "--out-candidate-mcts"
        )
        current_mcts_holdout_output = self.command_flag_value(
            commands["aggregate_holdout_command"], "--out-current-mcts"
        )
        candidate_forensic_output = self.command_flag_value(
            commands["candidate_forensic_command"], "--out"
        )
        regression_output = self.command_flag_value(
            commands["regression_command"], "--out"
        )
        self.assertEqual(
            arena_holdout_output,
            self.command_flag_value(
                commands["local_promotion_gate_command"], "--stub-arena-report"
            ),
        )
        self.assertEqual(
            candidate_mcts_holdout_output,
            self.command_flag_value(
                commands["local_promotion_gate_command"], "--stub-candidate-mcts-report"
            ),
        )
        self.assertEqual(
            current_mcts_holdout_output,
            self.command_flag_value(
                commands["local_promotion_gate_command"], "--stub-current-mcts-report"
            ),
        )
        self.assertEqual(
            candidate_forensic_output,
            self.command_flag_value(
                commands["local_promotion_gate_command"], "--stub-forensic-report"
            ),
        )
        self.assertEqual(
            regression_output,
            self.command_flag_value(
                commands["local_promotion_gate_command"], "--stub-regression-report"
            ),
        )
        self.assertEqual(
            "120",
            self.command_flag_value(
                commands["local_promotion_gate_command"], "--arena-games"
            ),
        )
        self.assertEqual(
            "40",
            self.command_flag_value(
                commands["local_promotion_gate_command"], "--mcts-games"
            ),
        )
        self.assertEqual(
            "40",
            self.command_flag_value(
                commands["local_promotion_gate_command"], "--min-mcts-games"
            ),
        )
        self.assertEqual(
            "0.55",
            self.command_flag_value(
                commands["local_promotion_gate_command"], "--min-arena-score"
            ),
        )
        self.assertEqual(
            "200",
            self.command_flag_value(
                commands["local_promotion_gate_command"],
        ...
        self.assertEqual(
            "350",
            self.command_flag_value(
                commands["local_promotion_gate_command"], "--max-arena-move-time-p95-ms"
            ),
        )

        self.assertTrue(
            commands["bucket_gate_command"][1].endswith(
                "ml/alphazero_lite/check_bucket_promotion_gate.py"
            )
        )
        self.assertTrue(
            commands["regression_command"][0].endswith(
                "script/ai/check_superhuman_regressions"
            )
        )
        self.assertTrue(
            commands["local_promotion_gate_command"][0].endswith(
                "script/ai/local_promotion_gate"
            )
        )
        self.assertTrue(
            commands["decision_command"][1].endswith(
                "ml/alphazero_lite/write_tactical_lane_decision.py"
            )
        )
    ...
    def test_tactical_replay_launcher_supports_explicit_exploratory_summary_path(self):
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-exploratory-summary-"
        ) as tmp:
            output_root = Path(tmp)
            run_paths = module.build_run_paths(output_root, "demo-run")
            exploratory_summary_path = (
                output_root / "external" / "aggregate_summary.json"
            )
            commands = module.build_stage_commands(
                root=repo_root,
                python_bin=module.python_executable(repo_root),
        ...
        self.assertEqual(
            ...
            self.command_flag_value(
                commands["decision_command"], "--exploratory-summary"
            ),
        )
    ...
    def test_tactical_replay_launcher_script_is_executable(self):
        ...
        self.assertTrue(
            ...
            msg=f"script is not executable: {script_path}",
        )
    ...
    def test_tactical_replay_launcher_dry_run_respects_start_stage_and_skip_stage(self):
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-dry-run-stages-"
        ) as tmp:
            output_root = Path(tmp)
            args = module.parse_args(
                [
            ...
            ((output_root / "provided-artifact") / "model.npz").write_text(
                "stub", encoding="utf-8"
            )
    ...
    def test_tactical_replay_launcher_dry_run_requires_exploratory_summary_for_decision_stage(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-dry-run-decision-"
        ) as tmp:
            output_root = Path(tmp)
            args = module.parse_args(
                [
    ...
    def test_tactical_replay_launcher_dry_run_supports_validation_pack_without_decision(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-dry-run-validation-pack-"
        ) as tmp:
            tmp_path = Path(tmp)
        ...
        self.assertTrue(
            any(
                command[1].endswith("ml/alphazero_lite/build_forensic_references.py")
                for command in validation_commands
            )
        )
        self.assertTrue(
            any(
                command[1].endswith("ml/alphazero_lite/run_forensic_suite.py")
                for command in validation_commands
            )
        )
        self.assertTrue(
        ...
        self.assertTrue(
            any(
                command[1].endswith("ml/alphazero_lite/check_bucket_promotion_gate.py")
                for command in validation_commands
            )
        )
        self.assertTrue(
            any(
                command[0].endswith("script/ai/check_superhuman_regressions")
                for command in validation_commands
            )
        )
        self.assertFalse(
            any(
                command[0].endswith("script/ai/local_promotion_gate")
                for command in validation_commands
            )
        )
        self.assertFalse(
    ...
    def test_tactical_replay_launcher_runs_validation_pack_without_promotion_outputs(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-run-validation-pack-"
        ) as tmp:
            tmp_path = Path(tmp)
            output_root = tmp_path / "runs"
            selected_artifact = tmp_path / "provided-artifact"
            ...
            final_dir = (
                module.build_run_paths(output_root, args.run_id)["base_dir"] / "final"
            )
            command_roots = [
                command[0]
                if command[0].endswith("check_superhuman_regressions")
                else command[1]
            ...
            self.assertIn(
                str(repo_root / "ml/alphazero_lite/build_forensic_references.py"),
                command_roots,
            )
            self.assertEqual(
                2,
                sum(
                    path.endswith("ml/alphazero_lite/run_forensic_suite.py")
                    for path in command_roots
                ),
            )
            self.assertIn(str(repo_root / "ml/alphazero_lite/arena.py"), command_roots)
            self.assertIn(
                str(repo_root / "ml/alphazero_lite/check_bucket_promotion_gate.py"),
                command_roots,
            )
            self.assertIn(
                str(repo_root / "script/ai/check_superhuman_regressions"), command_roots
            )
            self.assertNotIn(
                str(repo_root / "script/ai/local_promotion_gate"),
                [command[0] for command in commands_run],
            )
            self.assertNotIn(
                str(repo_root / "ml/alphazero_lite/write_tactical_lane_decision.py"),
                command_roots,
            )
    ...
    def test_tactical_replay_launcher_allows_bucket_gate_failure_during_validation_pack(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-run-validation-pack-bucket-gate-"
        ) as tmp:
            tmp_path = Path(tmp)
            output_root = tmp_path / "runs"
            selected_artifact = tmp_path / "provided-artifact"
    ...
    def test_tactical_replay_launcher_dry_run_continues_into_final_holdout_without_duplicate_validation_commands(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-dry-run-validation-continue-"
        ) as tmp:
            tmp_path = Path(tmp)
            output_root = tmp_path / "runs"
            selected_artifact = tmp_path / "provided-artifact"
        ...
        self.assertEqual(
            ["validation_pack", "final_holdout"], list(payload["commands"].keys())
        )
        self.assertEqual(6, len(payload["commands"]["validation_pack"]))
        ...
        self.assertIn(
            str(repo_root / "ml/alphazero_lite/mcts1200_baseline.py"),
            final_holdout_roots,
        )
        self.assertIn(
            str(repo_root / "ml/alphazero_lite/aggregate_holdout_reports.py"),
            final_holdout_roots,
        )
        self.assertIn(
            str(repo_root / "script/ai/local_promotion_gate"), final_holdout_roots
        )
        self.assertNotIn("decision", payload["commands"])
        self.assertFalse(
            any(
                path.endswith("ml/alphazero_lite/run_forensic_suite.py")
                for path in final_holdout_roots
            )
        )
        self.assertFalse(
            any(
                path.endswith("ml/alphazero_lite/arena.py")
                for path in final_holdout_roots
            )
        )
        self.assertFalse(
            any(
                path.endswith("ml/alphazero_lite/check_bucket_promotion_gate.py")
                for path in final_holdout_roots
            )
        )
        self.assertFalse(
            any(
                path.endswith("script/ai/check_superhuman_regressions")
                for path in final_holdout_roots
            )
        )

    def test_tactical_replay_launcher_dry_run_final_holdout_resume_builds_references_before_forensics(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-final-holdout-resume-"
        ) as tmp:
            tmp_path = Path(tmp)
            output_root = tmp_path / "runs"
            selected_artifact = tmp_path / "provided-artifact"
        ...
        self.assertTrue(
            command_roots[0].endswith("ml/alphazero_lite/build_forensic_references.py")
        )
        self.assertEqual(
            1,
            sum(
                path.endswith("ml/alphazero_lite/build_forensic_references.py")
                for path in command_roots
            ),
        )
        self.assertLess(
            ...
            command_roots.index(
                str(repo_root / "ml/alphazero_lite/run_forensic_suite.py")
            ),
        )

    def test_tactical_replay_launcher_rejects_validation_pack_to_decision_without_explicit_exploratory_summary(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-stale-exploratory-"
        ) as tmp:
            tmp_path = Path(tmp)
            output_root = tmp_path / "runs"
            run_paths = module.build_run_paths(output_root, "demo-run")
    ...
    def test_tactical_replay_launcher_rejects_final_holdout_to_decision_without_explicit_exploratory_summary(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-final-stale-exploratory-"
        ) as tmp:
            tmp_path = Path(tmp)
            output_root = tmp_path / "runs"
            run_paths = module.build_run_paths(output_root, "demo-run")
    ...
    def test_tactical_replay_launcher_rejects_skip_based_resume_to_decision_without_explicit_exploratory_summary(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-skip-resume-exploratory-"
        ) as tmp:
            tmp_path = Path(tmp)
            output_root = tmp_path / "runs"
            run_paths = module.build_run_paths(output_root, "demo-run")
    ...
    def test_tactical_replay_launcher_rejects_final_only_without_selected_artifact(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
    ...
    def test_tactical_replay_launcher_rejects_final_holdout_when_exploratory_failed(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()

        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-final-exploratory-failed-"
        ) as tmp:
            root = Path(tmp)
            runs_root = root / "runs"
    ...
    def test_tactical_replay_launcher_writes_summary_after_stubbed_run(self):
        ...
        with tempfile.TemporaryDirectory(prefix="tactical-replay-summary-") as tmp:
            ...
            replay_dataset_path = (
                run_paths["inputs_dir"] / "labeled_tactical_states.jsonl"
            )
            replay_dataset_path.write_text("{}\n", encoding="utf-8")
            ...
            def fake_run_command(command, cwd, allowed_returncodes=(0,)):
                ...
                if any(
                    token.endswith("ml/alphazero_lite/pipeline.py") for token in command
                ):
                    ...
                    candidate_dir = (
                        Path(runtime_config["versions_dir"])
                        / f"{runtime_config['run_id']}-iter1"
                    )
            ...
            self.assertEqual(
                ["train", "select", "validation_pack"], summary["executed_stages"]
            )
            self.assertEqual(
                str(selection_artifact),
                summary["selection_manifest"]["selected_artifact"],
            )
            self.assertEqual(
                "single_candidate", summary["selection_manifest"]["selection_rule"]
            )
            self.assertEqual(1, summary["selection_manifest"]["candidate_count"])
            self.assertEqual(8, len(commands_run))

    def test_tactical_replay_launcher_final_holdout_with_selected_artifact_materializes_run_tree_artifact(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-final-selected-"
        ) as tmp:
            tmp_path = Path(tmp)
            output_root = tmp_path / "runs"
            selected_artifact = tmp_path / "provided-artifact"
            ...
            def fake_run_command(command, cwd, allowed_returncodes=(0,)):
                ...
                if "--challenger-path" in command:
                    candidate_path = Path(
                        command[command.index("--challenger-path") + 1]
                    )
                    seen_candidate_paths.append(candidate_path)
                    self.assertTrue(candidate_path.exists())
            ...
            self.assertEqual(
                str(selection_artifact_path), seen_candidate_paths[0].as_posix()
            )
            self.assertEqual(["final_holdout"], summary["executed_stages"])
            self.assertEqual(["final_holdout"], manifest["executed_stages"])

    def test_tactical_replay_launcher_materializes_relative_selected_artifact_symlink_to_existing_path(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()

        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-relative-artifact-", dir=Path.cwd()
        ) as tmp:
            tmp_path = Path(tmp)
    ...
    def test_tactical_replay_launcher_records_original_selected_target_when_copy_materialization_is_reused(
        self,
    ):
        module = self.load_local_tactical_replay_experiment_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            prefix="tactical-replay-copy-materialization-"
        ) as tmp:
            tmp_path = Path(tmp)
            output_root = tmp_path / "runs"
            selected_artifact = tmp_path / "provided-artifact"
            ...
            selection_artifact_path = (
                module.build_run_paths(output_root, args.run_id)["selection_dir"]
                / "artifact"
            )
            self.assertTrue(selection_artifact_path.exists())
            ...
            self.assertEqual(
                str(selected_artifact.resolve()),
                summary["selection_manifest"]["selected_target"],
            )
    ...
    def test_tactical_replay_launcher_does_not_report_noop_stage_as_executed(self):
        ...
        with tempfile.TemporaryDirectory(prefix="tactical-replay-noop-stage-") as tmp:
            ...
            self.assertEqual(
                ["train", "select", "validation_pack"], manifest["executed_stages"]
            )

    def test_pipeline_initial_iteration_uses_parent_artifact_path_for_parent_checkpoint_and_keeps_current_path_for_runtime_targets(
        self,
    ):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-parent-artifact-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
            out_dir = tmp_path / "versions"
            ...
            parent_contract = json.loads(
                (iter_dir / "parent_contract.json").read_text(encoding="utf-8")
            )
            runtime_target = (iter_dir / "runtime_target.txt").read_text(
                encoding="utf-8"
            )

            self.assertEqual(str(parent_artifact), parent_contract["parent_model_dir"])
            self.assertEqual(
                str(parent_artifact / "checkpoint.npz"),
                parent_contract["parent_checkpoint"],
            )
            self.assertEqual(str(runtime_current), runtime_target)

    def test_pipeline_initial_iteration_materializes_parent_checkpoint_from_weights_json(
        self,
    ):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-parent-weights-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
            out_dir = tmp_path / "versions"
    ...
    def test_pipeline_accepts_parent_artifact_path_with_weights_json_only(self):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-parent-weights-validate-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
            out_dir = tmp_path / "versions"
    ...
    def test_pipeline_initial_iteration_resolves_relative_parent_artifact_path_against_repo_root_not_caller_cwd(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with (
            tempfile.TemporaryDirectory(
                prefix="azlite-pipeline-relative-parent-cwd-"
            ) as tmp,
            tempfile.TemporaryDirectory(
                ...
                json.dumps(
                    {
                        ...
                        "iterations": 1,
                        "versions_dir": str(out_dir),
                        "parent_artifact_path": parent_artifact.relative_to(
                            repo_root
                        ).as_posix(),
                        "steps": [
                            {
    ...
    def test_pipeline_initial_iteration_resolves_relative_current_path_against_repo_root_when_parent_artifact_path_is_omitted(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with (
            tempfile.TemporaryDirectory(
                prefix="azlite-pipeline-relative-current-cwd-"
            ) as tmp,
            tempfile.TemporaryDirectory(
                ...
                json.dumps(
                    {
                        ...
                        "iterations": 1,
                        "versions_dir": str(out_dir),
                        "current_path": current_artifact.relative_to(
                            repo_root
                        ).as_posix(),
                        "steps": [
                            {
            ...
            self.assertEqual(
                str(current_artifact / "checkpoint.npz"),
                parent_contract["parent_checkpoint"],
            )

    def test_pipeline_fails_before_running_steps_when_parent_artifact_path_is_missing(
        self,
    ):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-missing-parent-artifact-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
            out_dir = tmp_path / "versions"
    ...
    def test_pipeline_fails_before_running_steps_when_parent_artifact_has_no_checkpoint_or_model(
        self,
    ):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-parent-artifact-without-checkpoint-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
            out_dir = tmp_path / "versions"
    ...
    def test_pipeline_fails_before_running_steps_when_hard_state_validation_path_is_missing(
        self,
    ):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-missing-hard-state-validation-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
            out_dir = tmp_path / "versions"
                ...
                json.dumps(
                    {
                        ...
                        "hard_state_validation_path": str(
                            tmp_path / "missing-hard-state-validation.jsonl"
                        ),
                        "steps": [
                            {
                                "name": "should_not_run",
    ...
    def test_pipeline_dry_run_allows_missing_parent_artifact_and_hard_state_validation_paths(
        self,
    ):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-dry-run-missing-inputs-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
            out_dir = tmp_path / "versions"
                ...
                json.dumps(
                    {
                        ...
                        "parent_artifact_path": str(
                            tmp_path / "missing-parent-artifact"
                        ),
                        "hard_state_validation_path": str(
                            tmp_path / "missing-hard-state-validation.jsonl"
                        ),
            ...
            manifest = json.loads(
                (
                    out_dir / "dry-run-missing-inputs-iter1" / "run_manifest.json"
                ).read_text(encoding="utf-8")
            )
            self.assertEqual("planned", manifest["status"])
    ...
    def test_pipeline_dry_run_rejects_nested_value_trust_schedule_when_command_has_explicit_flag(
        ...
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-dry-run-value-trust-conflict-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
    ...
    def test_pipeline_dry_run_rejects_invalid_search_options_shape(self):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-dry-run-invalid-search-options-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
            ...
            self.assertNotEqual(0, result.returncode)
            self.assertIn(
                "self_play step search_options must be an object", result.stderr
            )

    def test_pipeline_dry_run_rejects_invalid_value_trust_schedule_shape(self):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-dry-run-invalid-value-trust-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
    ...
    def test_pipeline_dry_run_rejects_non_boolean_value_trust_enabled(self):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-dry-run-invalid-value-trust-enabled-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
    ...
    def test_pipeline_dry_run_rejects_non_numeric_value_trust_phase_value(self):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-dry-run-invalid-value-trust-phase-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
    ...
    def test_pipeline_dry_run_rejects_non_finite_value_trust_phase_value(self):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-dry-run-non-finite-value-trust-phase-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
    ...
    def test_pipeline_dry_run_rejects_boolean_value_trust_phase_value(self):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-dry-run-bool-value-trust-phase-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
    ...
    def test_pipeline_dry_run_rejects_unknown_value_trust_schedule_key(self):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-dry-run-unknown-value-trust-key-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
    ...
    def test_pipeline_dry_run_rejects_non_positive_value_trust_schedule_multiplier(
        self,
    ):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-dry-run-nonpositive-value-trust-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
    ...
    def test_pipeline_manifest_uses_parent_artifact_path_for_first_iteration_parent_version(
        self,
    ):
        with tempfile.TemporaryDirectory(
            prefix="azlite-pipeline-parent-version-"
        ) as tmp:
            tmp_path = Path(tmp)
            config_path = tmp_path / "config.json"
            out_dir = tmp_path / "versions"
            ...
            manifest = json.loads(
                (
                    out_dir / "parent-version-contract-iter1" / "run_manifest.json"
                ).read_text(encoding="utf-8")
            )
            self.assertEqual(str(parent_artifact), manifest["parent_version"])

    def test_aggressive_config_uses_python_arena_workers_for_prefilter_and_confirm(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        ...
        for step_name, out_name, games in (
            ...
            rendered = render_command(
                ...
                iter_dir=iter_dir,
                run_id="aggressive-v1",
                versions_dir=repo_root
                / "storage"
                / "ai"
                / "alphazero_lite"
                / "versions",
                current_path="storage/ai/alphazero_lite/current",
                parent_model_dir=repo_root
                / "storage"
                / "ai"
                / "alphazero_lite"
                / "current",
                parent_checkpoint=repo_root
                / "storage"
                / "ai"
                / "alphazero_lite"
                / "current"
    ...
    def test_experiment_fast_config_uses_direct_python_arena_reports(self):
        repo_root = Path(__file__).resolve().parents[2]
        config = load_config(
            repo_root / "ml/alphazero_lite/configs/experiment1_fast_s41.yaml"
        )
    ...
    def test_experiment2_dynamic_config_uses_python_mcts1200_baseline(self):
        repo_root = Path(__file__).resolve().parents[2]
        config = load_config(
            repo_root / "ml/alphazero_lite/configs/experiment2_dynamic_s41.yaml"
        )
        steps = {step["name"]: step["command"] for step in config["steps"]}
        rendered = render_command(
        ...
        self.assertIn("--out", rendered)
        self.assertIn(
            str(repo_root / "tmp" / "exp2-dynamic-s41-iter1" / "mcts1200_report.json"),
            rendered,
        )
    ...
    def test_aggressive_config_uses_baseline_relative_mcts_gate(self):
        ...
        self.assertIn(
            "ml/alphazero_lite/mcts1200_baseline.py", current_baseline_rendered
        )
        self.assertIn("storage/ai/alphazero_lite/current", current_baseline_rendered)
        self.assertIn(
            str(iter_dir / "current_mcts1200_report.json"), current_baseline_rendered
        )
        ...
        self.assertNotIn(
            "storage/ai/alphazero_lite/current/mcts1200_report.json", rendered
        )
    ...
    def test_aggressive_v2_config_targets_v2_encoding_and_model_family(self):
        ...
        self.assertEqual(
            "kalah_v2",
            steps["self_play"][steps["self_play"].index("--input-encoding") + 1],
        )

        train_step = steps["train"]
        self.assertEqual(
            "residual_v2", train_step[train_step.index("--model-type") + 1]
        )
        self.assertEqual("64,2", train_step[train_step.index("--hidden-sizes") + 1])
        self.assertEqual(
            "kalah_v2", train_step[train_step.index("--input-encoding") + 1]
        )

        export_step = steps["export_artifact"]
        self.assertEqual(
            "residual_v2", export_step[export_step.index("--model-type") + 1]
        )
        self.assertEqual(
            "kalah_v2", export_step[export_step.index("--input-encoding") + 1]
        )
        self.assertEqual(
            "kalah_v1", export_step[export_step.index("--rules-version") + 1]
        )

    def test_aggressive_v2_config_preserves_benchmark_and_promotion_shape(self):
        repo_root = Path(__file__).resolve().parents[2]
        base_config = load_config(
            repo_root / "ml/alphazero_lite/configs/aggressive_v1.yaml"
        )
        v2_config = load_config(
            repo_root / "ml/alphazero_lite/configs/aggressive_v2.yaml"
        )
    ...
    def test_aggressive_v2_config_keeps_current_artifact_contract_compatibility(self):
        ...
        self.assertIn(
            str(iter_dir / "current_mcts1200_report.json"), rendered_benchmark
        )
    ...
    def test_v2_local_tuning_configs_preserve_aggressive_v2_contract_shape(self):
        ...
        for filename in self.V2_LOCAL_CONFIG_EXPECTATIONS:
            with self.subTest(config=filename):
                ...
                self.assertEqual(
                    base_config["replay_window"], variant_config["replay_window"]
                )
                self.assertEqual(
                    base_config["current_path"], variant_config["current_path"]
                )
        ...
        for filename, expected in self.V2_LOCAL_CONFIG_EXPECTATIONS.items():
            with self.subTest(config=filename):
                ...
                self.assertEqual(
                    ...
                    self.command_flag_value(
                        variant_steps["self_play"]["command"], "--games"
                    ),
                )
                self.assertEqual(
                    expected["self_play_simulations"],
                    self.command_flag_value(
                        variant_steps["self_play"]["command"], "--simulations"
                    ),
                )
                self.assertEqual(
                ...
                expected_self_play[
                    expected_self_play.index("--total-iterations") + 1
                ] = "1"
                expected_self_play[expected_self_play.index("--games") + 1] = expected[
                    "self_play_games"
                ]
                expected_self_play[expected_self_play.index("--simulations") + 1] = (
                    expected["self_play_simulations"]
                )
                expected_self_play[
                    expected_self_play.index("--temperature-late") + 1
                ] = expected["self_play_temperature_late"]
                ...
                expected_train[expected_train.index("--epochs") + 1] = expected[
                    "train_epochs"
                ]
                expected_train[expected_train.index("--hidden-sizes") + 1] = expected[
                    "train_hidden_sizes"
                ]
                ...
                self.assertIn(
                    f"--simulations {expected['bootstrap_simulations']}",
                    bootstrap_shell,
                )
                self.assertEqual(
                    bootstrap_shell,
                    base_bootstrap.replace(
                        "--games 600", f"--games {expected['bootstrap_games']}"
                    ).replace(
                        "--simulations 1200",
                        f"--simulations {expected['bootstrap_simulations']}",
                    )
                    if filename == "aggressive_v2_search_up_local.json"
                    else base_bootstrap.replace(
                        "--games 600", f"--games {expected['bootstrap_games']}"
                    ),
                )
    ...
    def test_search_quality_local_config_renders_expected_search_flags(self):
        ...
        self.assertEqual(
            "parent_q",
            self.command_flag_value(
                variant_steps["self_play"]["command"], "--fpu-mode"
            ),
        )
        ...
        self.assertEqual(
            "deterministic",
            self.command_flag_value(
                variant_steps["self_play"]["command"], "--root-policy-mode"
            ),
        )
        self.assertEqual(
            "0.1",
            self.command_flag_value(
                variant_steps["self_play"]["command"], "--tactical-root-bias"
            ),
        )
    ...
    def test_aggressive_v3_tactical_encoding_local_config_renders_kalah_v3_commands(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        base_config = load_config(
        ...
        self.assertEqual(
            "/tmp/azlite_v3_tactical_encoding_local_versions",
            variant_config["versions_dir"],
        )
        self.assertEqual(1, variant_config["iterations"])
        self.assertEqual(base_config["gates"], variant_config["gates"])
        ...
        for step_name in ("self_play", "train", "export_artifact"):
            ...
            self.assertEqual(
                "kalah_v3", self.command_flag_value(rendered, "--input-encoding")
            )
        ...
        self.assertEqual(
            self.command_flag_value(
                base_steps["export_artifact"]["command"], "--model-type"
            ),
            self.command_flag_value(
                variant_steps["export_artifact"]["command"], "--model-type"
            ),
        )
        ...
        self.assertNotEqual(
            base_steps["mcts_bootstrap_dataset"],
            variant_steps["mcts_bootstrap_dataset"],
        )

    def test_aggressive_v3_self_play_omits_incompatible_parent_checkpoint(self):
        repo_root = Path(__file__).resolve().parents[2]
        variant_config = self.load_v2_local_config(self.TACTICAL_ENCODING_LOCAL_CONFIG)
        self_play_command = self.config_steps_by_name(variant_config)["self_play"][
            "command"
        ]
        iter_dir = repo_root / "tmp" / "aggressive-v3-tactical-encoding-local-iter1"
        ...
        with tempfile.TemporaryDirectory(prefix="pipeline-parent-checkpoint-") as tmp:
            ...
            with mock.patch(
                "ml.alphazero_lite.pipeline.checkpoint_feature_count", return_value=10
            ):
                rendered = render_command(
                    self_play_command,
                    iteration=1,
    ...
    def test_aggressive_v3_tactical_replay_train_omits_incompatible_init_checkpoint(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        config = load_config(
    ...
    def test_render_command_drops_later_matching_parent_checkpoint_after_non_matching_occurrence(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
    ...
    def test_aggressive_v3_specialized_heads_local_config_renders_residual_v3_and_kalah_v3_commands(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        ...
        self.assertEqual(
            "aggressive-v3-specialized-heads-local", variant_config["run_id"]
        )
        self.assertEqual(
            "/tmp/azlite_v3_specialized_heads_local_versions",
            variant_config["versions_dir"],
        )
        self.assertEqual(1, variant_config["iterations"])
        self.assertEqual(base_config["gates"], variant_config["gates"])
        ...
        self.assertEqual(
            self.bootstrap_shell(base_config), self.bootstrap_shell(variant_config)
        )
        ...
        self.assertEqual(
            "kalah_v3", self.command_flag_value(self_play_rendered, "--input-encoding")
        )
        ...
        for step_name in ("train", "export_artifact"):
            ...
            self.assertEqual(
                "residual_v3", self.command_flag_value(rendered, "--model-type")
            )
            self.assertEqual(
                "kalah_v3", self.command_flag_value(rendered, "--input-encoding")
            )
    ...
    def test_aggressive_v3_specialized_heads_wide_local_only_changes_hidden_sizes(self):
        ...
        wide_config = self.load_v2_local_config(
            self.SPECIALIZED_HEADS_WIDE_LOCAL_CONFIG
        )
        ...
        self.assertEqual(
            "aggressive-v3-specialized-heads-wide-local", wide_config["run_id"]
        )
        self.assertEqual(
            "/tmp/azlite_v3_specialized_heads_wide_local_versions",
            wide_config["versions_dir"],
        )
        ...
        for step_name in (
            ...
            self.assertEqual(
                self.normalize_rendered_command(
                    self.render_config_step(base_config, step_name), config=base_config
                ),
                self.normalize_rendered_command(
                    self.render_config_step(wide_config, step_name), config=wide_config
                ),
            )

        for command in (base_train, wide_train, base_export, wide_export):
            self.assertEqual(
                "residual_v3", self.command_flag_value(command, "--model-type")
            )
            self.assertEqual(
                "kalah_v3", self.command_flag_value(command, "--input-encoding")
            )
        ...
        self.assertEqual(
            base_hidden_sizes.split(",")[1], wide_hidden_sizes.split(",")[1]
        )

        normalized_base_train = self.normalize_rendered_command(
            base_train, config=base_config
        )
        normalized_wide_train = self.normalize_rendered_command(
            wide_train, config=wide_config
        )
    ...
    def test_readme_documents_v3_specialized_heads_local_lane(self):
        ...
        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v3_specialized_heads_local.json",
            readme,
        )
    ...
    def test_readme_documents_v3_specialized_heads_wide_local_lane(self):
        ...
        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v3_specialized_heads_wide_local.json",
            readme,
        )
    ...
    def test_aggressive_v3_policy_target_local_only_changes_policy_target_mode(self):
        ...
        self.assertEqual(
            "kalah_v3", self.command_flag_value(variant_self_play, "--input-encoding")
        )
        self.assertEqual(
            "residual_v3", self.command_flag_value(variant_train, "--model-type")
        )
        self.assertEqual(
            "kalah_v3", self.command_flag_value(variant_train, "--input-encoding")
        )
        self.assertEqual(
            "sharpened",
            self.command_flag_value(variant_self_play, "--policy-target-mode"),
        )
        self.assertEqual(
            "sharpened", self.command_flag_value(variant_train, "--policy-target-mode")
        )
        self.assertIn(
            "--policy-target-mode sharpened", self.bootstrap_shell(variant_config)
        )
        ...
        for step_name in (
            ...
            self.assertEqual(
                ...
                self.normalize_rendered_command(
                    self.render_config_step(variant_config, step_name),
        ...
        self.assertEqual(
            expected_self_play,
            self.normalize_rendered_command(variant_self_play, config=variant_config),
        )
        ...
        self.assertEqual(
            expected_train,
            self.normalize_rendered_command(variant_train, config=variant_config),
        )

        self.assertEqual(
            self.bootstrap_shell(base_config).replace(
                base_config["run_id"], variant_config["run_id"]
            ),
            self.bootstrap_shell(variant_config).replace(
    ...
    def test_readme_documents_v3_policy_target_local_lane(self):
        ...
        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v3_policy_target_local.json", readme
        )
    ...
    def test_aggressive_v3_value_target_local_only_changes_value_target_mode(self):
        ...
        self.assertEqual(
            "/tmp/azlite_v3_value_target_local_versions", variant_config["versions_dir"]
        )
        self.assertEqual(base_config["seed"], variant_config["seed"])
        self.assertEqual(base_config["iterations"], variant_config["iterations"])
        self.assertEqual(base_config["replay_window"], variant_config["replay_window"])
        ...
        self.assertEqual(
            "sharpened",
            self.command_flag_value(variant_self_play, "--value-target-mode"),
        )
        self.assertEqual(
            "sharpened", self.command_flag_value(variant_train, "--value-target-mode")
        )
        self.assertIn(
            "--policy-target-mode sharpened", self.bootstrap_shell(variant_config)
        )
        self.assertIn(
            "--value-target-mode sharpened", self.bootstrap_shell(variant_config)
        )
    ...
    def test_readme_documents_v3_value_target_local_lane(self):
        ...
        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v3_value_target_local.json", readme
        )
    ...
    def test_aggressive_v3_value_target_aligned_local_only_changes_lane_identity(self):
        ...
        variant_config = self.load_v2_local_config(
            self.VALUE_TARGET_ALIGNED_LOCAL_CONFIG
        )
        variant_self_play = self.render_config_step(variant_config, "self_play")
        variant_train = self.render_config_step(variant_config, "train")

        self.assertEqual(
            "aggressive-v3-value-target-aligned-local", variant_config["run_id"]
        )
        self.assertEqual(
    ...
    def test_readme_documents_v3_value_target_aligned_local_lane(self):
        repo_root = Path(__file__).resolve().parents[2]
        readme = (repo_root / "ml/alphazero_lite/README.md").read_text(encoding="utf-8")
        aligned_section = readme.split(
            "### Aligned sharpened value-target lane", maxsplit=1
        )[1]
        ...
        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v3_value_target_aligned_local.json",
            readme,
        )
        self.assertIn(
            ".venv/bin/python ml/alphazero_lite/pipeline.py", aligned_commands
        )
    ...
    def test_aggressive_v3_capacity_local_only_changes_lane_identity_and_hidden_sizes(
        self,
    ):
        ...
        self.assertEqual(
            "/tmp/azlite_v3_capacity_local_versions", variant_config["versions_dir"]
        )
        self.assertNotIn("--checkpoint", variant_self_play)
        self.assertEqual(
            "96,3", self.command_flag_value(variant_train, "--hidden-sizes")
        )

        expected_config = json.loads(json.dumps(base_config))
        expected_config["run_id"] = variant_config["run_id"]
        expected_config["versions_dir"] = variant_config["versions_dir"]
        expected_self_play = self.config_steps_by_name(expected_config)["self_play"][
            "command"
        ]
        ...
        expected_steps["train"]["command"][
            expected_steps["train"]["command"].index("--hidden-sizes") + 1
        ] = "96,3"
        self.assertEqual(expected_config, variant_config)
    ...
    def test_readme_documents_v3_capacity_local_lane(self):
        ...
        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v3_capacity_local.json", readme
        )
        self.assertIn(
            ".venv/bin/python ml/alphazero_lite/pipeline.py", capacity_commands
        )
    ...
    def test_aggressive_v3_capacity_large_local_only_changes_lane_identity_and_hidden_sizes(
        self,
    ):
        base_config = self.load_v2_local_config(self.VALUE_TARGET_ALIGNED_LOCAL_CONFIG)
        ...
        self.assertEqual(
            "/tmp/azlite_v3_capacity_large_local_versions",
            variant_config["versions_dir"],
        )
        self.assertNotIn("--checkpoint", variant_self_play)
        self.assertEqual(
            "128,3", self.command_flag_value(variant_train, "--hidden-sizes")
        )
    ...
    def test_readme_documents_v3_capacity_large_local_lane(self):
        ...
        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v3_capacity_large_local.json", readme
        )
        self.assertIn(
            ".venv/bin/python ml/alphazero_lite/pipeline.py", capacity_commands
        )
    ...
    def test_aggressive_v3_stronger_bootstrap_local_only_changes_lane_identity_and_bootstrap_teacher(
        self,
    ):
        base_config = self.load_v2_local_config(self.V3_CAPACITY_LOCAL_CONFIG)
        variant_config = self.load_v2_local_config(
            self.V3_STRONGER_BOOTSTRAP_LOCAL_CONFIG
        )
        variant_bootstrap = self.render_config_step(
            variant_config, "mcts_bootstrap_dataset"
        )

        self.assertEqual(
            "aggressive-v3-stronger-bootstrap-local", variant_config["run_id"]
        )
        self.assertEqual(
            "/tmp/azlite_v3_stronger_bootstrap_local_versions",
            variant_config["versions_dir"],
        )
        self.assertEqual(
            "2400", self.command_flag_value(variant_bootstrap, "--simulations")
        )
    ...
    def test_readme_documents_v3_stronger_bootstrap_local_lane(self):
        ...
        commands = self.find_bash_block(
            section, containing=("aggressive_v3_stronger_bootstrap_local.json",)
        )

        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v3_stronger_bootstrap_local.json",
            readme,
        )
    ...
    def test_aggressive_v3_stronger_bootstrap_confirm_local_only_changes_lane_identity_and_seed_schedule(
        self,
    ):
        base_config = self.load_v2_local_config(self.V3_STRONGER_BOOTSTRAP_LOCAL_CONFIG)
        variant_config = self.load_v2_local_config(
            self.V3_STRONGER_BOOTSTRAP_CONFIRM_LOCAL_CONFIG
        )
        variant_self_play = self.render_config_step(variant_config, "self_play")

        self.assertEqual(
            "aggressive-v3-stronger-bootstrap-confirm-local", variant_config["run_id"]
        )
        self.assertEqual(
            "/tmp/azlite_v3_stronger_bootstrap_confirm_local_versions",
            variant_config["versions_dir"],
        )
        self.assertEqual(84, variant_config["seed"])
        self.assertEqual(
            "81,82,83", self.command_flag_value(variant_self_play, "--seed-sweep")
        )
        ...
        expected_steps = self.config_steps_by_name(expected_config)
        expected_steps["self_play"]["command"][
            expected_steps["self_play"]["command"].index("--seed-sweep") + 1
        ] = "81,82,83"
    ...
    def test_readme_documents_v3_stronger_bootstrap_confirm_local_lane(self):
        repo_root = Path(__file__).resolve().parents[2]
        readme = (repo_root / "ml/alphazero_lite/README.md").read_text(encoding="utf-8")
        section = readme.split("### Stronger bootstrap confirmation lane", maxsplit=1)[
            1
        ]
        commands = self.find_bash_block(
            section, containing=("aggressive_v3_stronger_bootstrap_confirm_local.json",)
        )

        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v3_stronger_bootstrap_confirm_local.json",
            readme,
        )
        self.assertIn("script/ai/local_promotion_gate", commands)
    ...
    def test_aggressive_v3_stronger_bootstrap_confirm_b_local_only_changes_lane_identity_and_seed_schedule(
        self,
    ):
        base_config = self.load_v2_local_config(self.V3_STRONGER_BOOTSTRAP_LOCAL_CONFIG)
        variant_config = self.load_v2_local_config(
            self.V3_STRONGER_BOOTSTRAP_CONFIRM_B_LOCAL_CONFIG
        )
        variant_self_play = self.render_config_step(variant_config, "self_play")

        self.assertEqual(
            "aggressive-v3-stronger-bootstrap-confirm-b-local", variant_config["run_id"]
        )
        self.assertEqual(
            "/tmp/azlite_v3_stronger_bootstrap_confirm_b_local_versions",
            variant_config["versions_dir"],
        )
        self.assertEqual(126, variant_config["seed"])
        self.assertEqual(
            "121,122,123", self.command_flag_value(variant_self_play, "--seed-sweep")
        )
    ...
    def test_aggressive_v3_stronger_bootstrap_confirm_c_local_only_changes_lane_identity_and_seed_schedule(
        self,
    ):
        base_config = self.load_v2_local_config(self.V3_STRONGER_BOOTSTRAP_LOCAL_CONFIG)
        variant_config = self.load_v2_local_config(
            self.V3_STRONGER_BOOTSTRAP_CONFIRM_C_LOCAL_CONFIG
        )
        variant_self_play = self.render_config_step(variant_config, "self_play")

        self.assertEqual(
            "aggressive-v3-stronger-bootstrap-confirm-c-local", variant_config["run_id"]
        )
        self.assertEqual(
            "/tmp/azlite_v3_stronger_bootstrap_confirm_c_local_versions",
            variant_config["versions_dir"],
        )
        self.assertEqual(168, variant_config["seed"])
        self.assertEqual(
            "161,162,163", self.command_flag_value(variant_self_play, "--seed-sweep")
        )
    ...
    def test_readme_documents_v3_stronger_bootstrap_confirmation_sweep(self):
        repo_root = Path(__file__).resolve().parents[2]
        readme = (repo_root / "ml/alphazero_lite/README.md").read_text(encoding="utf-8")
        section = readme.split("### Stronger bootstrap confirmation sweep", maxsplit=1)[
            1
        ]
        commands = self.find_bash_block(
            section,
            containing=("aggressive_v3_stronger_bootstrap_confirm_b_local.json",),
        )
    ...
    def test_aggressive_v3_stronger_bootstrap_more_data_local_only_changes_lane_identity_and_bootstrap_games(
        self,
    ):
        base_config = self.load_v2_local_config(self.V3_STRONGER_BOOTSTRAP_LOCAL_CONFIG)
        variant_config = self.load_v2_local_config(
            self.V3_STRONGER_BOOTSTRAP_MORE_DATA_LOCAL_CONFIG
        )
        variant_bootstrap = self.render_config_step(
            variant_config, "mcts_bootstrap_dataset"
        )
        ...
        self.assertEqual(
            "aggressive-v3-stronger-bootstrap-more-data-local", variant_config["run_id"]
        )
        self.assertEqual(
            "/tmp/azlite_v3_stronger_bootstrap_more_data_local_versions",
            variant_config["versions_dir"],
        )
    ...
    def test_readme_documents_v3_stronger_bootstrap_more_data_local_lane(self):
        ...
        commands = self.find_bash_block(
            section,
            containing=("aggressive_v3_stronger_bootstrap_more_data_local.json",),
        )
    ...
    def test_aggressive_v3_hybrid_value_target_local_only_changes_value_target_mode(
        self,
    ):
        base_config = self.load_v2_local_config(self.VALUE_TARGET_LOCAL_CONFIG)
        variant_config = self.load_v2_local_config(
            self.HYBRID_VALUE_TARGET_LOCAL_CONFIG
        )
        variant_self_play = self.render_config_step(variant_config, "self_play")
        variant_train = self.render_config_step(variant_config, "train")

        self.assertEqual(
            "aggressive-v3-hybrid-value-target-local", variant_config["run_id"]
        )
        self.assertEqual(
            "/tmp/azlite_v3_hybrid_value_target_local_versions",
            variant_config["versions_dir"],
        )
        self.assertEqual(base_config["seed"], variant_config["seed"])
        self.assertEqual(base_config["iterations"], variant_config["iterations"])
        ...
        self.assertEqual(
            "hybrid", self.command_flag_value(variant_self_play, "--value-target-mode")
        )
        self.assertEqual(
            "hybrid", self.command_flag_value(variant_train, "--value-target-mode")
        )
        self.assertIn(
            "--policy-target-mode sharpened", self.bootstrap_shell(variant_config)
        )
        self.assertIn(
            "--value-target-mode hybrid", self.bootstrap_shell(variant_config)
        )
        ...
        expected_self_play[expected_self_play.index("--value-target-mode") + 1] = (
            "hybrid"
        )
        self.assertEqual(
            expected_self_play,
    ...
    def test_readme_documents_v3_hybrid_value_target_local_lane(self):
        ...
        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v3_hybrid_value_target_local.json",
            readme,
        )
    ...
    def test_aggressive_v3_phase_aware_value_target_local_only_changes_value_target_mode(
        self,
    ):
        base_config = self.load_v2_local_config(self.VALUE_TARGET_LOCAL_CONFIG)
        variant_config = self.load_v2_local_config(
            self.PHASE_AWARE_VALUE_TARGET_LOCAL_CONFIG
        )
        variant_self_play = self.render_config_step(variant_config, "self_play")
        variant_train = self.render_config_step(variant_config, "train")

        self.assertEqual(
            "aggressive-v3-phase-aware-value-target-local", variant_config["run_id"]
        )
        self.assertEqual(
            "/tmp/azlite_v3_phase_aware_value_target_local_versions",
            variant_config["versions_dir"],
        )
        self.assertEqual(base_config["seed"], variant_config["seed"])
        self.assertEqual(base_config["iterations"], variant_config["iterations"])
        ...
        self.assertEqual(
            "phase_aware_sharpened",
            self.command_flag_value(variant_train, "--value-target-mode"),
        )
        self.assertIn(
            "--policy-target-mode sharpened", self.bootstrap_shell(variant_config)
        )
        self.assertIn(
            "--value-target-mode phase_aware_sharpened",
            self.bootstrap_shell(variant_config),
        )
        ...
        expected_train[expected_train.index("--value-target-mode") + 1] = (
            "phase_aware_sharpened"
        )
        self.assertEqual(
            expected_train,
    ...
    def test_readme_documents_v3_phase_aware_value_target_local_lane(self):
        repo_root = Path(__file__).resolve().parents[2]
        readme = (repo_root / "ml/alphazero_lite/README.md").read_text(encoding="utf-8")
        phase_aware_section = readme.split(
            "### Phase-aware value-target lane", maxsplit=1
        )[1]
        ...
        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v3_phase_aware_value_target_local.json",
            readme,
        )
        self.assertIn(
            ".venv/bin/python ml/alphazero_lite/pipeline.py", phase_aware_commands
        )
    ...
    def test_readme_documents_v2_local_promotion_gate_screening_without_special_flags(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        readme = (repo_root / "ml/alphazero_lite/README.md").read_text(encoding="utf-8")
        ...
        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v2.yaml", local_gate_section
        )
        self.assertIn(
            "dry run only confirms the planned iteration path/name", normalized_section
        )
    ...
    def test_readme_documents_search_quality_local_ablation_lane(self):
        ...
        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v2_search_quality_local.json", readme
        )
        self.assertIn(
    ...
    def test_readme_documents_v3_tactical_encoding_local_lane(self):
        ...
        self.assertIn(
            "ml/alphazero_lite/configs/aggressive_v3_tactical_encoding_local.json",
            readme,
        )
    ...
    def test_handoff_documents_forensic_suite_command(self):
        repo_root = Path(__file__).resolve().parents[2]
        handoff = (repo_root / "docs/alphazero-lite-ml-handoff.md").read_text(
            encoding="utf-8"
        )
    ...
    def test_handoff_documents_tactical_replay_launcher_entrypoint(self):
        ...
        tactical_section = handoff.split("## Tactical Hard-State Replay Lane", 1)[
            1
        ].split(
            "Implementation flow:",
            1,
    ...
    def test_bootstrap_300_config_renders_expected_dataset_step(self):
        repo_root = Path(__file__).resolve().parents[2]
        config = load_config(
            repo_root / "ml/alphazero_lite/configs/aggressive_v1_bootstrap_300.yaml"
        )
        step = next(
            step for step in config["steps"] if step["name"] == "mcts_bootstrap_dataset"
        )
        iter_dir = repo_root / "tmp" / "aggressive-v1-iter1"
    ...
    def test_bootstrap_300_config_only_changes_bootstrap_dataset_game_count(self):
        ...
        bootstrap_index = next(
            ...
            if step["name"] == "mcts_bootstrap_dataset"
        )
    ...
    def test_stronger_bootstrap_config_uses_smaller_but_stronger_teacher(self):
        ...
        variant_config = load_config(
            repo_root
            / "ml/alphazero_lite/configs/aggressive_v1_bootstrap_stronger.yaml"
        )
        ...
        base_bootstrap = " ".join(
            next(
                step for step in base_steps if step["name"] == "mcts_bootstrap_dataset"
            )["command"]
        )
        variant_bootstrap = " ".join(
            next(
                step
        ...
        self.assertEqual(
            base_bootstrap.replace("--games 1200", "--games 600").replace(
                "--simulations 1200", "--simulations 2400"
            ),
            variant_bootstrap,
        )
    ...
    def test_data_mix_configs_only_change_bootstrap_share(self):
        ...
        light_config = load_config(
            repo_root / "ml/alphazero_lite/configs/aggressive_v1_mix_light.yaml"
        )
        medium_config = load_config(
            repo_root / "ml/alphazero_lite/configs/aggressive_v1_mix_medium.yaml"
        )
        heavy_config = load_config(
            repo_root / "ml/alphazero_lite/configs/aggressive_v1_mix_heavy.yaml"
        )
        ...
        base_steps = {step["name"]: step for step in base_config["steps"]}
        variant_steps = [
            {step["name"]: step for step in variant["steps"]} for variant in variants
        ]
        ...
        light_bootstrap = " ".join(
            variant_steps[0]["mcts_bootstrap_dataset"]["command"]
        )
        medium_bootstrap = " ".join(
            variant_steps[1]["mcts_bootstrap_dataset"]["command"]
        )
        heavy_bootstrap = " ".join(
            variant_steps[2]["mcts_bootstrap_dataset"]["command"]
        )
        ...
        self.assertEqual(
            "{replay_weights},2", light_train[light_train.index("--replay-weights") + 1]
        )
        self.assertEqual(
            "{replay_weights},4",
            medium_train[medium_train.index("--replay-weights") + 1],
        )
        self.assertEqual(
            "{replay_weights},6", heavy_train[heavy_train.index("--replay-weights") + 1]
        )
    ...
    def test_staged_strict_candidate_config_adds_bootstrap_then_selfplay_training(self):
        ...
        candidate_config = load_config(
            repo_root / "ml/alphazero_lite/configs/aggressive_v1_staged_candidate.yaml"
        )
        ...
        self.assertEqual(
            "{replay_weights},8",
            bootstrap_train[bootstrap_train.index("--replay-weights") + 1],
        )
        ...
        self.assertIn(
            "{iter_dir}/bootstrap_checkpoint.npz",
            self_play_train[self_play_train.index("--init-checkpoint") + 1],
        )
        self.assertEqual(
            "{iter_dir}/mcts1200_report.json",
            benchmark_step["command"][
                benchmark_step["command"].index("--mcts-report") + 1
            ],
        )
    ...
    def test_local_promotion_gate_dry_run_writes_expected_report_plan(self):
        ...
        with tempfile.TemporaryDirectory(prefix="local-promotion-gate-") as tmp:
                ...
                [
                    {
                        ...
                        "games": 120,
                        "report_path": str(
                            tmp_path / "candidate_vs_current_arena.json"
                        ),
                    },
                    {
            ...
            forensic_evaluation = next(
                evaluation
                for evaluation in report["evaluations"]
                if evaluation["id"] == "candidate_forensic_suite"
            )
            self.assertEqual(
                str(tmp_path / "candidate_forensic_suite.json"),
                forensic_evaluation["report_path"],
            )
            ...
            self.assertEqual(
                ...
                [
                    forensic_evaluation["command"][
                        forensic_evaluation["command"].index("--challenger-artifact")
                        + 1
                    ],
                    forensic_evaluation["command"][
                        forensic_evaluation["command"].index("--current-artifact") + 1
                    ],
                    forensic_evaluation["command"][
                        forensic_evaluation["command"].index("--mcts-simulations") + 1
                    ],
                    forensic_evaluation["command"][
                        forensic_evaluation["command"].index("--teacher-simulations")
                        + 1
                    ],
                ],
            )
    ...
    def test_local_promotion_gate_fails_when_arena_score_below_threshold(self):
        ...
        self.assertEqual(
            [{"code": "arena_score_below_threshold"}], report["failure_reasons"]
        )
    ...
    def test_local_promotion_gate_fails_when_candidate_mcts_below_current(self):
        ...
        self.assertFalse(report["passed"])
        self.assertEqual(
            [{"code": "candidate_mcts_below_current"}], report["failure_reasons"]
        )
    ...
    def test_local_promotion_gate_fails_when_arena_games_below_minimum(self):
        ...
        self.assertFalse(report["passed"])
        self.assertEqual(
            [{"code": "arena_games_below_minimum"}], report["failure_reasons"]
        )
    ...
    def test_local_promotion_gate_fails_in_lossless_mode_when_any_loss_occurs(self):
        ...
        self.assertEqual(
            [{"code": "arena_losses_above_threshold"}], report["failure_reasons"]
        )

    def test_local_promotion_gate_runs_real_evaluations_and_writes_decision_report(
        self,
    ):
        ...
        with tempfile.TemporaryDirectory(prefix="local-promotion-gate-") as tmp:
            ...
            def fake_run(command, cwd=None, capture_output=None, text=None, check=None):
                ...
                self.write_real_gate_command_output(
                    command, candidate_path=candidate_path
                )
            ...
            self.assertEqual(
                [
                    ...
                    str(
                        tmp_path
                        / "endgame_exact_solve_threshold_8_candidate_vs_mcts1200.json"
                    ),
                    str(
                        tmp_path
                        / "endgame_exact_solve_threshold_8_current_vs_mcts1200.json"
                    ),
                    str(
                        tmp_path
                        / "endgame_exact_solve_threshold_10_candidate_vs_mcts1200.json"
                    ),
                    str(
                        tmp_path
                        / "endgame_exact_solve_threshold_10_current_vs_mcts1200.json"
                    ),
                    str(
                        tmp_path
                        / "endgame_exact_solve_threshold_12_candidate_vs_mcts1200.json"
                    ),
                    str(
                        tmp_path
                        / "endgame_exact_solve_threshold_12_current_vs_mcts1200.json"
                    ),
                ],
            ...
            self.assertEqual([], report["failure_reasons"])
            self.assertTrue(
                report["forensic_report_path"].endswith("candidate_forensic_suite.json")
            )

    def test_local_promotion_gate_forwards_search_quality_flags_from_config_to_real_commands(
        self,
    ):
        module = self.load_local_promotion_gate_module()
        ...
        with tempfile.TemporaryDirectory(prefix="local-promotion-gate-") as tmp:
            ...
            for command in calls:
                ...
                if (
                    len(command) > 1
                    and Path(command[1]).name == "run_forensic_suite.py"
                ):
                    self.assertEqual(
                        str(tmp_path / "candidate_forensic_suite.json"),
                    ...
                    self.assertEqual(
                        "1200", command[command.index("--mcts-simulations") + 1]
                    )
                    self.assertEqual(
                        "1800", command[command.index("--teacher-simulations") + 1]
                    )
                    continue
                if (
                    command[1].endswith("arena.py")
                    and "candidate_vs_hard_arena" not in command
                ):
                    self.assertIn("--fpu-mode", command)
                    self.assertIn("parent_q", command)
                    self.assertIn("--reuse-subtree", command)
    ...
    def test_local_promotion_gate_allows_phase1_config_without_required_real_search_steps(
        self,
    ):
        module = self.load_local_promotion_gate_module()
    ...
    def test_local_promotion_gate_ignores_stale_regression_report_when_command_fails(
        self,
    ):
        module = self.load_local_promotion_gate_module()
        ...
        with tempfile.TemporaryDirectory(prefix="local-promotion-gate-") as tmp:
            ...
            stale_regression_path.write_text(
                json.dumps({"passed": True, "results": []}), encoding="utf-8"
            )
            ...
            def fake_run(command, cwd=None, capture_output=None, text=None, check=None):
                if (
                    ...
                    return subprocess.CompletedProcess(
                        command, 1, stdout="", stderr="regression exploded"
                    )
                self.write_real_gate_command_output(
                    command, candidate_path=candidate_path
                )
    ...
    def test_local_promotion_gate_executes_threshold_specific_exact_solve_evaluations(
        self,
    ):
        module = self.load_local_promotion_gate_module()
        ...
        with tempfile.TemporaryDirectory(prefix="local-promotion-gate-") as tmp:
            ...
            def fake_run(command, cwd=None, capture_output=None, text=None, check=None):
                ...
                if (
                    len(command) > 1
                    and Path(command[1]).name == "mcts1200_baseline.py"
                    and "--exact-solve-stone-threshold" in command
                ):
                    threshold = int(
                        command[command.index("--exact-solve-stone-threshold") + 1]
                    )
                    payload = {
            ...
            self.assertEqual(
                [8, 8, 10, 10, 12, 12],
                [
                    int(command[command.index("--exact-solve-stone-threshold") + 1])
            ...
            self.assertEqual(
                ...
                [
                    result["threshold"]
                    for result in report["endgame_exact_solve"]["results"]
                ],
            )
            self.assertEqual(
                [0.7, 0.75, 0.8],
    ...
    def test_local_promotion_gate_forwards_config_search_flags_to_threshold_exact_solve_evaluations(
        self,
    ):
        module = self.load_local_promotion_gate_module()
    ...
    def test_local_promotion_gate_surfaces_real_subprocess_failures_with_command_context(
        self,
    ):
        module = self.load_local_promotion_gate_module()
        ...
        for label, expected_calls, failing_script, stderr_text in cases:
            with (
                ...
                tempfile.TemporaryDirectory(prefix="local-promotion-gate-") as tmp,
            ):
                tmp_path = Path(tmp)
                candidate_path = tmp_path / "candidate"
                current_path = tmp_path / "current"
                ...
                def fake_run(
                    ...
                    if script_name == failing_script:
                        return subprocess.CompletedProcess(
                            command, 1, stdout="", stderr=stderr_text
                        )
                    self.write_real_gate_command_output(
                        command, candidate_path=candidate_path
                    )
    ...
    def test_local_mix_ablation_runner_dry_run_writes_reduced_local_config(self):
        ...
        self_play_command = next(
            step["command"]
            for step in generated_config["steps"]
        ...
        self.assertEqual(
            "400", self_play_command[self_play_command.index("--games") + 1]
        )
        self.assertEqual(
            "60", bootstrap_command[bootstrap_command.index("--games") + 1]
        )
    ...
    def test_local_staged_ablation_runner_dry_run_writes_two_stage_configs(self):
        ...
        stage1_train = next(
            step["command"]
            for step in stage1_config["steps"]
            if step["name"] == "train"
        )
        stage2_train = next(
            step["command"]
            for step in stage2_config["steps"]
            if step["name"] == "train"
        )
        stage2_bootstrap = next(
            step["command"]
            for step in stage2_config["steps"]
            if step["name"] == "mcts_bootstrap_dataset"
        )
        ...
        self.assertIn(
            f"{report['run_id']}-stage1-iter1/checkpoint.npz",
            stage2_train[stage2_train.index("--init-checkpoint") + 1],
        )
        ...
        for preset, expected_replay_weights in presets.items():
            result = subprocess.run(
                [
                    ...
                    str(
                        repo_root
                        / "ml/alphazero_lite/configs/aggressive_v1_mix_heavy.yaml"
                    ),
                    "--run-id",
                    f"{preset}-test",
                    "--stage2-preset",
            ...
            stage2_config = json.loads(
                Path(report["stage2_config_path"]).read_text(encoding="utf-8")
            )
            stage2_train = next(
                step["command"]
                for step in stage2_config["steps"]
    ...
    def test_local_staged_ablation_runner_supports_wider_student_preset(self):
        ...
        stage1_config = json.loads(
            Path(report["stage1_config_path"]).read_text(encoding="utf-8")
        )
        stage2_config = json.loads(
            Path(report["stage2_config_path"]).read_text(encoding="utf-8")
        )
        ...
        self.assertEqual(
            "256,256,128", stage1_train[stage1_train.index("--hidden-sizes") + 1]
        )
        self.assertEqual(
            "256,256,128", stage2_train[stage2_train.index("--hidden-sizes") + 1]
        )
        self.assertIn("--init-checkpoint", stage2_train)
        self.assertNotIn("--data-files", stage2_train)
    ...
    def test_local_staged_ablation_runner_supports_stage2_optimization_presets(self):
        ...
        for preset, (expected_epochs, expected_lr) in presets.items():
            ...
            self.assertEqual(
                expected_epochs, stage2_train[stage2_train.index("--epochs") + 1]
            )
            self.assertIn("--init-checkpoint", stage2_train)
            self.assertNotIn("--data-files", stage2_train)
            ...
            if expected_lr is None:
                ...
                self.assertEqual(
                    expected_lr, stage2_train[stage2_train.index("--lr") + 1]
                )
    ...
    def test_start_iteration_produces_iter2_and_iter3_dirs_with_replay_context(self):
        ...
        with tempfile.TemporaryDirectory(prefix="pipeline-start-iter-") as tmp:
            ...
            self.assertTrue(
                (versions_dir / f"{run_id}-iter2").exists(), "iter2 dir missing"
            )
            self.assertTrue(
                (versions_dir / f"{run_id}-iter3").exists(), "iter3 dir missing"
            )

            # iter2 replay context must include iter1 data
            replay_capture_iter2 = (
                versions_dir / f"{run_id}-iter2" / "replay_capture.txt"
            ).read_text(encoding="utf-8")
            self.assertIn(f"{run_id}-iter2/self_play.jsonl", replay_capture_iter2)
            self.assertIn(
                f"{run_id}-iter1/self_play.jsonl",
                replay_capture_iter2,
                "iter1 data missing from iter2 replay context",
            )

            # iter3 replay context must include iter2 and iter1 data
            replay_capture_iter3 = (
                versions_dir / f"{run_id}-iter3" / "replay_capture.txt"
            ).read_text(encoding="utf-8")
    ...
    def test_skip_before_final_iteration_skips_on_non_final_and_executes_on_final(self):
        ...
        with tempfile.TemporaryDirectory(prefix="pipeline-skip-final-") as tmp:
            ...
            self.assertIn(
                "final_only_gate",
                iter1_step_names,
                "gate step must appear in iter1 manifest",
            )
            gate_iter1 = next(
                s for s in iter1_manifest["steps"] if s["name"] == "final_only_gate"
            )
            ...
            iter2_manifest = json.loads(
                (versions_dir / f"{run_id}-iter2" / "run_manifest.json").read_text(
                    encoding="utf-8"
                )
            )
            gate_iter2 = next(
                s for s in iter2_manifest["steps"] if s["name"] == "final_only_gate"
            )
            ...
            self.assertTrue(
                sentinel_file.exists(), "gate command must run on the final iteration"
            )
            self.assertEqual(
                1,
