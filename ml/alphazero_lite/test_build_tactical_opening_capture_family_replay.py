...
class BuildTacticalOpeningCaptureFamilyReplayTest(unittest.TestCase):
    def test_default_opening_capture_family_source_stays_within_repo(self):
        from ml.alphazero_lite import build_tactical_opening_capture_family_replay as module
    ...
    def test_select_opening_capture_family_rows_requires_teacher_move_three_and_opening_phase(self):
        from ml.alphazero_lite import build_tactical_opening_capture_family_replay as module

        source_rows = [
            self.capture_row("valid-ply", priority=9.0, ply=4),
            self.capture_row("wrong-teacher", priority=8.0, ply=4, teacher_selected_move=2),
            self.capture_row("late-ply", priority=7.0, ply=5),
            self.capture_row("wrong-legal-moves", priority=6.0, ply=4, legal_moves=[0, 1, 2, 3]),
    ...
    def test_select_opening_capture_family_rows_keeps_highest_priority_row_per_family_provenance(self):
        from ml.alphazero_lite import build_tactical_opening_capture_family_replay as module
    ...
    def test_build_opening_capture_family_replay_dataset_writes_explicit_roles(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            source_rows = [
                ...
                self.preservation_capture_row("capture-preservation-1", priority=7.0, variant="gain_4_5"),
                self.preservation_capture_row("capture-preservation-2", priority=6.0, variant="gain_2_3"),
            ]
            ...
            self.assertEqual(
                ["capture_protection"] + ["capture_preservation"] * 2 + ["opening_capture_family"] * 2 + ["nearby_preservation"] * 6,
    ...
    def test_default_opening_capture_family_source_builds_opening_rows_and_sanitizes_artifacts(self):
        from ml.alphazero_lite import build_tactical_opening_capture_family_replay as module
        ...
        opening_rows = [row for row in rows if row["replay_role"] == "opening_capture_family"]
    ...
    def test_default_opening_capture_family_source_retains_capture_protection_anchor(self):
        from ml.alphazero_lite import build_tactical_opening_capture_family_replay as module
        ...
        protection_rows = [row for row in rows if row["replay_role"] == "capture_protection"]
    ...
    def test_default_opening_capture_family_source_limits_family_rows_to_three_distinct_provenance_ids(self):
        from ml.alphazero_lite import build_tactical_opening_capture_family_replay as module
        ...
        family_rows = [row for row in rows if row["replay_role"] == "opening_capture_family"]
    ...
    def test_default_opening_capture_family_source_retains_two_capture_preservation_rows(self):
        from ml.alphazero_lite import build_tactical_opening_capture_family_replay as module
        ...
        preservation_rows = [row for row in rows if row["replay_role"] == "capture_preservation"]
