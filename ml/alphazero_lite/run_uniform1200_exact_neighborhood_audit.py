...
def solve_exact(
    ...
    # keep this evaluation-only module importable without a model runtime.
    from ml.alphazero_lite.exact_teacher_labeling import (
        ExactLabelError,
        exact_margin_to_training_value,
        validate_native_label,
    )
    ...
    try:
        for row in cohort:
            ...
            try:
                ...
                row.update(
                    {
                        "oracle_status": "exact_solved",
                        "exact_value": root,
                        "exact_action_values": values,
                        "exact_optimal_actions": sorted(result["optimal_actions"]),
                        "exact_root_value": exact_margin_to_training_value(
                            exact_margin=root, current_player=game.current_player
                        ),
                    }
                )
                ...
                row.update(
                    {
                        "oracle_status": "exact_timeout",
                        "oracle_failure_reason": str(error),
                    }
                )
                worker.replace()
            except (NativeRequestError, ExactLabelError, KeyError, ValueError) as error:
                row.update(
                    {
                        "oracle_status": "exact_error",
                        "oracle_failure_reason": str(error),
                    }
                )
                worker.replace()
