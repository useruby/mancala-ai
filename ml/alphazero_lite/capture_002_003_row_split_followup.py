...
def _validate_selected_artifact(selected_artifact: dict) -> dict:
    ...
    if not isinstance(path, str) or not path:
        raise ValueError("source shared drift artifact selected_artifact must include non-empty path")
...
def _validate_probe_mode_failure_paths(paired_summary: dict) -> dict:
    ...
    if not isinstance(failure_paths, dict):
        raise ValueError("source shared drift artifact must include paired_summary probe_mode_failure_paths")

    normalized_failure_paths = {}
    for row_id in ROW_IDS:
        row_failure_paths = failure_paths.get(row_id)
        if not isinstance(row_failure_paths, dict):
            raise ValueError(f"source shared drift artifact paired_summary missing row {row_id}")
...
def _validate_probe_mode_selected_moves(*, paired_summary: dict, rows: dict) -> dict:
    ...
    if not isinstance(selected_moves, dict):
        raise ValueError("source shared drift artifact must include paired_summary probe_mode_selected_moves")

    normalized_selected_moves = {}
    for row_id in ROW_IDS:
        row_selected_moves = selected_moves.get(row_id)
        if not isinstance(row_selected_moves, dict):
            raise ValueError(f"source shared drift artifact paired_summary probe_mode_selected_moves missing row {row_id}")
...
def _validate_shared_mechanism_supported(*, paired_summary: dict) -> bool:
    ...
    if not isinstance(shared_mechanism_supported, bool):
        raise ValueError("source shared drift artifact paired_summary shared_mechanism_supported must be bool")
...
def _validate_settings(settings: dict) -> dict:
    ...
    if not isinstance(search_settings, dict):
        raise ValueError("source shared drift artifact must include settings search_settings")
    for key in REQUIRED_FULL_SEARCH_SETTINGS_KEYS:
        if key not in search_settings:
            raise ValueError(f"source shared drift artifact search_settings missing required key: {key}")
...
def _validate_required_move(*, value, legal_moves: list[int], context: str) -> int:
    ...
    if value not in legal_moves:
        raise ValueError(f"source shared drift artifact {context} must be present in legal_moves")
    return int(value)


def _validate_canonical_state(*, canonical_state: str, reference_move: int, row_id: str) -> str:
    try:
    ...
    if not isinstance(state, dict):
        raise ValueError(f"source shared drift artifact row {row_id} canonical_state must be a JSON object")
...
def _validate_required_alias(*, row_id: str, field_name: str, row_value, nested_value) -> None:
    if row_value != nested_value:
        raise ValueError(f"source shared drift artifact row {row_id} {field_name} must match full_search alias")


def _require_row_alias_field(*, row: dict, row_id: str, field_name: str):
    if field_name not in row:
        raise ValueError(f"source shared drift artifact row {row_id} {field_name} is required")
...
def _require_trace_presence(*, row_id: str, probe_mode: str, trace: dict, missing_fields: list[str], legal_moves: list[int]) -> dict:
    if not isinstance(trace, dict):
        raise ValueError(f"source shared drift artifact row {row_id} missing probe_mode_traces.{probe_mode}")
    ...
    if probe_mode == "full_search":
        if not isinstance(root_start, dict):
            raise ValueError(f"source shared drift artifact row {row_id} full_search root_start must be a dict")
        if not isinstance(snapshots, list):
            raise ValueError(f"source shared drift artifact row {row_id} full_search snapshots must be a list")
        if not snapshots:
            raise ValueError(f"source shared drift artifact row {row_id} full_search snapshots are required")
        if not isinstance(final_deltas, dict):
            raise ValueError(f"source shared drift artifact row {row_id} full_search final_deltas must be a dict")
    ...
    if probe_mode == "value_only" and row_id == "capture_available-003":
        ...
        if isinstance(snapshots, list) and snapshots and "value_only.snapshots" in missing_field_set:
            raise ValueError(
        ...
        snapshots_missing = snapshots is None or (isinstance(snapshots, list) and not snapshots)
        allow_upstream_empty_value_trace = root_start is None and snapshots == [] and not missing_field_set
        if root_start_missing and "value_only.root_start" not in missing_field_set and not allow_upstream_empty_value_trace:
            raise ValueError(
        ...
        if snapshots_missing and "value_only.snapshots" not in missing_field_set and not allow_upstream_empty_value_trace:
            raise ValueError(
    ...
    if probe_mode in CONSUMED_TRACE_KEYS.get(row_id, set()):
        if not isinstance(final_deltas, dict):
            raise ValueError(f"source shared drift artifact row {row_id} {probe_mode} final_deltas must be a dict")
        for key in ["selected_visits", "reference_visits"]:
            if key not in final_deltas:
                raise ValueError(
        ...
        def validate_trace_point(point: dict, *, context: str) -> None:
            ...
            if isinstance(point_selected_move, bool) or not isinstance(point_selected_move, int):
                raise ValueError(
            ...
            _finite_number(simulation, context=f"row {row_id} {probe_mode} {context} simulation")
            visits = point["visits"]
            ...
            for visit in visits:
                _finite_number(visit, context=f"row {row_id} {probe_mode} {context} visits entry")
            ...
            for move_index, move in enumerate(moves):
                ...
                for required_key in ["move", "prior", "q_value", "selection_score", "used_fpu", "visit_count"]:
                ...
                if isinstance(move_value, bool) or not isinstance(move_value, int) or move_value not in legal_moves:
                    raise ValueError(
                ...
                for numeric_key in ["prior", "q_value", "selection_score", "visit_count"]:
                    _finite_number(
                        move[numeric_key],
            ...
            if set(seen_moves) != set(legal_moves) or len(seen_moves) != len(legal_moves):
                raise ValueError(
        ...
        for snapshot_index, snapshot in enumerate(normalized_trace.get("snapshots") or []):
...
def _normalize_row(row_id: str, row: dict) -> dict:
    ...
    if not isinstance(canonical_state, str) or not canonical_state:
        raise ValueError(f"source shared drift artifact row {row_id} canonical_state must be a non-empty string")
    ...
    if any(isinstance(move, bool) or not isinstance(move, int) for move in legal_moves):
        raise ValueError(f"source shared drift artifact row {row_id} legal_moves must contain ints")
    if any(move < 0 for move in legal_moves):
        raise ValueError(f"source shared drift artifact row {row_id} legal_moves must be non-negative")
    ...
    if not isinstance(probe_mode_traces, dict):
        raise ValueError(f"source shared drift artifact row {row_id} must include probe_mode_traces")
    ...
    row_root_start = _require_row_alias_field(row=row, row_id=row_id, field_name="root_start")
    row_snapshots = _require_row_alias_field(row=row, row_id=row_id, field_name="snapshots")
    row_final_deltas = _require_row_alias_field(row=row, row_id=row_id, field_name="final_deltas")
    _validate_required_alias(
        row_id=row_id,
...
def load_source_shared_drift_artifact(path: Path) -> dict:
    ...
    normalized_rows = {row_id: _normalize_row(row_id, rows.get(row_id)) for row_id in ROW_IDS}
    ...
    normalized_paired_summary["probe_mode_failure_paths"] = _validate_probe_mode_failure_paths(paired_summary)
    normalized_paired_summary["probe_mode_selected_moves"] = _validate_probe_mode_selected_moves(
        paired_summary=paired_summary,
        rows=rows,
    )
    normalized_paired_summary["shared_mechanism_supported"] = _validate_shared_mechanism_supported(
        paired_summary=paired_summary
    )
    ...
    for row_id in ROW_IDS:
        ...
        if (
            normalized_paired_summary["probe_mode_selected_moves"][row_id]["full_search"]
...
def _visit_share_from_trace(trace_point: dict | None, *, selected_move: int, reference_move: int) -> float | None:
    ...
    if not total_visits:
        return None
    return round((selected_entry["visit_count"] - reference_entry["visit_count"]) / total_visits, 10)


def _selected_minus_reference_metric(trace_point: dict | None, *, selected_move: int, reference_move: int, key: str):
    selected_entry = _move_entry(trace_point, selected_move)
    reference_entry = _move_entry(trace_point, reference_move)
    if selected_entry is None or reference_entry is None:
        return None
...
def _parse_rule_features(canonical_state: str, *, reference_move: int) -> dict:
    ...
    return {
        "player_empty_pits": [index for index, stones in enumerate(player_pits) if stones == 0],
        "opponent_empty_pits": [index for index, stones in enumerate(opponent_pits) if stones == 0],
...
def _build_lane_base(*, source_artifact: dict, row_id: str) -> tuple[dict, dict, dict]:
    ...
    return (
        ...
        {
            "row_id": row_id,
            "failure_path": paired_summary["probe_mode_failure_paths"][row_id]["full_search"],
            "canonical_state": row["canonical_state"],
            "legal_moves": list(row["legal_moves"]),
            "reference_move": row["reference_move"],
            "selected_moves_by_probe_mode": dict(paired_summary["probe_mode_selected_moves"][row_id]),
...
def build_lane_002(source_artifact: dict) -> dict:
    row, _, lane = _build_lane_base(source_artifact=source_artifact, row_id="capture_available-002")
    reference_move = row["reference_move"]
    ...
    lane["derived_metrics"] = {
        ...
        "root_start_reference_rank_by_selection_score": _selection_score_rank(root_start, reference_move),
        "root_start_selected_rank_by_selection_score": _selection_score_rank(root_start, selected_move),
        "root_start_reference_q": (_move_entry(root_start, reference_move) or {}).get("q_value"),
        "root_start_selected_q": (_move_entry(root_start, selected_move) or {}).get("q_value"),
        "root_start_reference_selection_score": (_move_entry(root_start, reference_move) or {}).get("selection_score"),
        "root_start_selected_selection_score": (_move_entry(root_start, selected_move) or {}).get("selection_score"),
...
def build_lane_003(source_artifact: dict) -> dict:
    row, _, lane = _build_lane_base(source_artifact=source_artifact, row_id="capture_available-003")
    reference_move = row["reference_move"]
    ...
    value_only_final_snapshot = value_only_snapshots[-1] if value_only_snapshots else None
    full_search_snapshots = full_search_trace.get("snapshots") or []
    full_search_final_snapshot = full_search_snapshots[-1] if full_search_snapshots else None
    ...
    lane["derived_metrics"] = {
        ...
        "policy_reference_prior": (_move_entry(policy_root_start, reference_move) or {}).get("prior"),
        "policy_value_selected_prior": (
            _move_entry(policy_root_start, value_only_trace.get("selected_move")) or {}
        ).get("prior"),
        "rule_features": _parse_rule_features(row["canonical_state"], reference_move=reference_move),
...
def _lane_002_snapshot_is_early(*, lane: dict, snapshot: float | None, thresholds: dict) -> bool:
    if snapshot is None:
        return False

    snapshots = ((lane.get("trace_inputs") or {}).get("full_search") or {}).get("snapshots") or []
    ...
    if len(snapshots) == minimum_count == 1:
        ...
        if isinstance(simulation_horizon, bool) or not isinstance(simulation_horizon, int) or simulation_horizon <= 0:
            return False
        return (float(snapshot) / float(simulation_horizon)) <= float(thresholds["early_snapshot_fraction"])

    last_snapshot = snapshots[-1] if snapshots else None
    last_simulation = (last_snapshot or {}).get("simulation") if isinstance(last_snapshot, dict) else None
    if isinstance(last_simulation, bool) or not isinstance(last_simulation, (int, float)) or last_simulation <= 0:
        return False

    return (float(snapshot) / float(last_simulation)) <= float(thresholds["early_snapshot_fraction"])
...
def classify_lane_002(*, lane: dict, thresholds: dict) -> dict:
    ...
    material_selection_score_margin = float(thresholds["material_selection_score_margin"])
    ...
    final_visit_share = derived_metrics.get("final_selected_minus_reference_visit_share")
    final_prior_margin = derived_metrics.get("final_selected_minus_reference_prior_margin")
    ...
    if (
        ...
        selection_score_gap = (
            float(derived_metrics["root_start_reference_selection_score"])
            - float(derived_metrics["root_start_selected_selection_score"])
        )

    first_selection_score_overtake = derived_metrics.get("first_selected_selection_score_overtake_snapshot")
    ...
    if (
        ...
        and (first_q_overtake is None or float(first_selection_score_overtake) < float(first_q_overtake))
    ):
        classification = "selection_score_overtake"
        evidence_summary = (
            "The selected move starts materially behind on root selection score and overtakes on selection score before Q does."
        )
    elif (
        _lane_002_snapshot_is_early(lane=lane, snapshot=first_visit_overtake, thresholds=thresholds)
        and final_q_margin is not None
        and float(final_q_margin) < small_q_margin
    ):
        classification = "early_fpu_pressure"
        evidence_summary = (
            "The selected move overtakes visits early while Q stays small, consistent with early FPU pressure rather than value evidence."
        )
    elif (
        _lane_002_snapshot_is_early(lane=lane, snapshot=first_q_overtake, thresholds=thresholds)
        and final_q_margin is not None
        and float(final_q_margin) >= meaningful_q_margin
    ):
        classification = "early_child_value_override"
        evidence_summary = (
            "The selected move gains an early Q overtake and finishes with a meaningful Q margin over the reference move."
        )
    elif (
        ...
        evidence_summary = (
            "The selected move only pulls ahead later in search, visit-share drift is meaningful, and the final Q margin stays small."
        )
    elif (
        final_q_margin is not None
        and float(final_q_margin) < small_q_margin
        ...
        evidence_summary = (
            "The selected move retains material prior pressure at the root while the final Q margin stays below the meaningful threshold."
        )
    else:
...
def _lane_003_trace_is_insufficient(*, lane: dict) -> bool:
    ...
    return (
        ...
        or derived_metrics.get("value_only_selected_minus_reference_visit_share") is None
    )
...
def classify_lane_003(*, lane: dict, thresholds: dict) -> dict:
    ...
    value_only_q_margin = derived_metrics.get("value_only_selected_minus_reference_q_margin")
    value_only_visit_share = derived_metrics.get("value_only_selected_minus_reference_visit_share")
    ...
    if _lane_003_trace_is_insufficient(lane=lane):
        ...
        evidence_summary = (
            "003 is missing value-only trace fields needed to interpret whether value evidence or visit amplification caused the divergence."
        )
    elif (
        ...
        and (
            (value_only_q_margin is not None and float(value_only_q_margin) >= meaningful_q_margin)
            or (
        ...
        evidence_summary = (
            "Policy keeps the reference move on top while value-only evidence prefers a different move, indicating a policy-value conflict in 003."
        )
    elif (
        value_only_selected_move != reference_move
        and policy_only_selected_move == reference_move
        ...
        evidence_summary = (
            "003 carries state-specific rule features that can collide with value evidence around the reference move."
        )
    elif (
        value_only_selected_move != reference_move
        and policy_only_selected_move == reference_move
        ...
        evidence_summary = (
            "Value-only child Q evidence prefers the wrong move over the reference move strongly enough to explain the 003 divergence."
        )
    elif (
        value_only_selected_move != reference_move
        ...
        evidence_summary = (
            "Value-only search amplifies visits toward the wrong move without a meaningful Q advantage over the reference move."
        )
    else:
        classification = "unresolved"
...
def top_level_decision(*, lane_002: dict, lane_003: dict) -> str:
    ...
    if lane_003.get("classification") == "insufficient_value_trace" and _lane_003_trace_is_insufficient(lane=lane_003):
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Diagnose row split follow-up for capture_available-002 and -003")
    parser.add_argument("--source-shared-drift-artifact", type=Path, required=True)
    parser.add_argument("--out", type=Path, required=True)
    return parser.parse_args(argv)


def build_payload(*, source_artifact: dict, lane_002: dict, lane_003: dict, decision: str) -> dict:
    lanes = {
    ...
    return {
        ...
        "summary": {
            ...
            "lane_classifications": {row_id: lanes[row_id]["classification"] for row_id in ROW_IDS},
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({"artifact_path": str(args.out), "schema": SCHEMA, "decision": decision}, separators=(",", ":")))
    return 0
