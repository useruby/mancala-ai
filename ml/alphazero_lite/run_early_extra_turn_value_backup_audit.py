#!/usr/bin/env python3
"""
Child-afterstate value/backup audit for early_extra_turn.

Run child-afterstate value/backup audit for the early_extra_turn corrected
non-opening failure family selected by PR #67 (v7 family mining).
...
DEFAULT_MINING_SUMMARY_PATH = Path(
    "/tmp/azlite_corrected_non_opening_failure_mining_v7/"
    "non_opening_failure_family_summary_v7.json"
)
DEFAULT_SUMMARY_OUT = Path(
    "/tmp/azlite_early_extra_turn_value_backup_audit/"
    "early_extra_turn_value_backup_audit_summary.json"
)
DEFAULT_REPORT_OUT = Path(
    "docs/alphazero-lite-early-extra-turn-value-backup-audit-results.md"
)
EPS = 1e-9
FAMILY = "early_extra_turn"
SCHEMA = "azlite_early_extra_turn_value_backup_audit_v1"
OPTIONAL_ROOT_BUDGET = 2400
MAX_PROJECTED_ROOT_2400_SECONDS = 600.0

# Representative target rows from PR #67 (v7 mining)
...
def validate_and_enrich_rows(
    ...
    validation_context = {
        ...
        "context_rows_present": [
            row_id
            for row_id in ("early_extra_turn-017",)
            if row_id in {row.row_id for row in audit_rows}
        ],
...
def classify_family(
    ...
    if real_conflicts >= max(2, max(0, len(target_rows)) // 3):
        return (
            ...
            "produce a non-mutating tablebase-backed reference patch artifact for early_extra_turn and rerun the audit before training.",
            dict(counts),
        )
    if counts.get("value_head_miscalibration", 0) > max(
        ...
        return (
            ...
            "build a small train-only child-afterstate value-calibration artifact for early_extra_turn, with controls and no arena until local value metrics improve.",
            dict(counts),
        )
    ...
    if counts.get("root_selection_pressure", 0) > max(
        ...
        return (
            ...
            "run cpuct/root-prior calibration diagnostics for early_extra_turn.",
            dict(counts),
        )
    if counts.get("corrected_reference_suspicious", 0) > max(
        ...
        return (
            ...
            "adjudicate early_extra_turn references before training.",
            dict(counts),
        )
    if combined_tablebase_issue > 0 and tot_non_tie == 0:
        return (
            "tablebase_tie_logic_fixed_early_extra_turn_reauditable",
            "tablebase tie-break artifacts have been reclassified; rerun early_extra_turn value/backup audit cleanly under the fixed logic.",
            dict(counts),
        )
    return (
        "mixed_family_gap",
        "split early_extra_turn into mechanism-specific buckets and choose the largest stable bucket.",
        dict(counts),
    )
