...
class DistributionAlignedSelfPlayIterationTest(unittest.TestCase):
    def test_corpus_is_unique_disjoint_and_includes_all_opening_depths(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                {"states": 0, "prefixes": 0}, manifest["exact_overlap_counts"]
            )
    ...
    def test_preflight_requires_distance_improvement_no_leakage_and_strata(
        ...
        leaked = {**aligned, "exact_state_overlap": 1}
        self.assertIn("evaluation_state_leakage", preflight(control, leaked)[1])

    def test_distribution_audit_reports_exact_state_leakage(self) -> None:
        with TemporaryDirectory() as directory:
            path = Path(directory) / "suite.jsonl"
            state = {
                "player_pits": [4] * 6,
            ...
            self.assertEqual(1, distribution_audit(rows, [path])["exact_state_overlap"])
