...
class PriorSourceCoverageForIncumbentProxy033Test(unittest.TestCase):
    ...
    def minimal_artifacts(self) -> dict:
        return {
                ...
                "payload": {
                    ...
                    "rows": {
                        "incumbent_proxy_disagreement-033": {
                            "reference_move": 4,
                            "configs": {"full_default": {"selected_move": 0, "passes_reference": False}},
                        },
                        "capture_available-002": {
                            "reference_move": 2,
                            "configs": {"full_default": {"selected_move": 2, "passes_reference": True}},
                        },
                        ...
                        "incumbent_proxy_disagreement-031": {
                            "reference_move": 4,
                            "configs": {"full_default": {"selected_move": 4, "passes_reference": True}},
                        },
                    },
                },
            },
            "candidate_search_interaction": {
                ...
                "payload": {"schema": "azlite_search_interaction_diagnostic_v1", "rows": {}},
            },
    ...
    def test_build_payload_emits_required_schema_and_row_roles(self):
        ...
        self.assertEqual("azlite_prior_source_coverage_for_incumbent_proxy_033_v1", payload["schema"])
        self.assertEqual("incumbent_proxy_disagreement-033", payload["target_row_id"])
        self.assertEqual(["capture_available-002", "capture_available-003"], payload["guard_row_ids"])
        self.assertEqual(["incumbent_proxy_disagreement-031"], payload["context_row_ids"])
        self.assertEqual("target", payload["rows"]["incumbent_proxy_disagreement-033"]["row_role"])
        ...
        self.assertEqual("context", payload["rows"]["incumbent_proxy_disagreement-031"]["row_role"])

    def test_decision_writes_candidate_spec_for_absent_target_when_guards_are_clean(self):
        ...
        self.assertEqual("absent_from_source_construction", payload["summary"]["source_gap_classification"])
        self.assertEqual("write_targeted_source_coverage_candidate_spec", payload["summary"]["decision"])
        self.assertTrue(payload["summary"]["future_candidate_justified"])

    def test_decision_writes_candidate_spec_for_misclassified_target_when_guards_are_clean(self):
        artifacts = self.minimal_artifacts()
        artifacts["replay_source"]["memberships"] = {
            "incumbent_proxy_disagreement-033": {"present": True, "count": 1, "classification": "misclassified"},
            "capture_available-002": {"present": True, "count": 2},
            "capture_available-003": {"present": True, "count": 2},
        }
        ...
        self.assertEqual("misclassified_in_source_construction", payload["summary"]["source_gap_classification"])
        self.assertEqual("write_targeted_source_coverage_candidate_spec", payload["summary"]["decision"])

    def test_underrepresentation_is_diagnostic_only_without_missing_or_misclassified_signal(self):
        artifacts = self.minimal_artifacts()
        artifacts["replay_source"]["memberships"] = {
        ...
        self.assertEqual("underrepresented_in_source_construction", payload["summary"]["source_gap_classification"])
        self.assertEqual("close_033_source_coverage_variant", payload["summary"]["decision"])
        self.assertFalse(payload["summary"]["future_candidate_justified"])

    def test_reference_or_teacher_drift_is_diagnostic_only_without_missing_or_misclassified_signal(self):
        artifacts = self.minimal_artifacts()
        artifacts["candidate_search_interaction"]["payload"]["rows"] = {
            "incumbent_proxy_disagreement-033": {"reference_move": 4, "teacher_value": 0.65},
        }
        artifacts["rebalance_search_interaction"]["payload"]["rows"] = {
            "incumbent_proxy_disagreement-033": {"reference_move": 3, "teacher_value": 0.65},
        }

        payload = module.build_payload(source_artifacts=artifacts)

        self.assertEqual("reference_or_teacher_drift_detected", payload["summary"]["source_gap_classification"])
        self.assertEqual("close_033_source_coverage_variant", payload["summary"]["decision"])
    ...
    def test_misclassified_guard_replay_membership_blocks_candidate_spec(self):
        ...
        artifacts["replay_source"]["memberships"] = {
            "incumbent_proxy_disagreement-033": {"present": False, "count": 0},
            "capture_available-002": {"present": True, "count": 2, "classification": "misclassified"},
            "capture_available-003": {"present": True, "count": 2},
            "incumbent_proxy_disagreement-031": {"present": True, "count": 1},
        }
    ...
    def test_replay_memberships_ignores_substring_mentions_in_unrelated_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            replay_path.write_text('{"note":"incumbent_proxy_disagreement-033"}\n', encoding="utf-8")
    ...
    def test_replay_memberships_matches_canonical_states_without_row_ids(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                {"present": True, "count": 1, "matched_by": ["canonical_state"], "line_numbers": [1]},
                memberships["capture_available-002"],
            )
            self.assertEqual(
                {"present": True, "count": 1, "matched_by": ["canonical_state"], "line_numbers": [2]},
    ...
    def test_canonical_state_guard_presence_keeps_target_absence_actionable(self):
        artifacts = self.minimal_artifacts()
        artifacts["replay_source"]["memberships"] = {
            "incumbent_proxy_disagreement-033": {"present": False, "count": 0, "matched_by": [], "line_numbers": []},
            "capture_available-002": {
                "present": True,
                "count": 1,
            ...
            "incumbent_proxy_disagreement-031": {"present": False, "count": 0, "matched_by": [], "line_numbers": []},
        }
        ...
        self.assertEqual("guard-002-state", payload["rows"]["capture_available-002"]["canonical_state"])
        self.assertEqual(
            {"present": True, "count": 1, "matched_by": ["canonical_state"], "line_numbers": [10]},
    ...
    def test_wrong_bucket_replay_match_marks_target_misclassified(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            replay_path.write_text(
                "\n".join([
                    json.dumps({"canonical_state": "target-state", "bucket": "capture_available"}),
                    json.dumps({"canonical_state": "guard-002-state", "bucket": "capture_available"}),
                    json.dumps({"canonical_state": "guard-003-state", "bucket": "capture_available"}),
                    "",
                ]),
                encoding="utf-8",
            )
            ...
            target_membership = payload["rows"]["incumbent_proxy_disagreement-033"]["replay_source_membership"]
    ...
    def test_canonical_state_conflict_is_recorded_per_row(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.write_json(candidate_run / "final" / "selected_candidate_forensics.json", {
                "systems": {
                    "current": {
                        "rows": [{
            ...
            self.write_json(rebalance_run / "final" / "selected_candidate_forensics.json", {
                "systems": {
                    "current": {
                        "rows": [{
                            "id": "incumbent_proxy_disagreement-033",
            ...
            artifacts = self.minimal_artifacts()
            artifacts["canonical_state_provenance"] = module.load_canonical_state_provenance(
            ...
            self.assertEqual("candidate-state", row["canonical_state_provenance"]["candidate_canonical_state"])
            self.assertEqual("rebalance-state", row["canonical_state_provenance"]["rebalance_canonical_state"])
            self.assertTrue(row["canonical_state_provenance"]["conflict"])
            self.assertIn("canonical state conflict between candidate and rebalance forensics", row["notes"])
    ...
    def test_replay_memberships_matches_both_conflicting_canonical_states(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            memberships = module.replay_memberships(
                replay_path,
                canonical_states={
                    "incumbent_proxy_disagreement-033": ["candidate-state", "rebalance-state"],
                },
            )

            self.assertEqual(
                {"present": True, "count": 2, "matched_by": ["canonical_state"], "line_numbers": [1, 2]},
                memberships["incumbent_proxy_disagreement-033"],
            )
...
class PriorSourceCoverageForIncumbentProxy033CliTest(unittest.TestCase):
    ...
    def test_main_writes_diagnostic_artifact(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            out_path = tmp_path / "final" / "prior_source_coverage_for_incumbent_proxy_033.json"

            self.write_json(pinned_path, {
                "schema": "azlite_artifact_pinned_value_trust_ablation_v1",
                "probe_settings": {"simulations": 384, "c_puct": 1.25, "seed": 42},
                "rows": {
                    "incumbent_proxy_disagreement-033": {"reference_move": 4, "configs": {"full_default": {"selected_move": 0, "passes_reference": False}}},
                    "capture_available-002": {"reference_move": 2, "configs": {"full_default": {"selected_move": 2, "passes_reference": True}}},
                    "capture_available-003": {"reference_move": 2, "configs": {"full_default": {"selected_move": 2, "passes_reference": True}}},
                    "incumbent_proxy_disagreement-031": {"reference_move": 4, "configs": {"full_default": {"selected_move": 4, "passes_reference": True}}},
                },
            })
            self.write_json(candidate_path, {"schema": "azlite_search_interaction_diagnostic_v1", "rows": {}})
            self.write_json(rebalance_path, {"schema": "azlite_search_interaction_diagnostic_v1", "rows": {}})
            replay_path.write_text('{"row_id":"capture_available-002"}\n{"row_id":"capture_available-003"}\n', encoding="utf-8")

            exit_code = module.main([
                "--artifact-pinned-ablation", str(pinned_path),
                "--candidate-search-interaction", str(candidate_path),
                "--rebalance-search-interaction", str(rebalance_path),
                "--replay-source", str(replay_path),
                "--out", str(out_path),
            ])

            self.assertEqual(0, exit_code)
            written = json.loads(out_path.read_text(encoding="utf-8"))
            self.assertEqual("azlite_prior_source_coverage_for_incumbent_proxy_033_v1", written["schema"])
            self.assertEqual(str(pinned_path), written["source_artifacts"]["artifact_pinned_ablation"])
            self.assertEqual(
                {"present": False, "count": 0, "matched_by": [], "line_numbers": []},
                written["rows"]["incumbent_proxy_disagreement-033"]["replay_source_membership"],
            )
            self.assertEqual(
                {"present": True, "count": 1, "matched_by": ["row_id"], "line_numbers": [1]},
                written["rows"]["capture_available-002"]["replay_source_membership"],
            )
            self.assertEqual(
                {"present": True, "count": 1, "matched_by": ["row_id"], "line_numbers": [2]},
                written["rows"]["capture_available-003"]["replay_source_membership"],
            )
            self.assertEqual("absent_from_source_construction", written["summary"]["source_gap_classification"])
            self.assertEqual("write_targeted_source_coverage_candidate_spec", written["summary"]["decision"])
