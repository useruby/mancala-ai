...
DEFAULT_SUMMARY_OUT = Path(
    "/tmp/azlite_starvation_pressure_reference_adjudication/"
    "starvation_pressure_reference_adjudication_summary.json"
)
DEFAULT_PATCH_OUT = Path(
    "/tmp/azlite_starvation_pressure_reference_adjudication/"
    "starvation_pressure_reference_review_patch_v1.json"
)
DEFAULT_REPORT_OUT = Path(
    "docs/alphazero-lite-starvation-pressure-reference-adjudication-results.md"
)
FAMILY = "starvation_pressure"
SCHEMA = "azlite_starvation_pressure_reference_adjudication_v1"
...
PR61_CLASSIFICATION = {
    "starvation_pressure-025": "corrected_reference_suspicious",
    "starvation_pressure-026": "corrected_reference_suspicious",
    "starvation_pressure-024": "corrected_reference_suspicious",
    "starvation_pressure-022": "corrected_reference_suspicious",
    "starvation_pressure-023": "corrected_reference_suspicious",
    "starvation_pressure-027": "corrected_reference_suspicious",
    "starvation_pressure-012": "inconclusive",
    "starvation_pressure-015": "holdout_context",
    "starvation_pressure-001": "holdout_context",
    "starvation_pressure-013": "preservation_control",
    "starvation_pressure-003": "preservation_control",
    "starvation_pressure-021": "preservation_control",
    "starvation_pressure-014": "preservation_control",
}
...
PR61_CONTEXT = {
    ...
    "corrected_reference_suspicious_count": 6,
    "inconclusive_count": 1,
    "blocked_next_action": "adjudicate starvation_pressure references before training.",
}
...
def projected_buckets(
    *,
    ...
    if flip_count >= 3:
        ...
        recommendation = "review and apply the proposed patch artifact, then rerun starvation_pressure value/backup audit."
    elif (
        confirmed_count >= max(5, (adjudicated_count * 3 + 4) // 5)
        and confirmed_count >= 4
    ):
        decision = "references_clean_enough_for_value_calibration"
        recommendation = "build a small train-only starvation_pressure value-calibration artifact with preservation controls; no arena until local value metrics improve."
    elif unstable_count >= 3:
        decision = "reference_suite_too_noisy_for_starvation_pressure"
        ...
        recommendation = "decide teacher policy for starvation_pressure before training, as we did for incumbent_proxy_disagreement."
    else:
        decision = "still_reference_uncertain"
        recommendation = "do not train on starvation_pressure; rerun non-opening family mining and pick the next family."
    return (
        rows,
        {
            ...
            + len(divergence_rows),
            "starvation_pressure_good_training_target_after_adjudication": decision
...
def build_report(summary: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Family-level classification: `{summary['pr61_context']['family_classification']}`.",
        f"- Mechanism counts: `{json.dumps(summary['pr61_context'], sort_keys=True)}`.",
        f"- Exact PR #61 recommendation: **{summary['pr61_context']['blocked_next_action']}**",
        "",
        "## 3. Row validation",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            f"- `starvation_pressure` remains a good training target after adjudication: `{str(bool(summary['starvation_pressure_good_training_target_after_adjudication'])).lower()}`.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    for row_id in ALL_REPORTED_ROW_IDS:
        ...
        validation_row = {
            "row_id": row_id,
            "role": role,
            "pr61_classification": PR61_CLASSIFICATION[row_id],
    ...
    for row in valid_adjudicated_rows:
        ...
        for budget in PUCT_BUDGETS:
            ...
            puct_budget_summaries[str(budget)] = {
                ...
                "notes": "deterministic artifact PUCT with PR #61 settings",
            }
        ...
        row_result = {
            ...
            "pr61_classification": row["pr61_classification"],
            "active_reference_move": reference_move,
