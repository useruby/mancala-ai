...
class OpeningCaptureFamilyValidationScriptTest(unittest.TestCase):
    def load_module(self):
        ...
        script_path = repo_root / "script/ai/run_tactical_opening_capture_family_validation"
    ...
    def test_dry_run_pins_stages_sources_and_report_command(self):
        ...
        with tempfile.TemporaryDirectory(prefix="azlite-opening-capture-family-validation-") as tmp:
        ...
        self.assertEqual(
            str(repo_root / "ml/alphazero_lite/configs/aggressive_v3_tactical_opening_capture_family_local.json"),
        ...
        self.assertEqual(str(run_paths["selection_artifact"]), pinned_sources["selected-artifact"])
        self.assertEqual(
        ...
        self.assertEqual(str(run_paths["final_dir"] / "bucket_gate.json"), pinned_sources["bucket-gate"])
        self.assertEqual(
            str(run_paths["final_dir"] / "candidate_regression_suite.json"),
            pinned_sources["regression-report"],
        )
        self.assertEqual(str(run_paths["final_dir"] / "arena_seed_1041.json"), pinned_sources["arena-report"])
        self.assertEqual(
        ...
        self.assertTrue(validation_command[1].endswith("script/ai/run_local_tactical_replay_experiment"))
        ...
        self.assertEqual(pinned_sources["forensic-suite"], family_report_command[family_report_command.index("--suite") + 1])
        self.assertEqual(args.current_path, family_report_command[family_report_command.index("--current-artifact") + 1])
        self.assertEqual(
            str(run_paths["selection_artifact"]),
            family_report_command[family_report_command.index("--candidate-artifact") + 1],
        )
        self.assertEqual(
            str(run_paths["family_report_path"]),
    ...
    def test_dry_run_pins_shared_reference_artifact_for_select_and_validation(self):
        ...
        select_command = payload["commands"]["select"]
        validation_command = payload["commands"]["validation_pack"]
        reference_artifact = str(run_paths["base_dir"] / "inputs" / "reference_moves.json")
    ...
    def test_run_executes_report_stage_before_verdict_and_writes_summary(self):
        ...
        with tempfile.TemporaryDirectory(prefix="azlite-opening-capture-family-validation-") as tmp:
            ...
            def fake_run_command(command, cwd, allowed_returncodes=(0,)):
                ...
                if command[1:3] == ["-m", "ml.alphazero_lite.write_opening_capture_family_report"]:
                    report_path = Path(command[command.index("--out") + 1])
                    report_path.parent.mkdir(parents=True, exist_ok=True)
                    report_path.write_text(json.dumps(self.family_report_payload()), encoding="utf-8")
        ...
        self.assertTrue(commands_run[2][0][1:3] == ["-m", "ml.alphazero_lite.write_opening_capture_family_report"])
        self.assertTrue(commands_run[3][0][1:3] == ["-m", "ml.alphazero_lite.write_tactical_validation_verdict"])
        self.assertEqual((0, 1), commands_run[3][1])
        self.assertEqual(self.family_report_payload(), summary["family_report"])
        self.assertEqual(str(run_paths["family_report_path"]), summary["family_report_path"])
    ...
    def test_run_requires_fresh_family_report_artifact(self):
        ...
        with tempfile.TemporaryDirectory(prefix="azlite-opening-capture-family-validation-") as tmp:
            ...
            run_paths["family_report_path"].write_text(json.dumps(self.family_report_payload()), encoding="utf-8")
    ...
    def test_run_validates_family_report_schema(self):
        ...
        with tempfile.TemporaryDirectory(prefix="azlite-opening-capture-family-validation-") as tmp:
            ...
            with mock.patch.object(module, "run_command", side_effect=fake_run_command):
                with self.assertRaisesRegex(SystemExit, "opening_capture_family_report_v1"):
                    module.run(args, repo_root)
    ...
    def test_run_continues_after_failing_validation_pack_to_emit_report_and_verdict(self):
        module = self.load_module()
        repo_root = Path(__file__).resolve().parents[2]
