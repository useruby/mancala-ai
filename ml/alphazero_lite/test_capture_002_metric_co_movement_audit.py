...
class Capture002MetricCoMovementAuditContractTest(unittest.TestCase):
    def test_contract_constants_are_stable(self):
        self.assertEqual(
            "azlite_capture_002_metric_co_movement_audit_v1", module.SCHEMA
        )
        self.assertEqual(
            "azlite_capture_002_nonseparable_decomposition_v1",
            module.SOURCE_DECOMPOSITION_SCHEMA,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            "metric_co_movement", module.EXPECTED_DECOMPOSITION_CLASSIFICATION
        )
        self.assertEqual(
            "stop_002_mechanism_not_isolated", module.EXPECTED_DECOMPOSITION_DECISION
        )
    ...
    def test_parse_args_reads_required_paths(self):
        ...
        self.assertEqual(
            Path("/tmp/decomposition.json"), args.source_decomposition_artifact
        )
        self.assertEqual(
    ...
    def test_parse_args_defaults_optional_checkpoint_canonicalization_path_to_none(
        self,
    ):
        args = module.parse_args(
            [
...
class Capture002MetricCoMovementAuditBuildPayloadTest(unittest.TestCase):
    ...
    def move_entry(
        self, move: int, *, q_value: float | None, selection_score: float | None
    ) -> dict:
        entry = {"move": move}
        ...
        *,
        ...
        return {
            ...
            "classification": {
                ...
                "evidence_summary": "metric co-movement",
            },
            "decision": decision,
            "input_artifacts": {
    ...
    def test_build_payload_rejects_wrong_decomposition_schema(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "decomposition artifact has wrong schema"
        ):
            self.build_payload(
                decomposition=decomposition, default=default, relaxed=relaxed
            )
    ...
    def test_build_payload_rejects_missing_thresholds(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "thresholds must contain material_visit_share_margin"
        ):
            self.build_payload(
                decomposition=decomposition, default=default, relaxed=relaxed
            )
    ...
    def test_build_payload_rejects_negative_thresholds(self):
        ...
        decomposition["thresholds_evaluated"]["relaxed"][
    ...
    def test_build_payload_fails_closed_for_insufficient_trace(self):
        ...
        with self.assertRaisesRegex(
            ValueError, r"relaxed trace artifact insufficiency_reasons must be \[\]"
        ):
            self.build_payload(
                decomposition=decomposition, default=default, relaxed=relaxed
            )
    ...
    def test_build_payload_rejects_mismatched_source_artifact(self):
        decomposition, default, relaxed = self.valid_inputs()
        relaxed["source_artifact"]["selected_artifact"]["selected_target"] = (
            "/tmp/other"
        )
    ...
    def test_build_payload_rejects_mismatched_input_paths(self):
        decomposition, default, relaxed = self.valid_inputs()
        decomposition["input_artifacts"]["source_selection_score_artifact_path"] = (
            "/tmp/other.json"
        )
    ...
    def test_build_payload_rejects_empty_checkpoint_sequences(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "trace_points must be a non-empty list"
        ):
            self.build_payload(
                decomposition=decomposition, default=default, relaxed=relaxed
            )
    ...
    def test_build_payload_still_rejects_duplicate_checkpoint_sequences_without_canonicalization_artifact(
        self,
    ):
        decomposition, default, relaxed = self.valid_inputs()
    ...
    def test_build_payload_rejects_out_of_order_checkpoint_sequences(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "checkpoint sequences must be strictly increasing"
        ):
            self.build_payload(
                decomposition=decomposition, default=default, relaxed=relaxed
            )
    ...
    def test_build_payload_rejects_wrong_canonicalization_schema(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "canonicalization artifact has wrong schema"
        ):
            self.build_payload(
                decomposition=decomposition,
                default=default,
    ...
    def test_build_payload_rejects_wrong_canonicalization_decision(self):
        decomposition, default, relaxed = self.valid_inputs()
        canonicalization = self.canonicalization_artifact(
            decision="stop_002_metric_audit_inconclusive"
        )
    ...
    def test_build_payload_rejects_canonicalization_source_artifact_provenance_mismatch(
        self,
    ):
        decomposition, default, relaxed = self.valid_inputs()
        ...
        canonicalization["source_artifact"]["artifact_path"] = (
            "/tmp/stale-upstream.json"
        )
    ...
    def test_build_payload_rejects_decomposition_source_artifact_provenance_mismatch_with_canonicalization(
        self,
    ):
        decomposition, default, relaxed = self.valid_inputs()
        full_source_artifact = self.source_artifact_with_provenance()
    ...
    def test_build_payload_accepts_duplicate_equivalent_root_snapshots_with_valid_canonicalization_artifact(
        ...
        self.assertEqual(
            [1, 2.0], [row["simulation"] for row in payload["checkpoint_audit"]]
        )
    ...
    def test_build_payload_accepts_regenerated_decomposition_with_full_provenance(self):
        ...
        self.assertEqual(
            "early_selection_score_only", payload["classification"]["classification"]
        )
        self.assertEqual(
            "write_002_selection_score_component_audit_spec", payload["decision"]
        )
    ...
    def test_build_payload_rejects_non_equivalent_skipped_duplicate_even_when_canonicalization_claims_safe(
        self,
    ):
        decomposition, default, relaxed = self.valid_inputs()
        duplicate_default_point = copy.deepcopy(default["trace_points"][0])
    ...
    def test_build_payload_rejects_skipped_duplicate_with_different_reference_move_by_prior(
        self,
    ):
        decomposition, default, relaxed = self.valid_inputs()
        duplicate_default_point = copy.deepcopy(default["trace_points"][0])
    ...
    def test_build_payload_rejects_skipped_duplicate_with_different_simulation_representation(
        self,
    ):
        decomposition, default, relaxed = self.valid_inputs()
        default["trace_points"][0]["simulation"] = 1
    ...
    def test_build_payload_rejects_extra_non_duplicate_simulation_missing_from_canonical_sequence(
        self,
    ):
        decomposition, default, relaxed = self.valid_inputs()
    ...
    def test_build_payload_computes_checkpoint_margins_for_default_and_relaxed_traces(
        self,
    ):
        decomposition, default, relaxed = self.valid_inputs()
    ...
    def test_build_payload_non_canonical_path_keeps_existing_float_simulation_output(
        self,
    ):
        decomposition, default, relaxed = self.valid_inputs()
        default["trace_points"][0]["simulation"] = 1
        ...
        self.assertEqual(
            [1.0, 2.0], [row["simulation"] for row in payload["checkpoint_audit"]]
        )
    ...
    def test_build_payload_preserves_null_margins_when_move_metrics_are_missing(self):
        ...
        self.assertIsNone(
            payload["checkpoint_audit"][1]["default"]["selected_minus_reference_q"]
        )
        self.assertIsNone(
            payload["checkpoint_audit"][1]["default"][
    ...
    def test_build_payload_classifies_weak_aligned_drift(self):
        ...
        self.assertEqual(
            "weak_aligned_drift", payload["classification"]["classification"]
        )
        self.assertEqual(
            "write_002_low_confidence_policy_value_interaction_spec",
            payload["decision"],
        )
    ...
    def test_build_payload_classifies_late_visit_share_only(self):
        ...
        self.assertEqual(
            "late_visit_share_only", payload["classification"]["classification"]
        )
        self.assertEqual("write_002_visit_accumulation_audit_spec", payload["decision"])

    def test_build_payload_classifies_mixed_low_confidence_signal_for_simultaneous_material_support(
        self,
    ):
        decomposition, default, relaxed = self.valid_inputs()
        ...
        self.assertEqual(
            "mixed_low_confidence_signal", payload["classification"]["classification"]
        )
        self.assertEqual(
            "write_002_low_confidence_trace_comparison_spec", payload["decision"]
        )

    def test_build_payload_keeps_simultaneous_first_material_support_out_of_weak_aligned_drift(
        self,
    ):
        decomposition, default, relaxed = self.valid_inputs()
        relaxed["trace_points"] = [
    ...
    def test_build_payload_classifies_metric_audit_inconclusive_when_no_signal_is_usable(
        self,
    ):
        decomposition, default, relaxed = self.valid_inputs()
        relaxed["trace_points"] = [
        ...
        self.assertEqual(
            "metric_audit_inconclusive", payload["classification"]["classification"]
        )
        self.assertEqual("stop_002_metric_audit_inconclusive", payload["decision"])

    def test_build_payload_omits_canonicalization_provenance_when_canonical_input_is_absent(
        self,
    ):
        payload = self.build_payload()
    ...
    def test_main_writes_sorted_payload_and_prints_compact_json(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            decomposition["input_artifacts"][
                "source_threshold_review_artifact_path"
            ] = str(relaxed_path)
    ...
    def test_main_includes_canonicalization_provenance_when_canonical_input_is_used(
        self,
    ):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            ...
            canonicalization_path.write_text(
                json.dumps(canonicalization), encoding="utf-8"
            )
