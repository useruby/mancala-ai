...
def _validate_trace_points(trace_points: list[dict]) -> None:
    for index, trace_point in enumerate(trace_points):
        if not isinstance(trace_point, dict):
            raise ValueError(
                f"trace capture artifact trace_points[{index}] must be an object"
            )
        ...
        if "selected_move" not in trace_point:
            raise ValueError(
                f"trace capture artifact trace_points[{index}] selected_move is required"
            )
        _validate_integer(
...
def _first_divergent_selected_move_index(
    trace_points: list[dict], *, final_selected_move: int
) -> int | None:
    for index, trace_point in enumerate(trace_points):
...
def _validate_matching_case(
    trace_capture_artifact: dict, selection_score_artifact: dict
) -> None:
    ...
    selection_identity = _validate_selection_source_artifact_identity(
    ...
    if trace_row_id != selection_row_id:
        raise ValueError(
            "trace capture artifact and selection score artifact must refer to the same row_id"
        )
    ...
    for field in ("reference_move", "full_search_selected_move"):
        ...
        if trace_value != selection_value:
            raise ValueError(
                f"trace capture artifact and selection score artifact must agree on {field}"
            )


def _validate_selection_source_artifact_identity(
    selection_source_artifact: dict,
) -> dict:
    for field in ("row_id", "reference_move", "full_search_selected_move"):
        if field not in selection_source_artifact:
            raise ValueError(
                f"selection score artifact source_artifact.{field} is required"
            )
...
def _normalized_trace_capture_input(artifact: dict) -> dict:
    ...
    if normalized.get("trace_origin") != expected_trace_origin:
        raise ValueError(
            f"trace capture artifact trace_origin must be {expected_trace_origin}"
        )
    return normalized
...
def _validate_dense_cadence_capture_artifact(artifact: dict) -> None:
    ...
    if (
        not isinstance(provenance_guard, dict)
        or provenance_guard.get("passed") is not True
    ):
        raise ValueError(
            "trace capture artifact dense cadence capture provenance_guard must be an object with passed == True"
        )

    trace_points_summary = artifact.get("trace_points_summary")
    if not isinstance(trace_points_summary, dict):
        raise ValueError(
            "trace capture artifact dense cadence capture trace_points_summary must be an object"
        )

    unique_simulation_checkpoints = trace_points_summary.get(
        "unique_simulation_checkpoints"
    )
    ...
    recomputed_unique_simulation_checkpoints = _unique_simulation_checkpoints(
    ...
    unique_simulation_checkpoint_count = trace_points_summary.get(
        "unique_simulation_checkpoint_count"
    )
    if unique_simulation_checkpoint_count != len(
        recomputed_unique_simulation_checkpoints
    ):
        raise ValueError(
    ...
    duplicate_root_snapshot_count = trace_points_summary.get(
        "duplicate_root_snapshot_count"
    )
    if duplicate_root_snapshot_count != len(artifact["trace_points"]) - len(
        recomputed_unique_simulation_checkpoints
    ):
        raise ValueError(
    ...
    recomputed_first_divergent_selected_move_index = (
        _first_divergent_selected_move_index(
            artifact["trace_points"],
    ...
    first_divergent_selected_move_index = trace_points_summary.get(
    ...
    if isinstance(first_divergent_selected_move_index, bool) or not isinstance(
        first_divergent_selected_move_index, int
    ):
        raise ValueError(
    ...
    divergent_simulation = _validate_simulation(
        artifact["trace_points"][recomputed_first_divergent_selected_move_index].get(
            "simulation"
        ),
        context=(
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Review 002 trace cadence for unresolved downstream traces"
    )
...
def load_trace_capture_artifact(path: Path) -> dict:
    ...
    if (
        not isinstance(artifact.get("trace_points"), list)
        or not artifact["trace_points"]
    ):
    ...
    _validate_matching_case(
        {
            ...
            "reference_move": (artifact.get("source_artifact") or {}).get(
                "reference_move"
            ),
            "full_search_selected_move": (artifact.get("source_artifact") or {}).get(
...
def _selected_move_changes_without_captured_crossing(trace_points: list[dict]) -> bool:
    ...
    return (
        len(set(selected_moves)) > 1
        and len(_unique_simulation_checkpoints(trace_points)) < 4
    )
    ...
    *,
    ...
    return {
        ...
        "trace_capture_excerpt": {
            ...
            "full_search_selected_move": trace_capture_artifact.get(
                "full_search_selected_move"
            ),
        },
...
def main(argv: list[str] | None = None) -> int:
    ...
    trace_capture_artifact = load_trace_capture_artifact(
        args.source_trace_capture_artifact
    )
    selection_score_artifact = load_selection_score_artifact(
