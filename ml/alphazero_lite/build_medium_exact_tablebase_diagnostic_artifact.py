#!/usr/bin/env python3
"""Build medium exact-tablebase diagnostic artifacts from adversarial mining + existing reuse.

Reuses PR #76/77 clean split rows and generates additional adversarial endgame
candidates to reach target row counts:
  - production candidates: 80 to 120
  - preservation controls: at least 60
  - untouched holdouts: at least 500
  - value-only candidates: at least 100

All policy targets use softened mass = 0.75.
...
PR76_OUTPUT_DIR = Path("/tmp/azlite_larger_harder_endgame_tablebase_diagnostic")
PR77_OUTPUT_DIR = Path("/tmp/azlite_exact_tablebase_stabilized_diagnostic")
OUTPUT_DIR = Path("/tmp/azlite_medium_exact_tablebase_diagnostic")

TARGET_PRODUCTION = 80
TARGET_CONTROLS = 60
TARGET_HOLDOUTS = 500
TARGET_VALUE_ONLY = 100
...
def is_exhausted_bucket(bucket: str) -> bool:
...
def generate_adversarial_candidates(
    *, samples: int, seeds: list[int], min_seeds: int = 2, max_seeds: int = 14
) -> list[dict[str, Any]]:
    ...
    while attrs < samples:
        ...
        for move in legal:
            ...
            if cv > best_val:
                second_val = best_val
                best_val = cv
            elif cv > second_val and cv < best_val + EPS:
                second_val = cv
        ...
        if not optimal_moves or len(optimal_moves) != 1:
            continue
...
def run_puct_baseline(
    evaluator: ArtifactEvaluator, candidates: list[dict[str, Any]]
) -> None:
    budgets = (64, 384, 1200)
...
def classify_candidates(candidates: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for cand in candidates:
        ...
        if puct_class == "persistent_exact_failure":
            ...
            elif control_count < max(40, TARGET_CONTROLS + 20):
...
def build_refined_clean_split(
    existing_rows_from_clean_split: list[dict[str, Any]],
    existing_diag_by_cid: dict[str, dict],
    new_candidates: list[dict[str, Any]],
    new_split: dict[str, Any],
    evaluator: ArtifactEvaluator,
    existing_production_ids: set[str],
    existing_control_ids: set[str],
    existing_value_only_ids: set[str],
    existing_holdout_ids: set[str],
) -> tuple[
    list[dict[str, Any]], dict[str, int], set[str], set[str], set[str], set[str]
]:
    ...
    value_only_ids: set[str] = set()
    holdout_ids: set[str] = set()

    for er in existing_rows_from_clean_split:
        bucket = er.get("bucket", "")
        if bucket == "search_diagnostic_only":
        ...
        if bucket == "production_candidate_later":
            production_ids.add(cid)
        elif bucket == "preservation_control":
            control_ids.add(cid)
        elif bucket == "value_only_candidate":
            value_only_ids.add(cid)
        elif bucket == "holdout_candidate":
            holdout_ids.add(cid)
    ...
    for cand in new_candidates:
        ...
        sr = next(
            (s for s in new_split["split_rows"] if s["candidate_hash"] == c_hash), None
        )
        if sr is None:
            continue
        ...
        cid = f"harder_{cand.get('source_game_seed', '?')}_{cand.get('ply', '?')}"
        output_rows.append(
            {
                "bucket": bucket,
                "candidate_id": cid,
    ...
    return output_rows, counts, production_ids, control_ids, value_only_ids, holdout_ids


def validate_artifacts(
    policy_artifact: list[dict],
    controls_artifact: list[dict],
    value_only_artifact: list[dict],
) -> list[str]:
    errors: list[str] = []
        ...
        if optimal_move is not None and optimal_move < len(policy):
            ...
            for m, mass in enumerate(policy):
                if m != optimal_move and mass > optimal_mass:
                    errors.append(
                        f"policy_value[{idx}]: move {m} has {mass:.4f} > optimal {optimal_mass:.4f}"
                    )
                    break
    ...
    for label, artifact in [
        ("policy_value", policy_artifact),
        ("value_only", value_only_artifact),
        ("controls", controls_artifact),
    ]:
        for idx, row in enumerate(artifact):
...
def build_artifacts(
    ...
    for sr in split_rows:
        ...
        if bucket == "production_candidate_later":
            policy = build_soft_policy_target(
                optimal_move, legal_moves, OPTIMAL_POLICY_MASS
            )
            row_data = {
                "candidate_id": cid,
                "canonical_state_hash": c_hash,
    ...
    summary = {
        "schema": "azlite_medium_exact_tablebase_diagnostic_artifact_v1",
        "family": MEDIUM_FAMILY,
        "description": "Medium exact-tablebase diagnostic artifact built from adversarial mining + PR #76/77 reuse.",
        "guardrails": {
    ...
    write_jsonl(
        OUTPUT_DIR / "exact_tablebase_policy_value_soft075.jsonl", policy_value_rows
    )
    write_jsonl(
        OUTPUT_DIR / "exact_tablebase_value_only_artifact.jsonl", value_only_rows
    )
    write_jsonl(
        OUTPUT_DIR / "exact_tablebase_expanded_controls_artifact.jsonl", controls_rows
    )

    errors = validate_artifacts(policy_value_rows, controls_rows, value_only_rows)
    if errors:
...
def main() -> int:
    ...
    print("=" * 70)
    print("MEDIUM EXACT TABLEBASE DIAGNOSTIC ARTIFACT BUILD")
    print("=" * 70)

    print("\nLoading existing clean split and PR #77 artifacts...")
    clean_split_path = PR76_OUTPUT_DIR / "harder_endgame_tablebase_clean_split.json"
    existing_clean_split = json.loads(clean_split_path.read_text(encoding="utf-8"))
    existing_split_rows = existing_clean_split.get("split_rows", [])
    existing_counts = existing_clean_split.get("counts", {})
    print(f"  Existing rows from clean split: {len(existing_split_rows)}")
    print(f"  Existing counts: {existing_counts}")

    existing_diag_path = Path(
        "/tmp/azlite_harder_endgame_tablebase_local_diagnostics/harder_endgame_tablebase_local_row_diagnostics.jsonl"
    )
    if existing_diag_path.exists():
        existing_diag_rows = load_jsonl(existing_diag_path)
        existing_diag_by_cid = {
            r.get("candidate_id", ""): r for r in existing_diag_rows
        }
        print(f"  Existing diagnostic rows: {len(existing_diag_rows)}")
    else:
        existing_diag_by_cid = {}
        print("  WARNING: No existing diagnostic rows found")

    existing_hashes: set[str] = set()
    for dr in existing_split_rows:
        h = dr.get("_canonical_state_hash", "")
        if h:
    ...
    print("\nLoading artifact evaluator (current)...")
    evaluator = ArtifactEvaluator(CURRENT_ARTIFACT)

    existing_production_ids: set[str] = set()
    existing_control_ids: set[str] = set()
    existing_value_only_ids: set[str] = set()
    existing_holdout_ids: set[str] = set()
    for er in existing_split_rows:
        cid = er.get("candidate_id", "")
        bucket = er.get("bucket", "")
        if bucket == "production_candidate_later":
            existing_production_ids.add(cid)
        elif bucket == "preservation_control":
            existing_control_ids.add(cid)
        elif bucket == "value_only_candidate":
            existing_value_only_ids.add(cid)
        elif bucket == "holdout_candidate":
            existing_holdout_ids.add(cid)

    print(
        f"  Existing counts: prod={len(existing_production_ids)}, ctrl={len(existing_control_ids)}, "
        f"val={len(existing_value_only_ids)}, hold={len(existing_holdout_ids)}"
    )

    print(f"\n=== Generating {GENERATE_SAMPLES} adversarial candidates ===")
    seeds = [SEED + i for i in range(GENERATE_SAMPLES)]
    new_candidates_raw = generate_adversarial_candidates(
        samples=GENERATE_SAMPLES, seeds=seeds
    )
    ...
    holdout_count = refined_counts.get("holdout_candidate", 0)
    val_count = refined_counts.get("value_only_candidate", 0)

    print("\n=== Artifact counts vs targets ===")
    print(f"  Production: {prod_count} (target {TARGET_PRODUCTION}-120)")
    print(f"  Controls: {ctrl_count} (target {TARGET_CONTROLS}+)")
    print(f"  Holdouts: {holdout_count} (target {TARGET_HOLDOUTS}+)")
    print(f"  Value-only: {val_count} (target {TARGET_VALUE_ONLY}+)")

    if prod_count < 60:
        print(f"\n  WARNING: production candidates ({prod_count}) < 60 minimum")
        if prod_count < 40:
            print(
                "  CRITICAL: Fewer than 40 production candidates. Classified as medium_exact_tablebase_not_enough_signal."
            )
            classification = "medium_exact_tablebase_not_enough_signal"
            print(f"\n  Classification: {classification}")
            return 1
    if ctrl_count < 40:
        print(f"\n  WARNING: controls ({ctrl_count}) < 40 minimum")
        if ctrl_count < 20:
            print("  CRITICAL: Fewer than 20 controls. Cannot proceed.")
            return 1

    print("\n=== Building artifacts ===")
    summary = build_artifacts(
        refined_rows, production_ids, control_ids, value_only_ids, holdout_ids
    )
    ...
    print(
        f"  Expanded controls: {art.get('expanded_controls', {}).get('row_count', 0)} rows"
    )
    print(f"  Holdouts: {len(summary.get('holdout_candidate_ids', []))}")

    refined_clean_split = {
        "schema": "azlite_medium_exact_tablebase_clean_split_v1",
        "family": MEDIUM_FAMILY,
        "counts": refined_counts,
        "split_rows": refined_rows,
        "production_ids": list(production_ids),
        "control_ids": list(control_ids),
        "value_only_ids": list(value_only_ids),
    ...
    write_json(
        OUTPUT_DIR / "medium_exact_tablebase_clean_split.json", refined_clean_split
    )

    print(f"\nOutput directory: {OUTPUT_DIR}")
    print("  artifact_summary.json")
    print("  exact_tablebase_policy_value_soft075.jsonl")
    print("  exact_tablebase_value_only_artifact.jsonl")
    print("  exact_tablebase_expanded_controls_artifact.jsonl")
    print("  medium_exact_tablebase_clean_split.json")

    if prod_count < 60 or ctrl_count < 40:
        print("\nTargets partially met. Artifacts usable but suboptimal.")
    else:
        print("\nTargets met. Artifacts ready for medium diagnostic traces.")
