...
DEFAULT_SELECTED_ROWS_PATH = Path(
    "/tmp/azlite_corrected_non_opening_failure_mining_v4/"
    "selected_non_opening_family_rows_v4.jsonl"
)
DEFAULT_SUMMARY_OUT = Path(
    "/tmp/azlite_capture_available_reference_adjudication/"
    "capture_available_reference_adjudication_summary.json"
)
DEFAULT_PATCH_OUT = Path(
    "/tmp/azlite_capture_available_reference_adjudication/"
    "capture_available_reference_review_patch_v1.json"
)
DEFAULT_REPORT_OUT = Path(
    "docs/alphazero-lite-capture-available-reference-adjudication-results.md"
)
FAMILY = "capture_available"
SCHEMA = "azlite_capture_available_reference_adjudication_v1"
...
VALUE_HEAD_CONTEXT_ROW_IDS = ("capture_available-019",)
PUCT_CHILD_MISMATCH_CONTEXT_ROW_IDS = ("capture_available-017",)
HOLDOUT_ROW_IDS = (
    "capture_available-015",
    "capture_available-027",
)
PRESERVATION_CONTROL_ROW_IDS = (
    "capture_available-021",
    "capture_available-010",
    "capture_available-020",
    "capture_available-011",
)
CORRECTED_GUARD_ROW_IDS = (
...
ALL_REPORTED_ROW_IDS = (
    *ADJUDICATED_ROW_IDS,
    *VALUE_HEAD_CONTEXT_ROW_IDS,
    *PUCT_CHILD_MISMATCH_CONTEXT_ROW_IDS,
    *HOLDOUT_ROW_IDS,
    *PRESERVATION_CONTROL_ROW_IDS,
)
PR58_CLASSIFICATION = {
    "capture_available-024": "corrected_reference_suspicious",
    "capture_available-013": "corrected_reference_suspicious",
    "capture_available-023": "corrected_reference_suspicious",
    "capture_available-022": "corrected_reference_suspicious",
    "capture_available-016": "corrected_reference_suspicious",
    "capture_available-025": "corrected_reference_suspicious",
    "capture_available-009": "corrected_reference_suspicious",
    "capture_available-001": "corrected_reference_suspicious",
    "capture_available-018": "inconclusive",
    "capture_available-012": "inconclusive",
    "capture_available-019": "value_head_miscalibration",
    "capture_available-017": "puct_child_search_mismatch",
    "capture_available-015": "holdout_context",
    "capture_available-027": "holdout_context",
    "capture_available-021": "preservation_control",
    "capture_available-010": "preservation_control",
    "capture_available-020": "preservation_control",
    "capture_available-011": "preservation_control",
}
FALLBACK_ROLE_BY_ROW = {
    **{row_id: "target_candidate" for row_id in ADJUDICATED_ROW_IDS},
    **{row_id: "target_candidate" for row_id in VALUE_HEAD_CONTEXT_ROW_IDS},
    **{row_id: "target_candidate" for row_id in PUCT_CHILD_MISMATCH_CONTEXT_ROW_IDS},
...
PR58_CONTEXT = {
    ...
    "value_head_miscalibration_count": 1,
    "puct_child_search_mismatch_count": 1,
    "root_selection_pressure_count": 0,
    "corrected_reference_suspicious_count": 8,
    "inconclusive_count": 2,
    "blocked_next_action": "adjudicate capture_available references before training.",
}
...
def rerun_family_mining_if_needed(root: Path, args: argparse.Namespace) -> None:
    ...
    command = [
        python_bin(root),
        "-m",
        "ml.alphazero_lite.run_corrected_non_opening_failure_family_mining_v4",
    ]
...
def deterministic_puct_run(
    ...
    visit_share_by_move = {
        int(move): round_float(float(visits[move]) / total_visits)
        if total_visits > 0 and move < len(visits)
        else None
        for move in legal_moves
    }
    return {
...
def child_support_label(
    child_comparison_rows: list[dict[str, Any]], *, comparison_move: int | None
) -> str:
...
def projected_buckets(
    *,
    ...
    validation_by_id = {row["row_id"]: row for row in validation_table}
    ...
    value_miscalibration_context = sorted(
        row_id
        for row_id in VALUE_HEAD_CONTEXT_ROW_IDS
        if validation_by_id.get(row_id, {}).get("status") == "ok"
    )
    puct_child_mismatch_context = sorted(
        row_id
        for row_id in PUCT_CHILD_MISMATCH_CONTEXT_ROW_IDS
        if validation_by_id.get(row_id, {}).get("status") == "ok"
    )
    rows = [
        {
        ...
        {
            "bucket": "confirmed_value_head_miscalibration_context",
            "row_count": len(value_miscalibration_context),
            "rows": value_miscalibration_context,
            "train_target_eligible_count": 0,
            "risks": "usable only after references settle and later value calibration is explicitly chosen",
            "next_action": "reserve for later value-calibration only after reference adjudication settles",
        },
        {
            "bucket": "confirmed_puct_child_mismatch_context",
            "row_count": len(puct_child_mismatch_context),
            "rows": puct_child_mismatch_context,
            "train_target_eligible_count": 0,
            "risks": "child-search teacher mismatch remains unresolved for deterministic PUCT",
            "next_action": "keep diagnostic-only until child-PUCT-specific follow-up is done",
        },
        {
    ...
    if flip_count >= 4:
        ...
        recommendation = "review and apply the proposed patch artifact, then rerun capture_available value/backup audit."
    elif (
        confirmed_count >= max(5, (adjudicated_count * 3 + 4) // 5)
        and confirmed_count >= 4
    ):
        decision = "references_clean_enough_for_value_calibration"
        recommendation = "build a small train-only capture_available value-calibration artifact with preservation controls; no arena until local value metrics improve."
    elif unstable_count >= 4:
        decision = "reference_suite_too_noisy_for_capture_available"
        ...
        recommendation = "decide teacher policy for capture_available before training, as we did for incumbent_proxy_disagreement."
    else:
        decision = "still_reference_uncertain"
        recommendation = "do not train on capture_available; rerun non-opening family mining and pick the next family."
    mismatch_row = validation_by_id.get("capture_available-017", {})
    value_row = validation_by_id.get("capture_available-019", {})
    return (
        rows,
        {
            ...
            + len(divergence_rows),
            "capture_available_good_training_target_after_adjudication": decision
            == "references_clean_enough_for_value_calibration",
            "capture_available_019_later_value_calibration_safe": (
                decision == "references_clean_enough_for_value_calibration"
                and value_row.get("status") == "ok"
            ),
            "capture_available_017_requires_child_puct_follow_up": bool(
                mismatch_row.get("status") == "ok"
                and mismatch_row.get("current_selected_1200")
                != mismatch_row.get("active_reference_move")
                and mismatch_row.get("current_selected_2400")
                != mismatch_row.get("active_reference_move")
            ),
        },
    )
...
def build_report(summary: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Corrected guard rows kept out of training targets: `{list(CORRECTED_GUARD_ROW_IDS)}`.",
        "",
        "## 2. Why PR #58 blocked value training",
        "",
        "- PR #58 did not confirm that `capture_available` is cleanly trainable.",
        f"- Family-level classification: `{summary['pr58_context']['family_classification']}`.",
        f"- Mechanism counts: `{json.dumps(summary['pr58_context'], sort_keys=True)}`.",
        f"- Exact PR #58 recommendation: **{summary['pr58_context']['blocked_next_action']}**",
        "",
        "## 3. Row validation",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            f"- `capture_available` remains a good training target after adjudication: `{str(bool(summary['capture_available_good_training_target_after_adjudication'])).lower()}`.",
            f"- `capture_available-019` is safe enough to seed a later value-calibration bucket: `{str(bool(summary['capture_available_019_later_value_calibration_safe'])).lower()}`.",
            f"- `capture_available-017` requires child-PUCT-specific follow-up: `{str(bool(summary['capture_available_017_requires_child_puct_follow_up'])).lower()}`.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    for row_id in ALL_REPORTED_ROW_IDS:
        ...
        if not notes:
            ...
            elif row_id in VALUE_HEAD_CONTEXT_ROW_IDS:
                notes.append("reported as value-head miscalibration context only")
            elif row_id in PUCT_CHILD_MISMATCH_CONTEXT_ROW_IDS:
                notes.append("reported as child-PUCT mismatch context only")
            else:
                notes.append("validated for adjudication")
        validation_row = {
            "row_id": row_id,
            "role": role,
            "pr58_classification": PR58_CLASSIFICATION[row_id],
    ...
    for row in valid_adjudicated_rows:
        ...
        for budget in PUCT_BUDGETS:
            ...
            puct_budget_summaries[str(budget)] = {
                ...
                "notes": "deterministic artifact PUCT with PR #58 settings",
            }
        ...
        row_result = {
            ...
            "pr58_classification": row["pr58_classification"],
            "active_reference_move": reference_move,
