...
class BuildTacticalBalancedReplayTest(unittest.TestCase):
    def import_builder_module(self):
        fake_label_tactical_states = types.ModuleType(
            "ml.alphazero_lite.label_tactical_states"
        )
    ...
    def targeted_033_canonical_state(self):
        return json.dumps(
            self.targeted_033_state(), separators=(",", ":"), sort_keys=True
        )
    ...
    def capture_guard_row(
        self, row_id, raw_state, *, policy, priority, teacher_selected_move
    ):
        return {
            **self.capture_row(
    ...
    def capture_row(
        self, canonical_state, *, raw_state, policy, priority, teacher_selected_move
    ):
        return {
            ...
            "legal_moves": [
                move for move, seeds in enumerate(raw_state["player_pits"]) if seeds > 0
            ],
    ...
    def test_build_balanced_replay_dataset_rejects_missing_balanced_replay_source(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(
                FileNotFoundError, "balanced replay source not found"
            ):
    ...
    def test_build_balanced_replay_dataset_rejects_source_without_required_bucket_counts(
        self,
    ):
        from ml.alphazero_lite import build_tactical_balanced_replay as module
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            regression_path.write_text(
                json.dumps(self.regression_fixture()), encoding="utf-8"
            )
            source_path.write_text(
                "\n".join(
                    json.dumps(
                        self.nearby_row("high_imbalance", index, priority=10 - index)
                    )
    ...
    def test_default_balanced_source_supports_minimum_two_non_protection_capture_shapes(
        self,
    ):
        from ml.alphazero_lite import build_tactical_balanced_replay as module
        ...
        def expected_move_preserving_regression_row(
            *, regression_position, teacher_labeler
        ):
        ...
        with patch.object(
            ...
            side_effect=expected_move_preserving_regression_row,
        ):
            protection_rows = module._select_capture_protection_rows(
                regression_position, source_rows
            )
            preservation_rows = module._select_capture_preservation_rows(
                source_rows, protection_rows
            )

        self.assertGreaterEqual(len(preservation_rows), 2)

    def test_build_balanced_replay_dataset_writes_role_labeled_capped_11_composition(
        self,
    ):
        from ml.alphazero_lite import build_tactical_balanced_replay as module
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            preservation_rows = [
                ...
                self.capture_row(
                    json.dumps(
                        self.diversity_state_gain_2_3_pit_3(),
                        separators=(",", ":"),
                        sort_keys=True,
                    ),
            ...
            with patch.object(
                module, "teacher_label_regression_row"
            ) as teacher_labeler:
            ...
            self.assertIn(
                "supports only 2 distinct capture-preservation shapes",
                stderr.getvalue(),
            )
    ...
    def test_build_balanced_replay_dataset_supports_expanded_12_guard_reinforced_variant(
        self,
    ):
        module = self.import_builder_module()
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            guard_rows = [
                row for row in rows if row.get("id") == "capture_available-002"
            ]
            ...
            self.assertEqual(
                self.canonical_state(self.capture_available_002_state()),
                guard_rows[0]["canonical_state"],
            )
            self.assertEqual(
                ["capture_available-002"], summary["guard_reinforcement_ids"]
            )
            self.assertTrue(summary["pass_flags"]["targeted_row_selected"])
            self.assertTrue(summary["pass_flags"]["guard_canonical_states_present"])
            self.assertTrue(summary["pass_flags"]["output_row_count_matches_variant"])
    ...
    def test_targeted_source_summary_fails_structural_validation_for_malformed_row(
        self,
    ):
        module = self.import_builder_module()
        ...
        rows = [
            ...
            {**malformed_targeted_row, "replay_role": "capture_preservation"},
            *(
                {**malformed_targeted_row, "replay_role": "nearby_preservation"}
                for _ in range(6)
            ),
            malformed_targeted_row,
        ]
    ...
    def test_build_balanced_replay_dataset_requires_at_least_two_distinct_capture_preservation_shapes(
        self,
    ):
        from ml.alphazero_lite import build_tactical_balanced_replay as module
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(
                ...
                with self.assertRaisesRegex(
                    ValueError, "capture_preservation distinct shapes below minimum: 1"
                ):
                    module.build_balanced_replay_dataset(
                        regression_positions_path=regression_path,
                        tactical_replay_path=source_path,
