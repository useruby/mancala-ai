...
DEFAULT_SELECTED_ROWS_PATH = Path(
    "/tmp/azlite_corrected_non_opening_failure_mining_v3/"
    "selected_non_opening_family_rows_v3.jsonl"
)
DEFAULT_SUMMARY_OUT = Path(
    "/tmp/azlite_high_imbalance_reference_adjudication/"
    "high_imbalance_reference_adjudication_summary.json"
)
DEFAULT_PATCH_OUT = Path(
    "/tmp/azlite_high_imbalance_reference_adjudication/"
    "high_imbalance_reference_review_patch_v1.json"
)
DEFAULT_REPORT_OUT = Path(
    "docs/alphazero-lite-high-imbalance-reference-adjudication-results.md"
)
FAMILY = "high_imbalance"
SCHEMA = "azlite_high_imbalance_reference_adjudication_v1"
...
PR55_CLASSIFICATION = {
    "high_imbalance-009": "value_head_miscalibration",
    "high_imbalance-018": "corrected_reference_suspicious",
    "high_imbalance-020": "corrected_reference_suspicious",
    "high_imbalance-021": "corrected_reference_suspicious",
    "high_imbalance-025": "corrected_reference_suspicious",
    "high_imbalance-024": "root_selection_pressure",
    "high_imbalance-016": "corrected_reference_suspicious",
    "high_imbalance-027": "corrected_reference_suspicious",
    "high_imbalance-019": "value_head_miscalibration",
    "high_imbalance-023": "value_head_miscalibration",
    "high_imbalance-026": "corrected_reference_suspicious",
    "high_imbalance-014": "value_head_miscalibration",
    "high_imbalance-022": "value_head_miscalibration",
    "high_imbalance-008": "holdout_context",
    "high_imbalance-002": "holdout_context",
    "high_imbalance-007": "preservation_control",
    "high_imbalance-012": "preservation_control",
    "high_imbalance-001": "preservation_control",
    "high_imbalance-017": "preservation_control",
}
...
PR55_CONTEXT = {
    ...
    "value_head_miscalibration_count": 5,
    "puct_child_search_mismatch_count": 0,
    "root_selection_pressure_count": 1,
    "corrected_reference_suspicious_count": 7,
    "inconclusive_count": 0,
    "blocked_next_action": "adjudicate high_imbalance references before training.",
}
...
def rerun_family_mining_if_needed(root: Path, args: argparse.Namespace) -> None:
    ...
    command = [
        python_bin(root),
        "-m",
        "ml.alphazero_lite.run_corrected_non_opening_failure_family_mining_v3",
    ]
...
def projected_buckets(
    *,
    ...
    value_miscalibration_context = sorted(
        row["row_id"]
        for row in row_decision_table
        if PR55_CLASSIFICATION.get(row["row_id"]) == "value_head_miscalibration"
    )
    rows = [
        {
        ...
        },
        {
            ...
            "train_target_eligible_count": 0,
            "risks": (
                "raw neural child values disagreed with the ClassicMCTS teacher "
                "in PR #55; usable only after explicit value calibration"
            ),
            "next_action": "reserve for later value-calibration only after references settle",
        },
        {
    ...
    if flip_count >= 4:
        ...
        recommendation = "review and apply the proposed patch artifact, then rerun high_imbalance value/backup audit."
    elif (
        confirmed_count >= max(5, (adjudicated_count * 3 + 4) // 5)
        and confirmed_count >= 4
    ):
        decision = "references_clean_enough_for_value_calibration"
        recommendation = "build a small train-only high_imbalance value-calibration artifact with preservation controls; no arena until local value metrics improve."
    elif unstable_count >= 4:
        decision = "reference_suite_too_noisy_for_high_imbalance"
        ...
        recommendation = "decide teacher policy for high_imbalance before training, as we did for incumbent_proxy_disagreement."
    else:
        decision = "still_reference_uncertain"
        recommendation = "do not train on high_imbalance; rerun non-opening family mining and pick the next family."
    return (
        rows,
        {
            ...
            + len(divergence_rows),
            "high_imbalance_good_training_target_after_adjudication": decision
            == "references_clean_enough_for_value_calibration",
            "value_head_miscalibration_later_calibration_safe": (
                decision == "references_clean_enough_for_value_calibration"
...
def build_report(summary: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Family-level classification: `{summary['pr55_context']['family_classification']}`.",
        f"- Mechanism counts: `{json.dumps(summary['pr55_context'], sort_keys=True)}`.",
        f"- Exact PR #55 recommendation: **{summary['pr55_context']['blocked_next_action']}**",
        "",
        "## 3. Row validation",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            f"- `high_imbalance` remains a good training target after adjudication: `{str(bool(summary['high_imbalance_good_training_target_after_adjudication'])).lower()}`.",
            f"- Later value-calibration bucket (5 value_head_miscalibration rows) safe to keep: `{str(bool(summary['value_head_miscalibration_later_calibration_safe'])).lower()}`.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    for row_id in ALL_REPORTED_ROW_IDS:
        ...
        validation_row = {
            "row_id": row_id,
            "role": role,
            "pr55_classification": PR55_CLASSIFICATION[row_id],
    ...
    for row in valid_adjudicated_rows:
        ...
        for budget in PUCT_BUDGETS:
            ...
            puct_budget_summaries[str(budget)] = {
                ...
                "notes": "deterministic artifact PUCT with PR #55 settings",
            }
        ...
        row_result = {
            ...
            "pr55_classification": row["pr55_classification"],
            "active_reference_move": reference_move,
