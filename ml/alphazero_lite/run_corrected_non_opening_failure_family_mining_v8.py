...
DEFAULT_OUT_ROOT = Path("/tmp/azlite_corrected_non_opening_failure_mining_v8")
DEFAULT_SUMMARY_PATH = DEFAULT_OUT_ROOT / "non_opening_failure_family_summary_v8.json"
DEFAULT_SELECTED_ROWS_PATH = (
    DEFAULT_OUT_ROOT / "selected_non_opening_family_rows_v8.jsonl"
)
...
EXCLUDED_CLOSED_BRANCH_FAMILIES = {
    "high_value_swing",
    "high_imbalance",
    "capture_available",
    "starvation_pressure",
...
CAPTURE_AVAILABLE_EXCLUSION_REASON = "reference_suite_too_noisy_for_capture_available"
...
STARVATION_PRESSURE_EXCLUSION_REASON = (
    "reference_suite_too_noisy_for_starvation_pressure"
)
SPARSE_ENDGAME_EXCLUSION_REASON = "forced_tied_dominated_no_trainable_signal"
EARLY_EXTRA_TURN_EXCLUSION_REASON = "reference_suite_too_noisy_for_early_extra_turn"
...
def exclusion_reason(row: dict[str, Any], *, family_label: str) -> str | None:
    ...
    if family == "capture_available":
        return "capture_available_branch_excluded"
    if family == "high_imbalance":
        return "high_imbalance_branch_excluded"
    if family == "high_value_swing":
        return "high_value_swing_branch_excluded"
    if family == "starvation_pressure":
        return "starvation_pressure_branch_excluded"
    if family == "sparse_endgame":
        return SPARSE_ENDGAME_EXCLUSION_REASON
    if family == "early_extra_turn":
        return EARLY_EXTRA_TURN_EXCLUSION_REASON
...
def exclusion_summary_rows(excluded_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    labels = [
        ...
        (
            ...
            "exclude capture_available 002/003/007/008 in the active inventory; historical guard id 006 remains non-selectable if reintroduced",
        ),
        (
            "capture_available_branch_excluded",
            "capture_available branch",
            "capture_available was adjudicated as not safe for training targets after PR #59",
            "0 eligible target rows remain under active corrected references; 3 rows would require reviewed fixture patching and 7 rows stay excluded",
        ),
        (
        ...
        (
            "starvation_pressure_branch_excluded",
            "starvation_pressure branch",
            "starvation_pressure reference suite too noisy after PR #62 adjudication",
            "0 clean-enough target rows remain; 0 reference-should-flip rows; 2 reference-unstable rows (starvation_pressure-025, starvation_pressure-027) and 5 still-inconclusive rows stay excluded",
        ),
        (
            "forced_tied_dominated_no_trainable_signal",
            "sparse_endgame branch",
            "sparse_endgame is dominated by forced/tied positions; 83% of rows have arbitrary policy labels",
            "Excluded after exact-tablebase targetability split (PR #67). 16 of 24 rows are forced_all_moves_equivalent. Only 1 row (sparse_endgame-023) has unique optimal + PUCT-suboptimal signal, but family-level training is not justified.",
        ),
        (
            "reference_suite_too_noisy_for_early_extra_turn",
            "early_extra_turn branch",
            "early_extra_turn reference suite is too noisy after reference adjudication",
            "0 confirmed reference targets remain; 2 reference-should-flip rows (early_extra_turn-013, early_extra_turn-018) require patch review; 3 rows are unstable or inconclusive.",
        ),
        (
...
def render_report(summary: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Projected family decision: `{EARLY_EXTRA_TURN_EXCLUSION_REASON}`.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    for family, rows in sorted(family_rows.items()):
        ...
        summary_row["rank_score"] = (
            (summary_row["stable_corrected_reference_count"] * 6.0)
            + (summary_row["persistent_1200_failures"] * 5.5)
            + (summary_row["high_severity_count"] * 3.5)
            + (summary_row["medium_severity_count"] * 2.0)
            + (min(summary_row["pass_rows"], 4) * 1.5)
            + (
                abs(
                    float(
                        summary_row.get("avg_selected_minus_reference_q_margin_1200")
                        or 0.0
                    )
                )
                * 10.0
            ...
            + (
                (1.0 - float(summary_row.get("avg_reference_visit_share_1200") or 0.0))
                * 6.0
            )
            + {
                "value_q": 4.0,
            ...
            - (summary_row["conflicting_target_count"] * 8.0)
            - (summary_row["duplicate_canonical_state_count"] * 1.0)
            - (summary_row["recovered_at_1200"] * 1.5)
        )
        summary_row["rank_score"] = round(float(summary_row["rank_score"]), 4)
    ...
    selected_family_report = {
        ...
        "control_rows": 0,
        "holdout_rows": 0,
        "reason_selected": "no coherent non-opening family remained safe after all exclusions",
        "risks": "all 6 corrected non-opening branches are exhausted under current active references",
    ...
    if selected_family_summary is not None:
        ...
        selected_family_report = {
            ...
            "reason_selected": (
                "highest-ranked remaining corrected non-opening family after excluding early_extra_turn after reference adjudication; "
    ...
    summary = {
        "schema": "azlite_corrected_non_opening_failure_family_mining_v8",
        "inventory_source": {
        ...
        "early_extra_turn_exclusion": {
            "excluded": True,
            "reason": EARLY_EXTRA_TURN_EXCLUSION_REASON,
            "projected_family_decision": EARLY_EXTRA_TURN_EXCLUSION_REASON,
            "clean_enough_row_count": 0,
            "reference_should_flip_row_count": 2,
            "reference_should_flip_rows": [
                "early_extra_turn-013",
                "early_extra_turn-018",
            ],
            "reference_unstable_rows": [
                "early_extra_turn-014",
                "early_extra_turn-016",
            ],
            "still_inconclusive_rows": [
                "early_extra_turn-012",
            ],
            "preservation_control_rows": [
                "early_extra_turn-002",
                "early_extra_turn-007",
                "early_extra_turn-009",
                "early_extra_turn-011",
            ],
            "early_extra_turn_remains_good_training_target": False,
            "source_doc_path": "docs/alphazero-lite-early-extra-turn-reference-adjudication-results.md",
        },
        "inventory_rows": [
            {
                "row_id": row["row_id"],
        ...
        "closed_branch_families": sorted(EXCLUDED_CLOSED_BRANCH_FAMILIES),
    }
