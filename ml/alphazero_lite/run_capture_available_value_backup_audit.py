...
DEFAULT_MINING_SUMMARY_PATH = Path(
    "/tmp/azlite_corrected_non_opening_failure_mining_v4/"
    "non_opening_failure_family_summary_v4.json"
)
DEFAULT_SUMMARY_OUT = Path(
    "/tmp/azlite_capture_available_value_backup_audit/"
    "capture_available_value_backup_audit_summary.json"
)
DEFAULT_REPORT_OUT = Path(
    "docs/alphazero-lite-capture-available-value-backup-audit-results.md"
)
FAMILY = "capture_available"
SCHEMA = "azlite_capture_available_value_backup_audit_v1"
...
def next_player_legal_count(state: dict[str, Any]) -> int:
    return len(KalahGame.from_state(state).possible_moves())


def next_player_max_capture_count(state: dict[str, Any]) -> int:
    game = KalahGame.from_state(state)
    legal_moves = game.possible_moves()
    if not legal_moves:
        return 0
    return max(
        int(move_consequence_for_state(state, move).get("capture_count", 0))
        for move in legal_moves
    )


def side_seed_total_for_player(state: dict[str, Any], player: int) -> int:
    if int(player) == 0:
        return int(sum(int(seed) for seed in state["player_pits"]))
    return int(sum(int(seed) for seed in state["opponent_pits"]))
...
def validate_and_enrich_rows(
    ...
    forbidden_target_ids = sorted(EXCLUDED_GUARD_ROW_IDS & target_candidate_ids)
    if forbidden_target_ids:
        raise ValueError(
            "excluded corrected guard rows must not appear as target candidates: "
            + ", ".join(forbidden_target_ids)
        )

    validation_context = {
        "required_target_rows_present": sorted(
            row_id for row_id in REQUIRED_TARGET_ROW_IDS if row_id in suite_by_id
        ),
        "required_control_rows_present": sorted(
            row_id for row_id in REQUIRED_CONTROL_ROW_IDS if row_id in suite_by_id
        ),
        "excluded_guard_rows_not_target_candidates": sorted(EXCLUDED_GUARD_ROW_IDS),
    ...
    *, row: AuditRow, baseline_1200: dict[str, Any]
    ...
    if selected_move is not None and selected_move != row.corrected_reference_move:
        if bool(selected_entry.get("gives_extra_turn")) and not bool(
            ...
            heuristic_notes.append("selected move gains extra turn")
        if int(selected_entry.get("capture_count", 0)) > int(
            reference_entry.get("capture_count", 0)
        ):
            heuristic_notes.append("selected move captures more immediately")
        if int(selected_entry.get("immediate_store_delta", 0)) > int(
            reference_entry.get("immediate_store_delta", 0)
        ):
            heuristic_notes.append("selected move has larger immediate store gain")
        if int(selected_entry.get("remaining_seed_count", 0)) < int(
            reference_entry.get("remaining_seed_count", 0)
        ):
            heuristic_notes.append("selected move leaves fewer seeds in pits")

        selected_child = child_state_from_move(row.suite_state, int(selected_move))
        reference_child = child_state_from_move(
            row.suite_state, int(row.corrected_reference_move)
        )
        selected_next_player = int(selected_child["current_player"])
        reference_next_player = int(reference_child["current_player"])
        selected_next_legal = next_player_legal_count(selected_child)
        reference_next_legal = next_player_legal_count(reference_child)
        selected_next_capture = next_player_max_capture_count(selected_child)
        reference_next_capture = next_player_max_capture_count(reference_child)

        if selected_next_player != int(row.suite_state["current_player"]):
            if selected_next_legal < reference_next_legal:
                heuristic_notes.append(
                    "selected move creates stronger starvation pressure"
                )
            if selected_next_capture < reference_next_capture:
                heuristic_notes.append(
                    "selected move lowers immediate opponent capture risk"
                )
            if (
                selected_next_legal <= reference_next_legal
                and selected_next_capture <= reference_next_capture
            ):
                heuristic_notes.append("selected move presents a safer-looking handoff")

        if side_seed_total_for_player(
            selected_child, selected_next_player
        ) < side_seed_total_for_player(reference_child, reference_next_player):
            heuristic_notes.append("selected move reduces next-side seed supply")
        ...
        rows.append(
            {
                ...
                "is_selected": selected_move is not None
    ...
    return (
        rows,
        "; ".join(heuristic_notes)
        if heuristic_notes
        else "no obvious immediate heuristic edge",
    )
...
def classify_row(
    *,
    ...
    if not teacher_stable:
        ...
        evidence = "neural, ClassicMCTS child teacher, and child PUCT support corrected reference child but root PUCT still selects another move"
...
def classify_family(
    ...
    if dominant == "value_head_miscalibration":
        return (
            ...
            "build a small train-only child-afterstate value-calibration artifact for capture_available, with controls and no arena until local value metrics improve.",
            dict(counts),
        )
    ...
    if dominant == "root_selection_pressure":
        return (
            ...
            "run cpuct/root-prior calibration diagnostics for capture_available.",
            dict(counts),
        )
    if dominant == "corrected_reference_suspicious":
        return (
            "reference_family_uncertain",
            "adjudicate capture_available references before training.",
            dict(counts),
        )
    return (
        "mixed_family_gap",
        "split capture_available into mechanism-specific buckets and choose the largest stable bucket.",
        dict(counts),
    )
...
def build_report(summary: dict[str, Any]) -> str:
    lines = [
        ...
        "## 3. Row validation",
        "",
        f"- Corrected guard rows kept out of target candidates: `{summary['row_selection']['excluded_guard_rows_not_target_candidates']}`.",
    ...
    lines.extend(["", "## 4. Root PUCT baseline", ""])
    lines.extend(
        markdown_table(
            [
                "row_id",
...
def main(argv: list[str] | None = None) -> int:
    ...
    for row in target_rows:
        ...
        for puct_row in puct_rows:
            puct_prefers_reference = puct_row["puct_prefers_corrected_reference"]
            child_puct_rows.append(
                {
                    **puct_row,
                    "puct_prefers_reference": puct_prefers_reference,
                    "agrees_with_classic_teacher": puct_prefers_reference
                    == teacher_prefers_reference,
                    "agrees_with_neural_child_value": puct_prefers_reference
                    == neural_prefers_reference,
                    "notes": (
                        "artifact PUCT child-afterstate audit; "
                        f"agrees_with_classic_teacher={str(puct_prefers_reference == teacher_prefers_reference).lower()}, "
                        f"agrees_with_neural_child_value={str(puct_prefers_reference == neural_prefers_reference).lower()}"
                    ),
                }
            )
