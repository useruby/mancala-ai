#!/usr/bin/env python3
"""
Child-afterstate value/backup audit for high_imbalance
"""
...
DEFAULT_MINING_SUMMARY_PATH = Path(
    "/tmp/azlite_corrected_non_opening_failure_mining_v3/"
    "non_opening_failure_family_summary_v3.json"
)
DEFAULT_SUMMARY_OUT = Path(
    "/tmp/azlite_high_imbalance_value_backup_audit/"
    "high_imbalance_value_backup_audit_summary.json"
)
DEFAULT_REPORT_OUT = Path(
    "docs/alphazero-lite-high-imbalance-value-backup-audit-results.md"
)
FAMILY = "high_imbalance"
SCHEMA = "azlite_high_imbalance_value_backup_audit_v1"
OPTIONAL_ROOT_BUDGET = 2400
MAX_PROJECTED_ROOT_2400_SECONDS = 600.0

# Representative target rows from PR #54
...
def classify_family(
    ...
    if dominant == "value_head_miscalibration":
        return (
            ...
            "build a small train-only child-afterstate value-calibration artifact for high_imbalance, with controls and no arena until local value metrics improve.",
            dict(counts),
        )
    ...
    if dominant == "root_selection_pressure":
        return (
            ...
            "run cpuct/root-prior calibration diagnostics for high_imbalance.",
            dict(counts),
        )
    if dominant == "corrected_reference_suspicious":
        return (
            "reference_family_uncertain",
            "adjudicate high_imbalance references before training.",
            dict(counts),
        )
    return (
        "mixed_family_gap",
        "split high_imbalance into mechanism-specific buckets and choose the largest stable bucket.",
        dict(counts),
    )
