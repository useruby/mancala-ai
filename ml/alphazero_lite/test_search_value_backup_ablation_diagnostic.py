...
class SearchValueBackupAblationFixtureTest(unittest.TestCase):
    ...
    SEARCH_VALUE_INTERACTION_FIXTURE_PATH = (
        FIXTURE_DIR / "search_value_interaction_diagnostic.json"
    )
    ...
    def test_fixture_manifest_preserves_tracked_copies_and_original_sources(self):
        self.assertTrue(
            ...
            f"missing fixture manifest: {self.MANIFEST_PATH}",
        )
        self.assertTrue(
        ...
        self.assertEqual(
            {
                "search_interaction_diagnostic": {
                    ...
                    "sha256": self._fixture_sha256(
                        self.SEARCH_INTERACTION_FIXTURE_PATH
                    ),
                },
                "search_value_interaction_diagnostic": {
                    ...
                    "sha256": self._fixture_sha256(
                        self.SEARCH_VALUE_INTERACTION_FIXTURE_PATH
                    ),
                },
            },
            manifest["fixtures"],
        ...
        self.assertEqual(
            json.loads(
                self.SEARCH_INTERACTION_FIXTURE_PATH.read_text(encoding="utf-8")
            )["schema"],
            "azlite_search_interaction_diagnostic_v1",
        )
        self.assertEqual(
            json.loads(
                self.SEARCH_VALUE_INTERACTION_FIXTURE_PATH.read_text(encoding="utf-8")
            )["schema"],
...
class SearchValueBackupAblationMatrixTest(unittest.TestCase):
    def test_fixture_candidate_row_ids_match_approved_constants(self):
        ...
        self.assertEqual(
            module.SEARCH_VALUE_PRIMARY_ROW_IDS, payload["primary_row_ids"]
        )
        self.assertEqual(
            module.SEARCH_VALUE_COMPARATOR_ROW_IDS, payload["comparator_row_ids"]
        )
    ...
    def test_build_matrix_rejects_fixture_row_id_drift(self):
        ...
        with self.assertRaisesRegex(ValueError, "primary_row_ids"):
            module.build_search_value_backup_ablation_matrix(
    ...
    def test_build_matrix_accepts_approved_row_subsets_in_preserved_order(self):
        ...
        matrix = module.build_search_value_backup_ablation_matrix(
            source_artifacts=source_artifacts
        )
        ...
        self.assertEqual(
            [],
            next(
                row for row in matrix if row["stable_key"] == "classic_only_comparator"
            )["target_row_ids"],
        )
    ...
    def test_build_source_artifacts_and_matrix_use_stable_metadata(self):
        ...
        self.assertEqual(
            module.LOW_OPENING_VALUE_TRUST_SCHEDULE,
            by_key["full_value_trust_low_opening"]["search_options_overrides"][
                "value_trust_schedule"
            ],
        )
        self.assertEqual(
            "parent_q",
            by_key["full_fpu_parent_q"]["search_options_overrides"]["fpu_mode"],
        )
        self.assertEqual(
            "candidate", by_key["full_fpu_parent_q"]["classification_role"]
        )
        self.assertEqual(
            {
                "is_supported": True,
        ...
        self.assertEqual(
            "visit_count",
            by_key["full_root_visit_count"]["search_options_overrides"][
                "root_policy_mode"
            ],
        )
        self.assertTrue(
            by_key["full_normalize_values"]["search_options_overrides"][
                "normalize_values"
            ]
        )
        self.assertEqual(
            "comparator_only", by_key["classic_only_comparator"]["classification_role"]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            module.search_value_backup_ablation_out_path(
...
class SearchValueBackupAblationPayloadTest(unittest.TestCase):
    def test_build_payload_probes_selected_rebalanced_artifact_per_configuration(self):
            ...
            *,
            ...
            return {
                ...
                "selection_breakdown": {
                    ...
                    "value_trust_multiplier": resolved_options.get(
                        "value_trust_schedule", {}
                    ).get("opening", 1.0),
                ...
                "visit_snapshots": [
                    {"simulation": 128, "visits": [10.0, 15.0, 90.0, 25.0, 30.0, 5.0]}
                ],
            }
        ...
        self.addCleanup(
            setattr,
            module,
            "load_selected_artifact_path",
            original_load_selected_artifact_path,
        )
        self.addCleanup(
            setattr, module, "load_arena_module", original_load_arena_module
        )
        self.addCleanup(
            setattr, module, "probe_artifact_position", original_probe_artifact_position
        )
        ...
        matrix_by_key = {
            row["stable_key"]: row for row in payload["matrix_configurations"]
        }
        self.assertEqual(
            "classic_only", matrix_by_key["classic_only_comparator"]["ablation_mode"]
        )
        self.assertEqual(
            module.LOW_OPENING_VALUE_TRUST_SCHEDULE,
            matrix_by_key["full_value_trust_low_opening"]["search_options_overrides"][
                "value_trust_schedule"
            ],
        )
        ...
        self.assertEqual(
            [
                *module.SEARCH_VALUE_PRIMARY_ROW_IDS,
                *module.SEARCH_VALUE_COMPARATOR_ROW_IDS,
            ],
            list(rows.keys()),
        )
        ...
        self.assertEqual(
            "search_overrides_prior", primary_row["classification_outcome"]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            "model-artifact/current", primary_row["source_artifacts"]["current"]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            0.14,
            primary_row["configurations"]["full_fpu_zero"]["observability"][
                "probe_value"
            ],
        )
        self.assertEqual(
            0.15,
            primary_row["configurations"]["full_fpu_parent_q"]["observability"][
                "probe_value"
            ],
        )
        self.assertEqual(
            "parent_q",
            primary_row["configurations"]["full_fpu_parent_q"]["search_options"][
                "fpu_mode"
            ],
        )
        self.assertEqual(
            3,
            primary_row["configurations"]["full_root_visit_count"]["observability"][
                "visit_top_move"
            ],
        )
        self.assertTrue(
            primary_row["configurations"]["full_normalize_values"]["search_options"][
                "normalize_values"
            ]
        )
        self.assertEqual(
            0.25,
            primary_row["configurations"]["full_value_trust_low_opening"][
                "observability"
            ]["value_trust_multiplier"],
        )
        self.assertEqual(
            [], primary_row["configurations"]["full_default"]["unsupported_comparisons"]
        )
        ...
        self.assertEqual(
            "comparator_only",
            comparator_row["configurations"]["classic_only_comparator"][
                "classification_role"
            ],
        )
        self.assertEqual(
        ...
        primary_calls = [
            call for call in captured_calls if call["row_id"] == "capture_available-002"
        ]
        self.assertEqual(
            [
                "full_default",
        ...
        self.assertTrue(
            all(
                call["artifact_path"] == selected_artifact_path
                for call in captured_calls
            )
        )
        self.assertTrue(
            all(
                call["evaluator"] == {"evaluator_path": selected_artifact_path}
                for call in captured_calls
            )
        )
        self.assertTrue(
            all(call["simulations"] == requested_simulations for call in captured_calls)
        )
        self.assertTrue(
            all(call["c_puct"] == requested_c_puct for call in captured_calls)
        )
        self.assertEqual(requested_seed, captured_calls[0]["seed"])
        self.assertEqual(requested_seed + 100, captured_calls[8]["seed"])
        comparator_calls = [
            call for call in captured_calls if call["row_id"] == "opening_plies_1_8-057"
        ]
        self.assertEqual(
            ["classic_only_comparator"],
            [call["stable_key"] for call in comparator_calls],
        )
    ...
    def test_build_payload_marks_unsupported_configurations_explicitly(self):
        ...
        module.load_row_context = lambda **kwargs: {
            ...
            "teacher_value": 0.5,
            "current_row": {
                "state": {"row_id": kwargs["row_id"]},
                "legal_moves": [0, 1, 2, 3, 4, 5],
            },
            "rebalanced_row": {"system_value": 0.44, "value_error": 0.06},
        }
        module.load_selected_artifact_path = lambda _run_dir: (
            "/tmp/rebalanced-selected-artifact"
        )
        module.load_arena_module = lambda: type(
            ...
            (),
            {
                ...
                "ArtifactEvaluator": staticmethod(
                    lambda path: {"evaluator_path": str(path)}
                ),
            },
        )
        module.probe_artifact_position = lambda **kwargs: (
            probe_calls.append(kwargs) or {"selected_move": 2}
        )
        self.addCleanup(
            setattr,
            module,
            "build_search_value_backup_ablation_matrix",
            original_build_matrix,
        )
        self.addCleanup(setattr, module, "load_row_context", original_load_row_context)
        self.addCleanup(
            setattr,
        ...
        primary_entry = payload["rows"][module.SEARCH_VALUE_PRIMARY_ROW_IDS[0]][
            "configurations"
        ]["unsupported_parent_value"]
        self.assertEqual(
            [
                {
    ...
    def test_probe_artifact_position_unwraps_diagnostic_search_options_and_ablation_mode(
        self,
    ):
        calls = {}
            ...
            class ArtifactEvaluator:
                def __init__(self, _path):
                    raise AssertionError(
                        "evaluator should not be constructed in this test"
                    )
        ...
        self.assertEqual(
            {"fpu_mode": "parent_q", "max_depth": 6}, calls["search_options"]
        )
        self.assertEqual("policy_only", calls["ablation_mode"])


class SearchValueBackupAblationMainTest(unittest.TestCase):
    def test_main_writes_ablation_artifact_from_existing_source_and_sibling_payloads(
        self,
    ):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fake_build_ablation_payload(
                *, source_artifacts, artifact_simulations, c_puct, seed
            ):
                self.assertEqual(
                    module.diagnostic_out_path(rebalanced_run_dir=rebalanced_run),
                ...
                self.assertIs(
                    source_payload,
                    source_artifacts["search_interaction_diagnostic"]["payload"],
                )
                self.assertEqual(
                    module.search_value_interaction_diagnostic_out_path(
            ...
            with (
                ...
                mock.patch(
                    ...
                    side_effect=AssertionError(
                        "main should not rebuild sibling from scratch"
                    ),
                ),
                mock.patch(
                ...
                mock.patch(
                    ...
                    side_effect=AssertionError(
                        "main should not rebuild ablation sources from scratch"
                    ),
                ),
                mock.patch(
            ...
            sibling_path = module.search_value_interaction_diagnostic_out_path(
                rebalanced_run_dir=rebalanced_run
            )
            ablation_path = module.search_value_backup_ablation_out_path(
                rebalanced_run_dir=rebalanced_run
            )
            ...
            self.assertEqual(
                str(sibling_path), reported["search_value_interaction_artifact_path"]
            )
            self.assertEqual(
                str(ablation_path),
                reported["search_value_backup_ablation_artifact_path"],
            )
