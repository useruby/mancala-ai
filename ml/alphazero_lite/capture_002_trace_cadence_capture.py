...
def load_baseline_inputs(
    *,
    ...
    trace_capture_artifact = trace_cadence_review.load_trace_capture_artifact(trace_capture_artifact_path)
    selection_score_artifact = trace_cadence_review.load_selection_score_artifact(selection_score_artifact_path)
    cadence_review_artifact = trace_cadence_review.load_json(trace_cadence_review_artifact_path)

    if cadence_review_artifact.get("schema") != SOURCE_TRACE_CADENCE_REVIEW_SCHEMA:
        raise ValueError(f"trace cadence review artifact has wrong schema: expected {SOURCE_TRACE_CADENCE_REVIEW_SCHEMA}")
    if (cadence_review_artifact.get("classification") or {}).get("classification") != "trace_too_sparse":
        raise ValueError("trace cadence review artifact must classify trace_too_sparse")
    if cadence_review_artifact.get("decision") != "write_002_trace_cadence_capture_spec":
        raise ValueError("trace cadence review artifact must emit write_002_trace_cadence_capture_spec")

    nonseparable_review_artifact = None
    if nonseparable_review_artifact_path is not None:
        nonseparable_review_artifact = nonseparable_review.load_json(nonseparable_review_artifact_path)
        if nonseparable_review_artifact.get("schema") != SOURCE_NONSEPARABLE_REVIEW_SCHEMA:
            raise ValueError(
                f"nonseparable review artifact has wrong schema: expected {SOURCE_NONSEPARABLE_REVIEW_SCHEMA}"
            )
        if (nonseparable_review_artifact.get("classification") or {}).get("classification") != "prerequisite_preempted":
            raise ValueError("nonseparable review artifact must classify prerequisite_preempted")
        if nonseparable_review_artifact.get("decision") != "write_002_trace_cadence_capture_spec":
            raise ValueError("nonseparable review artifact must emit write_002_trace_cadence_capture_spec")

    selection_source_artifact = copy.deepcopy(selection_score_artifact.get("source_artifact") or {})
    trace_row_context = copy.deepcopy(trace_capture_artifact.get("row_context") or {})
    baseline_row_context = {
        "row_id": trace_row_context.get("row_id") or trace_capture_artifact.get("row_id"),
    ...
    for move_field in ("reference_move", "full_search_selected_move"):
        ...
        if selection_move is not None and selection_move != baseline_row_context.get(move_field):
            raise ValueError("selection score artifact source_artifact move pair must match trace capture baseline")
    ...
    return {
        ...
        "source_selected_artifact": copy.deepcopy(selection_source_artifact.get("selected_artifact")),
        "search_settings": copy.deepcopy((trace_capture_artifact.get("upstream_inputs") or {}).get("search_settings")),
        "baseline_trigger": {
            ...
            + ([] if nonseparable_review_artifact is None else [SOURCE_NONSEPARABLE_REVIEW_SCHEMA]),
            "baseline_source_paths": {
                ...
                "trace_cadence_review_artifact_path": str(trace_cadence_review_artifact_path),
                "nonseparable_review_artifact_path": None if nonseparable_review_artifact_path is None else str(nonseparable_review_artifact_path),
            },
            "sparse_cadence_trigger_decision": cadence_review_artifact["decision"],
            "baseline_unique_checkpoint_list": copy.deepcopy(cadence_review_artifact.get("unique_simulation_checkpoints") or []),
            "baseline_ambiguity_signals": copy.deepcopy(cadence_review_artifact.get("ambiguity_signals") or []),
        },
    }
...
def build_provenance_guard(
    ...
    *,
    ...
    for field in ("row_id", "canonical_state", "legal_moves", "reference_move", "full_search_selected_move"):
    ...
    return {
        ...
        "baseline_selected_artifact": copy.deepcopy(baseline.get("source_selected_artifact")),
...
def build_dense_trace(baseline: dict, *, dense_trace_points: list[dict], checkpoint_capture_policy: dict) -> dict:
    ...
    if len(unique_checkpoints) <= 2:
        insufficiency_reasons.append("dense trace still collapses to only root/final checkpoints after deduplication")
    ...
    if divergent_index is not None:
        divergent_simulation = float(normalized_trace_points[divergent_index]["simulation"])
    ...
    return {
        ...
        "duplicate_root_snapshot_count": len(normalized_trace_points) - len(unique_checkpoints),
...
def build_regenerated_shared_drift_artifact(
    ...
    *,
    ...
    source_shared_drift_artifact = source_shared_drift_artifact or baseline["trace_capture_artifact"][
    ...
    regenerated["trace_capture_provenance"] = {
        ...
        "source_shared_drift_artifact_path": source_shared_drift_artifact["artifact_path"],
        "source_shared_drift_artifact_sha256": source_shared_drift_artifact["artifact_sha256"],
    }
    return regenerated
...
def _stable_trace_capture_artifact_sha256(payload: dict) -> str:
    ...
    regenerated_shared_drift_artifact = normalized_payload.get("regenerated_shared_drift_artifact") or {}
    trace_capture_provenance = regenerated_shared_drift_artifact.get("trace_capture_provenance") or {}
    trace_capture_provenance["trace_capture_artifact_sha256"] = None
    regenerated_shared_drift_artifact["trace_capture_provenance"] = trace_capture_provenance
    normalized_payload["regenerated_shared_drift_artifact"] = regenerated_shared_drift_artifact
    return hashlib.sha256((json.dumps(normalized_payload, indent=2, sort_keys=True) + "\n").encode("utf-8")).hexdigest()
...
def _classification_and_decision(
    *,
    ...
    for artifact in (
        ...
        artifact_classification = ((artifact or {}).get("classification") or {}).get("classification")
        artifact_decision = (artifact or {}).get("decision")
        if artifact_classification in {
    ...
    default_classification = ((dense_selection_score_artifact or {}).get("classification") or {}).get("classification")
    relaxed_classification = (
        (dense_threshold_relaxed_selection_score_artifact or {}).get("classification") or {}
    ).get("classification")
    if (
...
def build_payload(
    *,
    ...
    payload = {
        ...
        "full_search_selected_move": baseline["row_context"]["full_search_selected_move"],
        "trace_points": copy.deepcopy(dense_trace["trace_points"]),
        "insufficiency_reasons": copy.deepcopy(dense_trace["insufficiency_reasons"]),
        "checkpoint_capture_policy": copy.deepcopy(dense_trace["checkpoint_capture_policy"]),
        "trace_points_summary": {
            "unique_simulation_checkpoints": copy.deepcopy(dense_trace["unique_simulation_checkpoints"]),
            "unique_simulation_checkpoint_count": dense_trace["unique_simulation_checkpoint_count"],
            "duplicate_root_snapshot_count": dense_trace["duplicate_root_snapshot_count"],
            "first_divergent_selected_move_index": dense_trace["first_divergent_selected_move_index"],
        ...
        "source_baseline_artifacts": {
            ...
            "selection_score_artifact": copy.deepcopy(baseline["selection_score_artifact"]),
            "trace_cadence_review_artifact": copy.deepcopy(baseline["trace_cadence_review_artifact"]),
            "nonseparable_review_artifact": copy.deepcopy(baseline["nonseparable_review_artifact"]),
        },
        "row_context": copy.deepcopy(baseline["row_context"]),
        "source_selected_artifact": copy.deepcopy(baseline.get("source_selected_artifact")),
        "search_settings": copy.deepcopy(baseline.get("search_settings")),
        "cadence_validation": {
            "passed": provenance_guard.get("passed") and not dense_trace.get("insufficiency_reasons"),
            "insufficiency_reasons": copy.deepcopy(dense_trace.get("insufficiency_reasons") or [])
            + copy.deepcopy(provenance_guard.get("failures") or []),
        },
        "downstream_artifacts": {
            "dense_selection_score_artifact": copy.deepcopy(dense_selection_score_artifact),
            "dense_trace_cadence_review_artifact": copy.deepcopy(dense_trace_cadence_review_artifact),
            "dense_threshold_relaxed_selection_score_artifact": copy.deepcopy(
                dense_threshold_relaxed_selection_score_artifact
            ),
            "dense_nonseparable_review_artifact": copy.deepcopy(dense_nonseparable_review_artifact),
        },
    ...
    payload["artifact_write_summary"]["trace_capture_sha256"] = payload["trace_capture_artifact_sha256"]
    payload["regenerated_shared_drift_artifact"] = copy.deepcopy(regenerated_shared_drift_artifact)
    if (
        payload["regenerated_shared_drift_artifact"] is None
        and payload["cadence_validation"]["passed"]
        and dense_trace.get("trace_points")
    ):
        payload["regenerated_shared_drift_artifact"] = build_regenerated_shared_drift_artifact(
            baseline,
            dense_trace=dense_trace,
            trace_cadence_capture_artifact_path=trace_cadence_capture_artifact_path,
            trace_cadence_capture_artifact_sha256=payload["trace_capture_artifact_sha256"],
        )
    payload["trace_capture_artifact_sha256"] = _stable_trace_capture_artifact_sha256(payload)
    payload["artifact_write_summary"]["trace_capture_sha256"] = payload["trace_capture_artifact_sha256"]
    ...
    if isinstance(regenerated, dict):
        ...
        if isinstance(provenance, dict):
            provenance["trace_capture_artifact_sha256"] = payload["trace_capture_artifact_sha256"]
    return payload
...
def capture_dense_rerun_result(baseline: dict) -> dict:
    ...
    rerun_result = trace_capture._default_rerun_result(source_artifact, capture_mode="extract_then_rerun") or {}
    return {
        ...
        "search_settings": copy.deepcopy((trace_capture._upstream_inputs(source_artifact) or {}).get("search_settings")),
        "source_shared_drift_artifact": copy.deepcopy(source_artifact),
        "insufficiency_reasons": copy.deepcopy(rerun_result.get("insufficiency_reasons") or []),
    }
    ...
    *,
    ...
    if not provenance_guard.get("passed"):
        return {
            ...
            "downstream_rerun_summary": {"stopped_early": True, "stop_reason": "provenance_guard_failed"},
        }
    ...
    if dense_trace["insufficiency_reasons"]:
        return {
            ...
            "dense_threshold_relaxed_selection_score_artifact": None,
            "dense_nonseparable_review_artifact": None,
            "downstream_rerun_summary": {"stopped_early": True, "stop_reason": "trace_cadence_unresolved"},
        }

    dense_selection_score_path = str(out_path.with_name("capture_002_dense_selection_score_trace.json"))
    dense_trace_cadence_review_path = str(out_path.with_name("capture_002_dense_trace_cadence_review.json"))
    dense_threshold_relaxed_selection_score_path = str(
        out_path.with_name("capture_002_dense_threshold_relaxed_selection_score_trace.json")
    )
    dense_nonseparable_review_path = str(out_path.with_name("capture_002_dense_nonseparable_review.json"))
    ...
    dense_source_artifact = selection_score_trace.load_source_shared_drift_artifact_document(
        regenerated_shared_drift_artifact,
        artifact_path=str(out_path.with_name("capture_002_trace_cadence_rehydrated_shared_drift.json")),
    )
    dense_selection_score_artifact = selection_score_trace.build_payload(dense_source_artifact)
    ...
    if dense_selection_score_artifact["decision"] != "write_002_unresolved_trace_review_spec":
        stopped_early = True
        stop_reason = "dense_default_mechanism_resolved"
    elif dense_trace_cadence_review_artifact["decision"] != "continue_002_threshold_too_strict_check":
        stopped_early = True
        stop_reason = "trace_cadence_unresolved"
    else:
        dense_threshold_relaxed_selection_score_artifact = selection_score_trace.build_payload(
            dense_source_artifact,
            meaningful_q_margin=0.03,
            material_selection_score_margin=0.05,
            material_visit_share_margin=0.04,
        )
        if dense_threshold_relaxed_selection_score_artifact["decision"] != "write_002_unresolved_trace_review_spec":
            stopped_early = True
    ...
    return {
        ...
        "downstream_rerun_summary": {
            ...
            "emitted_decisions": {
                "dense_default_selection_score": dense_selection_score_artifact["decision"],
                "dense_trace_cadence_review": dense_trace_cadence_review_artifact["decision"],
...
def main(argv: list[str] | None = None) -> int:
    ...
    dense_trace = build_dense_trace(
        ...
        checkpoint_capture_policy={
            ...
            "checkpoint_schedule": [trace_point["simulation"] for trace_point in dense_trace_points],
    ...
    payload = build_payload(
        ...
        regenerated_shared_drift_artifact=rerun_outputs["regenerated_shared_drift_artifact"],
        dense_selection_score_artifact=rerun_outputs["dense_selection_score_artifact"],
        dense_trace_cadence_review_artifact=rerun_outputs["dense_trace_cadence_review_artifact"],
        dense_threshold_relaxed_selection_score_artifact=rerun_outputs[
            "dense_threshold_relaxed_selection_score_artifact"
        ],
        dense_nonseparable_review_artifact=rerun_outputs["dense_nonseparable_review_artifact"],
