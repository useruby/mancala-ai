#!/usr/bin/env python3
"""Run tiny train-only exact-tablebase diagnostic traces.
...
ARTIFACT_DIR = Path("/tmp/azlite_harder_endgame_tablebase_diagnostic_artifact")
...
REPORT_PATH = Path(
    "docs/alphazero-lite-harder-endgame-tablebase-diagnostic-artifact-results.md"
)
...
def convert_artifact_to_npz(artifact_dir: Path, out_path: Path) -> None:
    weights = json.loads((artifact_dir / "weights.json").read_text(encoding="utf-8"))
    arrays: dict[str, np.ndarray] = {}
    for key, val in weights.items():
        arrays[key] = np.asarray(val, dtype=np.float32)

    arrays["policy_head.weight"] = arrays.pop("w_policy").T.copy()
    arrays["policy_head.bias"] = arrays.pop("b_policy").copy()
    arrays["value_head.weight"] = arrays.pop("w_value").T.copy()
    arrays["value_head.bias"] = arrays.pop("b_value").copy()

    if "w_input" in arrays:
        arrays["input_layer.weight"] = arrays.pop("w_input").T.copy()
        arrays["input_layer.bias"] = arrays.pop("b_input").copy()
        idx = 1
        while f"w_residual_{idx}_1" in arrays:
            arrays[f"residual_layers.{idx - 1}.0.weight"] = arrays.pop(
                f"w_residual_{idx}_1"
            ).T.copy()
            arrays[f"residual_layers.{idx - 1}.0.bias"] = arrays.pop(
                f"b_residual_{idx}_1"
            ).copy()
            arrays[f"residual_layers.{idx - 1}.1.weight"] = arrays.pop(
                f"w_residual_{idx}_2"
            ).T.copy()
            arrays[f"residual_layers.{idx - 1}.1.bias"] = arrays.pop(
                f"b_residual_{idx}_2"
            ).copy()
            idx += 1
    else:
        idx = 1
        while f"w_hidden_{idx}" in arrays:
            arrays[f"hidden_layers.{idx - 1}.weight"] = arrays.pop(
                f"w_hidden_{idx}"
            ).T.copy()
            arrays[f"hidden_layers.{idx - 1}.bias"] = arrays.pop(
                f"b_hidden_{idx}"
            ).copy()
            idx += 1

    if "w_policy_hidden" in arrays:
        arrays["policy_hidden_layer.weight"] = arrays.pop("w_policy_hidden").T.copy()
        arrays["policy_hidden_layer.bias"] = arrays.pop("b_policy_hidden").copy()
    if "w_value_hidden" in arrays:
        arrays["value_hidden_layer.weight"] = arrays.pop("w_value_hidden").T.copy()
        arrays["value_hidden_layer.bias"] = arrays.pop("b_value_hidden").copy()
...
def apply_decision_rules(
    ...
    if control_regressed and prod_rate >= 0.3:
        results["classification"] = "exact_tablebase_overfit_or_control_regression"
        results["supporting_evidence"] = [
            f"prod_rate={prod_rate:.2f}",
            "control_regressed=True",
        ]
        results["rejected_alternatives"] = [
            "exact_tablebase_diagnostic_local_success",
            "exact_tablebase_weight_needed",
        ]
        results["next_action"] = (
            "add more preservation controls / reduce LR before scaling"
        )

    elif control_regressed:
        results["classification"] = "exact_tablebase_catastrophic_interference"
        results["supporting_evidence"] = ["control_regressed=True"]
        results["rejected_alternatives"] = [
            "all alternatives rejected due to control regression"
        ]
        results["next_action"] = (
            "stop artifact training and investigate representation interference"
        )

    elif prod_rate >= 0.5 and not control_regressed:
        results["classification"] = "exact_tablebase_diagnostic_local_success"
        results["supporting_evidence"] = [
            f"prod_rate={prod_rate:.2f}",
            f"control_regressed={control_regressed}",
        ]
        results["rejected_alternatives"] = [
            "exact_tablebase_value_only_signal",
            "exact_tablebase_no_local_signal",
        ]
        results["next_action"] = (
        ...
        results["classification"] = "exact_tablebase_weight_needed"
        results["supporting_evidence"] = [
            f"w1_rate={w1_rate:.2f}",
            f"w2_rate={w2_rate:.2f}",
        ]
        results["rejected_alternatives"] = ["exact_tablebase_diagnostic_local_success"]
        results["next_action"] = (
            "rerun a slightly larger exact-tablebase diagnostic with production weight 2 and more controls"
        )

    elif has_value_improvement and prod_rate < 0.3:
        results["classification"] = "exact_tablebase_value_only_signal"
        results["supporting_evidence"] = [
        ...
        results["rejected_alternatives"] = ["exact_tablebase_diagnostic_local_success"]
        results["next_action"] = (
            "build value-only exact-tablebase diagnostic plan; do not use policy targets yet"
        )
...
def main() -> int:
    ...
    print("=" * 70)
    print("HARDER ENDGAME TABLEBASE DIAGNOSTIC ARTIFACT TRACE")
    print("=" * 70)
    ...
    # Load holdout row states from diagnostics
    diag_rows = load_jsonl(
        Path(
            "/tmp/azlite_harder_endgame_tablebase_local_diagnostics/harder_endgame_tablebase_local_row_diagnostics.jsonl"
        )
    )
    diag_by_cid = {r.get("candidate_id", ""): r for r in diag_rows}

    holdout_rows = []
    for hid in holdout_ids:
        dr = diag_by_cid.get(hid)
        if dr:
            exact_tb = dr.get("exact_tablebase", {})
            holdout_rows.append(
                {
                    "candidate_id": hid,
                    "raw_state": dr.get("state"),
                    "exact_optimal_move": exact_tb.get("optimal_move"),
                    "exact_root_value": exact_tb.get("root_value"),
                }
            )
    ...
    traces = [
        {
            "name": "policy_value_w1",
            "sources": [POLICY_VALUE_PATH, CONTROLS_PATH],
            "weights": [1, 1],
            "epochs_list": [1, 2, 4],
        },
        {
            "name": "policy_value_w2",
            "sources": [POLICY_VALUE_PATH, CONTROLS_PATH],
            "weights": [2, 1],
            "epochs_list": [1, 2, 4],
        },
    ...
    # Check if value-only rows can be used
    has_value_only = len(value_only_rows) > 0
    if has_value_only:
        traces.append(
            {
                "name": "value_augmented_w1",
    ...
    for tc in traces:
        ...
        # Build combined training data with uniform policies for value-only rows
        ...
        for epochs in tc["epochs_list"]:
            # Use weight 1 for the single combined file
    ...
    # Build summary
    trace_summary = {
        "schema": "azlite_harder_endgame_tablebase_diagnostic_trace_v1",
        "family": FAMILY,
        "guardrails": {
    ...
    generate_report(
        all_evaluations, all_training_metrics, baseline, decision, eval_rows
    )
...
def generate_report(
    ...
    lines: list[str] = []
    lines.append("# Harder Endgame Tablebase Diagnostic Artifact — Results")
    lines.append("")
    ...
    lines.append("a tiny train-only exact-tablebase diagnostic artifact with controls.")
    lines.append("This run executes that recommendation.")
    lines.append("")

    # Section 2
    lines.append("## 2. Artifact construction")
    lines.append("")
    lines.append(
        "Rows from the PR #74 clean split were used to build three artifact files:"
    )
    lines.append("")
    lines.append("| artifact | row_count | roles | target_types | value_source |")
    lines.append("|---|---|---|---|---|")
    art_summary = json.loads(ARTIFACT_SUMMARY_PATH.read_text(encoding="utf-8"))
    for aname, ainfo in art_summary.get("artifacts", {}).items():
        lines.append(
            f"| {aname} | {ainfo.get('row_count', 0)} | "
            f"{', '.join(ainfo.get('roles', []))} | "
            f"{', '.join(ainfo.get('target_types', []))} | "
            f"{ainfo.get('value_source', '')} |"
        )
    lines.append("")

    # Section 3
    lines.append("## 3. Current baseline")
    lines.append("")
    ...
    # Section 4: Traces
    lines.append("## 4. Trace definitions")
    lines.append("")
    lines.append("| trace_name | data_files | replay_weights | epochs | status |")
    lines.append("|---|---|---|---|---|")
    trace_configs = {
        "policy_value_w1": {"files": "policy_value + controls", "weights": "1,1"},
        "policy_value_w2": {"files": "policy_value + controls", "weights": "2,1"},
        "value_augmented_w1": {
    ...
    for tc_name, tc_info in trace_configs.items():
        ...
        lines.append(
            f"| {tc_name} | {tc_info['files']} | {tc_info['weights']} | 1,2,4 | {status} |"
        )
    lines.append("")

    # Section 5: Production results
    lines.append("## 5. Production-candidate local results")
    lines.append("")
    for tn, td in all_evaluations.items():
        if tn == "baseline":
            continue
    ...
    # Section 6: Value-only results
    lines.append("## 6. Value-only local results")
    lines.append("")
    for tn, td in all_evaluations.items():
        if tn == "baseline":
            continue
    ...
    lines.append("## 7. Preservation-control results")
    lines.append("")
    for tn, td in all_evaluations.items():
        if tn == "baseline":
            continue
    ...
    # Section 8: Holdout
    lines.append("## 8. Holdout generalization")
    lines.append("")
    for tn, td in all_evaluations.items():
        if tn == "baseline":
            continue
    ...
    # Section 9: Training metrics
    lines.append("## 9. Training metrics")
    lines.append("")
    lines.append("| trace_name | epochs | policy_loss | value_loss | elapsed_s |")
    lines.append("|---|---|---|---|---|")
    for tm in all_training_metrics:
        lines.append(
    ...
    lines.append("## 11. Exactly one recommended next action")
    lines.append("")
