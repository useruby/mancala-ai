...
DEFAULT_SELECTED_ROWS_PATH = Path(
    "/tmp/azlite_corrected_non_opening_failure_mining_v7/"
    "selected_non_opening_family_rows_v7.jsonl"
)
DEFAULT_SUMMARY_OUT = Path(
    "/tmp/azlite_early_extra_turn_reference_adjudication/"
    "early_extra_turn_reference_adjudication_summary.json"
)
DEFAULT_PATCH_OUT = Path(
    "/tmp/azlite_early_extra_turn_reference_adjudication/"
    "early_extra_turn_reference_review_patch_v1.json"
)
DEFAULT_REPORT_OUT = Path(
    "docs/alphazero-lite-early-extra-turn-reference-adjudication-results.md"
)
FAMILY = "early_extra_turn"
SCHEMA = "azlite_early_extra_turn_reference_adjudication_v1"
...
# Rows to adjudicate: corrected_reference_suspicious from PR #67
CORRECTED_REFERENCE_SUSPICIOUS_ROW_IDS = (
    "early_extra_turn-016",
    "early_extra_turn-012",
    "early_extra_turn-013",
    "early_extra_turn-014",
    "early_extra_turn-018",
)
VALUE_HEAD_MISCALIBRATION_ROW_IDS = ("early_extra_turn-025",)
HOLDOUT_ROW_IDS = ("early_extra_turn-017",)
...
ALL_REPORTED_ROW_IDS = (
    *CORRECTED_REFERENCE_SUSPICIOUS_ROW_IDS,
    *VALUE_HEAD_MISCALIBRATION_ROW_IDS,
    *HOLDOUT_ROW_IDS,
    *PRESERVATION_CONTROL_ROW_IDS,
)

PR67_CLASSIFICATION = {
    "early_extra_turn-016": "corrected_reference_suspicious",
    "early_extra_turn-012": "corrected_reference_suspicious",
    "early_extra_turn-013": "corrected_reference_suspicious",
    "early_extra_turn-014": "corrected_reference_suspicious",
    "early_extra_turn-018": "corrected_reference_suspicious",
    "early_extra_turn-025": "value_head_miscalibration",
    "early_extra_turn-017": "holdout_context",
    "early_extra_turn-007": "preservation_control",
    "early_extra_turn-011": "preservation_control",
    "early_extra_turn-002": "preservation_control",
    "early_extra_turn-009": "preservation_control",
}

FALLBACK_ROLE_BY_ROW = {
    **{row_id: "target_candidate" for row_id in CORRECTED_REFERENCE_SUSPICIOUS_ROW_IDS},
    **{row_id: "target_candidate" for row_id in VALUE_HEAD_MISCALIBRATION_ROW_IDS},
...
ADJUDICATED_ROW_IDS = CORRECTED_REFERENCE_SUSPICIOUS_ROW_IDS

PR67_CONTEXT = {
    "family_classification": "reference_family_uncertain",
    "value_head_miscalibration_count": 1,
    "puct_child_search_mismatch_count": 0,
    "root_selection_pressure_count": 0,
    "corrected_reference_suspicious_count": 5,
    "inconclusive_count": 0,
    "blocked_next_action": "adjudicate early_extra_turn references before training.",
}
...
def rerun_family_mining_if_needed(root: Path, args: argparse.Namespace) -> None:
    ...
    command = [
        python_bin(root),
        "-m",
        "ml.alphazero_lite.run_corrected_non_opening_failure_family_mining_v7",
    ]
...
def tablebase_legal_move_values(
    ...
    return {
        ...
        "is_forcing": abs(root_value) >= 1.0 - 1e-9,
...
def projected_buckets(
    *,
    ...
    tablebase_tie_rows = sorted(
        row["row_id"]
        for row in row_decision_table
        if row["adjudicated_decision"] == "tablebase_tie_not_conflict"
    )
    value_miscalibration_context = sorted(
        row_id
        for row_id in VALUE_HEAD_MISCALIBRATION_ROW_IDS
    ...
    buckets = [
        ...
        {
            "bucket": "tablebase_tie_or_arbitrary_policy_rows",
            "row_count": len(tablebase_tie_rows),
            "rows": tablebase_tie_rows,
            "train_target_eligible_count": 0,
            "risks": "tablebase-tied positions have arbitrary policy labels",
            "next_action": "exclude from hard policy gates; keep as diagnostic context only",
        },
        {
            ...
            "train_target_eligible_count": 0,
            "risks": "miscalibration may require separate value-calibration pass",
            "next_action": "reserve for later value-calibration if references settle",
        },
        {
    ...
    if flip_count >= 3:
        ...
        recommendation = "review and apply the proposed patch artifact, then rerun early_extra_turn value/backup audit."
    elif (
        confirmed_count >= max(5, (adjudicated_count * 3 + 4) // 5)
        and confirmed_count >= 4
    ):
        decision = "references_clean_enough_for_value_calibration"
        recommendation = "build a small train-only early_extra_turn value-calibration artifact with preservation controls; no arena until local value metrics improve."
    elif unstable_count >= 3:
        decision = "reference_suite_too_noisy_for_early_extra_turn"
        recommendation = (
            "exclude early_extra_turn and select the next non-opening family."
        )
    elif divergence_count >= 3:
        decision = "teacher_family_divergence"
        recommendation = "decide teacher policy for early_extra_turn before training."
    else:
        decision = "still_reference_uncertain"
        recommendation = "do not train on early_extra_turn; rerun non-opening family mining and pick the next family."
    ...
    return (
        buckets,
        {
            ...
            + len(divergence_rows)
            + len(tablebase_tie_rows),
            "early_extra_turn_good_training_target_after_adjudication": decision
            == "references_clean_enough_for_value_calibration",
            "early_extra_turn_025_later_value_calibration_safe": (
                decision == "references_clean_enough_for_value_calibration"
                and len(value_miscalibration_context) > 0
            ),
        },
    )
...
def build_report(summary: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Family-level classification: `{summary['pr67_context']['family_classification']}`.",
        f"- Mechanism counts: `{json.dumps(summary['pr67_context'], sort_keys=True)}`.",
        f"- Exact PR #67 recommendation: **{summary['pr67_context']['blocked_next_action']}**",
        "",
        "## 3. Row validation",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            f"- `early_extra_turn` remains a good training target after adjudication: `{str(bool(summary['early_extra_turn_good_training_target_after_adjudication'])).lower()}`.",
            f"- `early_extra_turn-025` is safe enough to seed a later value-calibration bucket: `{str(bool(summary['early_extra_turn_025_later_value_calibration_safe'])).lower()}`.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    for row_id in ALL_REPORTED_ROW_IDS:
        ...
        if not notes:
            ...
            elif row_id in VALUE_HEAD_MISCALIBRATION_ROW_IDS:
        ...
        validation_row = {
            "row_id": row_id,
            "role": role,
            "pr67_classification": PR67_CLASSIFICATION[row_id],
    ...
    for row in valid_adjudicated_rows:
        ...
        for budget in PUCT_BUDGETS:
            ...
            puct_budget_summaries[str(budget)] = {
                ...
                "notes": "deterministic artifact PUCT with PR #67 settings",
            }
        ...
        row_result = {
            ...
            "pr67_classification": row["pr67_classification"],
            "active_reference_move": reference_move,
