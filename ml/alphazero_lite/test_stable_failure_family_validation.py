...
class StableFailureFamilyValidationScriptTest(unittest.TestCase):
    def load_module(self):
        ...
        script_path = (
            repo_root / "script/ai/run_tactical_stable_failure_family_validation"
        )
    ...
    def test_dry_run_pins_reference_replay_and_summary_artifacts(self):
        ...
        with tempfile.TemporaryDirectory(
            prefix="azlite-stable-failure-family-validation-"
        ) as tmp:
        ...
        self.assertEqual(
            str(run_paths["stable_family_summary_path"]), pins["stable-family-summary"]
        )
        ...
        self.assertEqual(
            ...
            stable_summary_command[
                stable_summary_command.index("--candidate-forensics") + 1
            ],
        )
        self.assertEqual(
            pins["family-report"],
            stable_summary_command[
                stable_summary_command.index("--opening-family-report") + 1
            ],
        )
        self.assertEqual(
    ...
    def test_run_executes_stable_summary_stage_before_verdict_and_writes_summary(self):
        ...
        with tempfile.TemporaryDirectory(
            ...
            run_paths["replay_summary_path"].write_text(
                json.dumps(stale_replay_summary), encoding="utf-8"
            )
            ...
            def fake_run_command(command, cwd, allowed_returncodes=(0,)):
                ...
                if command[1].endswith(
                    ...
                    if "--start-stage" not in command:
                        ...
                        replay_summary_seen_by_validation.append(
                            run_paths["replay_summary_path"].exists()
                        )
                        run_paths["replay_summary_path"].write_text(
                            json.dumps(fresh_replay_summary), encoding="utf-8"
                        )
                    else:
                    ...
                    run_paths["family_report_path"].write_text(
                        json.dumps(
                            {
                                "schema": "opening_capture_family_report_v1",
                                "rows": [],
                                "missing_references": [],
                            }
                        ),
                    ...
                    run_paths["stable_family_summary_path"].parent.mkdir(
                        parents=True, exist_ok=True
                    )
                    run_paths["stable_family_summary_path"].write_text(
                        json.dumps(self.stable_family_summary()), encoding="utf-8"
                    )
                elif command[1:3] == [
                    "-m",
        ...
        stage_names = [
            command[0][2]
            if command[0][1:3] and command[0][1] == "-m"
            else command[0][1]
        ...
        self.assertEqual(
            str(run_paths["replay_summary_path"]), summary["replay_summary_path"]
        )
        self.assertEqual(
            "azlite_stable_failure_family_summary_v1",
            summary["stable_family_summary"]["schema"],
        )
        self.assertEqual(
            str(run_paths["stable_family_summary_path"]),
            summary["stable_family_summary_path"],
        )
    ...
    def test_run_uses_original_selected_target_for_validation_pack_when_selection_artifact_is_materialized_copy(
        self,
    ):
        module = self.load_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            ...
            (original_selected_target / "model.npz").write_text(
                "stub", encoding="utf-8"
            )
            ...
            def fake_run_command(command, cwd, allowed_returncodes=(0,)):
                if command[1].endswith(
                    ...
                    if "--start-stage" not in command:
                        ...
                        run_paths["replay_summary_path"].parent.mkdir(
                            parents=True, exist_ok=True
                        )
                        run_paths["replay_summary_path"].write_text(
                            json.dumps(self.replay_summary()), encoding="utf-8"
                        )
                    else:
                        validation_selected_artifacts.append(
                            command[command.index("--selected-artifact") + 1]
                        )
                        run_paths["final_dir"].mkdir(parents=True, exist_ok=True)
                        for artifact_name in (
                            "baseline_candidate_forensics.json",
        ...
        with tempfile.TemporaryDirectory(
            ...
            with mock.patch.object(module, "run_command", side_effect=fake_run_command):
                with self.assertRaisesRegex(
                    SystemExit,
                    "azlite_tactical_validation_verdict_v1|invalid verdict artifact|boolean passed",
                ):
                    module.run(args, repo_root)
    ...
    def test_run_rejects_malformed_stable_family_summary(self):
        ...
        with tempfile.TemporaryDirectory(
            ...
            with mock.patch.object(module, "run_command", side_effect=fake_run_command):
                with self.assertRaisesRegex(
                    SystemExit, "stable family summary.*capture_available.tracked_rows"
                ):
                    module.run(args, repo_root)
    ...
    def test_load_fresh_stable_family_summary_rejects_non_finite_metrics(self):
        ...
        with tempfile.TemporaryDirectory(prefix="azlite-stable-family-summary-") as tmp:
            ...
            with self.assertRaisesRegex(
                SystemExit, "stable family summary.*capture_available.average_regret"
            ):
    ...
    def test_run_aborts_when_validation_pack_fails_with_stale_final_artifacts_present(
        self,
    ):
        module = self.load_module()
        repo_root = Path(__file__).resolve().parents[2]
        ...
        with tempfile.TemporaryDirectory(
            ...
            run_paths["family_report_path"].write_text(
                json.dumps(
                    {"schema": "opening_capture_family_report_v1", "stale": True}
                ),
                encoding="utf-8",
            )
            run_paths["stable_family_summary_path"].write_text(
            ...
            def fake_run_command(command, cwd, allowed_returncodes=(0,)):
                ...
                raise AssertionError(
                    f"wrapper should not continue after validation_pack failure: {command}"
                )

            with mock.patch.object(module, "run_command", side_effect=fake_run_command):
                with self.assertRaisesRegex(
                    SystemExit, "validation_pack|exit code 1|failed"
                ):
                    module.run(args, repo_root)
