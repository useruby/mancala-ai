...
DEFAULT_SELECTED_ROWS_PATH = Path(
    "/tmp/azlite_corrected_non_opening_failure_mining_v5/"
    "selected_non_opening_family_rows_v5.jsonl"
)
DEFAULT_MINING_SUMMARY_PATH = Path(
    "/tmp/azlite_corrected_non_opening_failure_mining_v5/"
    "non_opening_failure_family_summary_v5.json"
)
DEFAULT_SUMMARY_OUT = Path(
    "/tmp/azlite_starvation_pressure_value_backup_audit/"
    "starvation_pressure_value_backup_audit_summary.json"
)
DEFAULT_REPORT_OUT = Path(
    "docs/alphazero-lite-starvation-pressure-value-backup-audit-results.md"
)
FAMILY = "starvation_pressure"
SCHEMA = "azlite_starvation_pressure_value_backup_audit_v1"
...
REQUIRED_TARGET_ROW_IDS = (
    ...
    "starvation_pressure-015",
    "starvation_pressure-001",
)
...
def rerun_family_mining_if_needed(root: Path, args: argparse.Namespace) -> None:
    ...
    command = [
        python_bin(root),
        "-m",
        "ml.alphazero_lite.run_corrected_non_opening_failure_family_mining_v5",
    ]
...
def starvation_metrics_for_player(
    ...
    if int(player) == 0:
        ...
        player_pits = [int(seed) for seed in state["opponent_pits"]]
        opponent_pits = [int(seed) for seed in state["player_pits"]]
    player_total = sum(player_pits)
    opponent_total = sum(opponent_pits)
    empty_player = sum(1 for seed in player_pits if seed == 0)
    empty_opponent = sum(1 for seed in opponent_pits if seed == 0)
    is_pressure = (
        player_total <= 6
        or opponent_total <= 6
        or empty_player >= 4
    ...
    pressure_score = int(player_total <= 6) + int(opponent_total <= 6)
    pressure_score += max(0, empty_player - 3) + max(0, empty_opponent - 3)
    return {
        "player_total": int(player_total),
        "opponent_total": int(opponent_total),
        "empty_player": int(empty_player),
        "empty_opponent": int(empty_opponent),
        "is_starvation_pressure": bool(is_pressure),
        "pressure_score": int(pressure_score),
    }


def starvation_pressure_delta_label(
    before: dict[str, int | bool], after: dict[str, int | bool]
) -> str:
    before_score = int(before["pressure_score"])
    after_score = int(after["pressure_score"])
    if after_score > before_score:
        ...
        trend = "unchanged"
    return (
        f"{trend}; self_total={after['player_total']}; opp_total={after['opponent_total']}; "
        f"empty_self={after['empty_player']}; empty_opp={after['empty_opponent']}; "
        f"pressure={str(bool(after['is_starvation_pressure'])).lower()}"
    )
...
def compare_move_consequences(
    *, row: AuditRow, baseline_1200: dict[str, Any]
    ...
    root_pressure = starvation_metrics_for_player(row.suite_state, root_player)
    notes = []
    if selected_move is not None and selected_move != row.corrected_reference_move:
        ...
        selected_child_state = child_state_from_move(
            row.suite_state, int(selected_move)
        )
        reference_child_state = child_state_from_move(
            row.suite_state, row.corrected_reference_move
        )
        selected_pressure = starvation_metrics_for_player(
            selected_child_state, root_player
        )
        reference_pressure = starvation_metrics_for_player(
            reference_child_state, root_player
        )
        if int(selected_pressure["pressure_score"]) < int(
            reference_pressure["pressure_score"]
        ):
            notes.append("selected move reduces starvation pressure")
        elif int(selected_pressure["pressure_score"]) > int(
            reference_pressure["pressure_score"]
        ):
            notes.append("selected move increases starvation pressure")
        if (
            int(selected_entry.get("side_to_move_after", -1)) != root_player
            and int(reference_entry.get("side_to_move_after", -1)) == root_player
        ):
    ...
    for move in baseline_legal_moves:
        ...
        child_state = child_state_from_move(row.suite_state, int(move))
        child_pressure = starvation_metrics_for_player(child_state, root_player)
        rows.append(
            {
                "row_id": row.row_id,
                "move": int(move),
                ...
                "pit_index": int(move),
                "move_index": int(move),
                "starvation_pressure_feature_if_available": starvation_pressure_delta_label(
                    root_pressure, child_pressure
                ),
                "notes": "; ".join(notes)
                if selected_move is not None
...
def classify_family(
    ...
    if dominant == "value_head_miscalibration":
        return (
            ...
            "build a small train-only child-afterstate value-calibration artifact for starvation_pressure, with controls and no arena until local value metrics improve.",
            dict(counts),
        )
    ...
    if dominant == "root_selection_pressure":
        return (
            ...
            "run cpuct/root-prior calibration diagnostics for starvation_pressure.",
            dict(counts),
        )
    if dominant == "corrected_reference_suspicious":
        return (
            "reference_family_uncertain",
            "adjudicate starvation_pressure references before training.",
            dict(counts),
        )
    return (
        "mixed_family_gap",
        "split starvation_pressure into mechanism-specific buckets and choose the largest stable bucket.",
        dict(counts),
    )
    ...
    lines.extend(
        markdown_table(
            [
                ...
                "starvation_pressure_feature_if_available",
                "notes",
            ],
            [
                [
                    ...
                    row["starvation_pressure_feature_if_available"],
                    row["notes"],
                ]
