...
SOURCE_SELECTION_SCORE_COMPONENT_AUDIT_SCHEMA = "azlite_capture_002_selection_score_component_audit_v1"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audit capture 002 prior-pressure support signatures")
    parser.add_argument("--source-selection-score-component-audit-artifact", type=Path, required=True)
...
def _validate_matching_source_identity(source_artifact: dict, *, expected_source_artifact: dict, context: str) -> None:
    for field in ("reference_move", "full_search_selected_move"):
    ...
    if source_artifact.get("selected_artifact") != expected_source_artifact.get("selected_artifact"):
        raise ValueError(
...
def _validated_payload_metadata_thresholds(trace_artifact: dict, *, context: str) -> dict:
...
def _validate_canonicalization_artifact(
    ...
    *,
    ...
    if artifact.get("source_path") not in (None, source_checkpoint_canonicalization_artifact_path):
...
def _validate_component_audit_inputs(
    ...
    *,
    ...
    input_artifacts = artifact.get("input_artifacts")
    if not isinstance(input_artifacts, dict):
        raise ValueError("prior-pressure component audit upstream input_artifacts must be an object")
    if input_artifacts.get("source_metric_audit_artifact_path") != source_metric_audit_artifact_path:
        raise ValueError(
    ...
    component_canonicalization_path = input_artifacts.get("source_checkpoint_canonicalization_artifact_path")
    ...
    elif component_canonicalization_path != source_checkpoint_canonicalization_artifact_path:
        raise ValueError(
...
def _validate_required_prior_pressure_checkpoints(artifact: dict) -> None:
    ...
    if not isinstance(first_material_checkpoints, dict):
        raise ValueError("prior-pressure component audit upstream first_material_checkpoints must be an object")
...
def _matching_trace_point(trace_points: list[dict], *, simulation: float) -> dict | None:
...
def _selected_and_reference_visits(trace_point: dict, *, selected_move: int, reference_move: int) -> tuple[float | None, float | None]:
    ...
    if isinstance(selected_visits, bool) or isinstance(reference_visits, bool):
        return None, None
    if not isinstance(selected_visits, (int, float)) or not isinstance(reference_visits, (int, float)):
        return None, None
...
def _visit_shares(trace_point: dict, *, selected_move: int, reference_move: int) -> tuple[float | None, float | None]:
...
def _validated_branch_threshold(trace_artifact: dict, *, branch: str, key: str) -> float:
    thresholds = trace_artifact.get("thresholds")
    value = thresholds.get(key) if isinstance(thresholds, dict) else None
    return _validated_non_negative_threshold(value, context=f"{branch} branch evidence thresholds.{key}")
...
def _stronger_upstream_residual_threshold(component_audit_artifact: dict) -> float | None:
...
def _residual_threshold(component_audit_artifact: dict) -> float:
    upstream_residual_threshold = _stronger_upstream_residual_threshold(component_audit_artifact)
    if upstream_residual_threshold is not None:
        return upstream_residual_threshold
    return FLOAT_TOLERANCE


def _prior_pressure_checkpoint(trace_point: dict, *, selected_move: int, reference_move: int, thresholds: dict) -> dict | None:
    ...
    if q_margin is not None and q_margin >= thresholds["meaningful_q_margin"] - FLOAT_TOLERANCE:
        return None
    return {
        "simulation": _finite_number(trace_point.get("simulation"), context="prior-pressure checkpoint simulation"),
...
def _checkpoint_audit(
    ...
    *,
    ...
    audit = []
    for default_trace_point, relaxed_trace_point in zip(default_trace_points, relaxed_trace_points, strict=True):
        default_simulation = _finite_number(
            default_trace_point.get("simulation"), context="checkpoint audit default simulation"
        )
        relaxed_simulation = _finite_number(
            relaxed_trace_point.get("simulation"), context="checkpoint audit relaxed simulation"
        )
        if default_simulation != relaxed_simulation:
            raise ValueError("checkpoint audit default and relaxed trace simulations must align")
        audit.append(
            {
...
def _branch_has_material_residual_vs_visit_conflict(branch_evidence: dict) -> bool:
    selection_score_residual_margin = branch_evidence.get("selection_score_residual_margin")
    selection_score_residual_threshold = branch_evidence.get("selection_score_residual_threshold")
...
def _overall_evidence_summary(classification: str, branch_disagreement_summary: dict) -> str:
...
def _build_branch_level_evidence(
    ...
    *,
    ...
    trace_point = _matching_trace_point(trace_artifact.get("trace_points", []), simulation=checkpoint.get("simulation"))
    selection_score_margin = _selection_score_margin(
        trace_point,
        selected_move=selected_move,
        reference_move=reference_move,
    ...
    selection_score_residual_margin = _selection_score_residual_margin(selection_score_margin, q_margin)
    return {
...
def _validate_prior_pressure_checkpoint_against_trace(
    ...
    *,
    ...
    trace_point = _matching_trace_point(trace_artifact.get("trace_points", []), simulation=simulation)
    ...
    if not _matches_expected_margin(checkpoint.get("selection_score_margin"), selection_score_margin):
        raise ValueError(
    ...
    if earliest_material_checkpoint is None or not (
        _matches_expected_margin(checkpoint.get("simulation"), earliest_material_checkpoint["simulation"])
        and _matches_expected_margin(
            checkpoint.get("selection_score_margin"),
            earliest_material_checkpoint["selection_score_margin"],
        )
        and _matches_expected_margin(checkpoint.get("q_margin"), earliest_material_checkpoint["q_margin"])
    ):
        raise ValueError(
...
def build_payload(
    ...
    *,
    ...
    if not isinstance(selection_score_component_audit_artifact, dict):
        raise ValueError("prior-pressure component audit upstream artifact must be an object")
    if selection_score_component_audit_artifact.get("schema") != SOURCE_SELECTION_SCORE_COMPONENT_AUDIT_SCHEMA:
        raise ValueError("prior-pressure component audit upstream artifact has wrong schema")
    ...
    if not isinstance(classification, dict):
        raise ValueError("prior-pressure component audit upstream classification must be an object")
    ...
    if selection_score_component_audit_artifact.get("decision") != EXPECTED_COMPONENT_AUDIT_DECISION:
        raise ValueError(
    ...
    _validate_required_prior_pressure_checkpoints(selection_score_component_audit_artifact)
    ...
    if _validated_classification_name(metric_audit_artifact, context="metric audit artifact") != EXPECTED_METRIC_AUDIT_CLASSIFICATION:
    ...
    metric_source_artifact = _validated_source_artifact(metric_audit_artifact, context="metric audit artifact")
    ...
    if _validated_classification_name(default_trace_artifact, context="default trace artifact") != EXPECTED_TRACE_CLASSIFICATION:
        raise ValueError(f"default trace artifact classification must be {EXPECTED_TRACE_CLASSIFICATION}")
    if default_trace_artifact.get("decision") != EXPECTED_TRACE_DECISION:
        raise ValueError(f"default trace artifact decision must be {EXPECTED_TRACE_DECISION}")
    default_trace_source_artifact = _validated_source_artifact(default_trace_artifact, context="default trace artifact")
    _validate_matching_source_identity(
    ...
    if _validated_classification_name(relaxed_trace_artifact, context="relaxed trace artifact") != EXPECTED_TRACE_CLASSIFICATION:
        raise ValueError(f"relaxed trace artifact classification must be {EXPECTED_TRACE_CLASSIFICATION}")
    if relaxed_trace_artifact.get("decision") != EXPECTED_TRACE_DECISION:
        raise ValueError(f"relaxed trace artifact decision must be {EXPECTED_TRACE_DECISION}")
    relaxed_trace_source_artifact = _validated_source_artifact(relaxed_trace_artifact, context="relaxed trace artifact")
    _validate_matching_source_identity(
    ...
    default_trace_points = _validated_trace_points(default_trace_artifact, context="default trace artifact")
    relaxed_trace_points = _validated_trace_points(relaxed_trace_artifact, context="relaxed trace artifact")
    default_thresholds = default_trace_artifact.get("thresholds")
    relaxed_thresholds = relaxed_trace_artifact.get("thresholds")
    default_trace_origin = _validated_trace_origin(default_trace_artifact, context="default trace artifact")
    relaxed_trace_origin = _validated_trace_origin(relaxed_trace_artifact, context="relaxed trace artifact")
    ...
    first_material_checkpoints = selection_score_component_audit_artifact["first_material_checkpoints"]
    ...
    thresholds_evaluated = {
        "default": _validated_payload_metadata_thresholds(default_trace_artifact, context="default trace artifact"),
        "relaxed": _validated_payload_metadata_thresholds(relaxed_trace_artifact, context="relaxed trace artifact"),
    }
    ...
    return {
        ...
        "classification": {
            ...
            "evidence_summary": _overall_evidence_summary(overall_classification, branch_disagreement_summary),
        },
        ...
        "source_snapshots": {
            "component_audit_classification": copy.deepcopy(selection_score_component_audit_artifact.get("classification")),
...
def main(argv: list[str] | None = None) -> int:
    ...
    selection_score_component_audit_artifact = load_json(args.source_selection_score_component_audit_artifact)
    ...
    payload = build_payload(
        ...
        source_selection_score_component_audit_artifact_path=str(args.source_selection_score_component_audit_artifact),
