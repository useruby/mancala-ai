...
DEFAULT_OUT_ROOT = Path("/tmp/azlite_corrected_non_opening_failure_mining_v3")
DEFAULT_SUMMARY_PATH = DEFAULT_OUT_ROOT / "non_opening_failure_family_summary_v3.json"
DEFAULT_SELECTED_ROWS_PATH = (
    DEFAULT_OUT_ROOT / "selected_non_opening_family_rows_v3.jsonl"
)
DEFAULT_REPORT_PATH = Path(
    "docs/alphazero-lite-corrected-non-opening-failure-family-mining-v3-results.md"
)
...
def exclusion_summary_rows(excluded_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    labels = [
        ...
        (
            ...
            (
                "every target-candidate row was classified reference_unstable "
                "or still_inconclusive; no eligible training-target rows remain"
            ),
        ),
        (
...
def render_report(summary: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Projected family decision: `{HIGH_VALUE_SWING_EXCLUSION_REASON}`.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if selected_family_summary is not None:
        ...
        selected_family_report = {
            ...
            "reason_selected": (
                ...
                "exclusions (including high_value_swing after PR #53); "
    ...
    summary = {
        "schema": "azlite_corrected_non_opening_failure_family_mining_v3",
        "inventory_source": {
        ...
        "high_value_swing_exclusion": {
            "excluded": True,
            "reason": HIGH_VALUE_SWING_EXCLUSION_REASON,
            "projected_family_decision": HIGH_VALUE_SWING_EXCLUSION_REASON,
            "target_candidate_rows_eligible_after_active_references": 0,
            "target_candidate_rows_requiring_patching": 0,
            "target_candidate_rows_that_should_stay_excluded": 14,
            "source_doc_path": "docs/alphazero-lite-high-value-swing-reference-adjudication-results.md",
        },
        "inventory_rows": [
            {
                "row_id": row["row_id"],
