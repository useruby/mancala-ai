...
class BuildTacticalStableFailureFamilyReplayTest(unittest.TestCase):
    ...
    def preservation_capture_row(self, canonical_state, *, priority, variant):
        rows = {
            "gain_4_5": {
                ...
                "policy": [0.0013778912834823132, 0.24870026111602783, 0.059389978647232056, 0.0, 0.4638570547103882, 0.22667483985424042],
            ...
            "gain_2_3": {
                ...
                "policy": [0.01769360341131687, 0.03087548352777958, 0.44671961665153503, 0.3265913426876068, 0.0, 0.17811991274356842],
    ...
    def nearby_row(self, bucket, index, *, priority):
        ...
        raw_state = json.loads(json.dumps(bucket_states[bucket][index % len(bucket_states[bucket])]))
        raw_state["player_store"] += index // len(bucket_states[bucket])
        raw_state["opponent_store"] += (index + 1) // len(bucket_states[bucket])
        side_pits = raw_state["player_pits"] if raw_state["current_player"] == 0 else raw_state["opponent_pits"]
    ...
    def rebalance_reference_payload(self, module, *, unstable_opening_ids=None, unstable_high_imbalance_ids=None):
        ...
        for row in self.rebalance_suite_rows():
            row_id = row["id"]
            reference_move = opening_reference_moves.get(row_id, imbalance_reference_moves.get(row_id))
            unstable = row_id in unstable_opening_ids or row_id in unstable_high_imbalance_ids
            alternate_reference_move = next(
                (move for move in row["legal_moves"] if move != reference_move),
                None,
            )
            observed_reference_moves = [reference_move] if not unstable else [reference_move, alternate_reference_move]
            seed_samples = [
                {"seed": 2040, "reference_move": reference_move, "teacher_value": 0.42},
            ]
            if unstable:
                if alternate_reference_move is None:
                    raise ValueError(f"unstable test fixture requires alternate legal move for {row_id}")
                seed_samples.append(
                    {"seed": 3040, "reference_move": observed_reference_moves[1], "teacher_value": 0.41}
                )
            rows.append(
                {
                    "id": row_id,
    ...
    def rebalance_source_rows(self, *, nearby_counts=None):
        nearby_counts = dict(nearby_counts or {
            "high_imbalance": 4,
            "high_value_swing": 6,
        ...
        source_rows = [
            self.preservation_capture_row("preserve-capture-a", priority=9.0, variant="gain_4_5"),
            self.preservation_capture_row("preserve-capture-b", priority=8.0, variant="gain_2_3"),
            self.preservation_capture_row("preserve-capture-c", priority=7.0, variant="gain_6_plus"),
            self.preservation_capture_row("preserve-capture-d", priority=6.0, variant="gain_6_plus_alt"),
        ]
    ...
    def test_selects_structural_opening_capture_rows_for_reference_moves_three_and_four(self):
        from ml.alphazero_lite import build_tactical_stable_failure_family_replay as module

        suite_rows = [
            {
                "id": "capture_available-016",
                "state": {"player_pits": [5, 1, 5, 5, 5, 0], "opponent_pits": [1, 6, 0, 7, 6, 5], "player_store": 1, "opponent_store": 1, "current_player": 0},
                "side_to_move": 0,
                "legal_moves": [0, 1, 2, 3, 4],
                "phase": "opening",
                "bucket": "capture_available",
            ...
            {
                "id": "capture_available-017",
                "state": {"player_pits": [5, 1, 6, 5, 5, 0], "opponent_pits": [1, 6, 6, 0, 6, 5], "player_store": 1, "opponent_store": 1, "current_player": 0},
                "side_to_move": 0,
                "legal_moves": [0, 1, 2, 3, 4],
                "phase": "opening",
                "bucket": "capture_available",
            ...
            {
                "id": "capture_available-018",
                "state": {"player_pits": [5, 1, 5, 5, 0, 6], "opponent_pits": [1, 6, 0, 7, 6, 5], "player_store": 1, "opponent_store": 1, "current_player": 0},
                "side_to_move": 0,
                "legal_moves": [0, 1, 2, 3, 5],
                "phase": "opening",
                "bucket": "capture_available",
        ...
        reference_rows = [
            {"id": "capture_available-016", "canonical_state": module.canonical_state_key(suite_rows[0]["state"]), "state": suite_rows[0]["state"], "reference_move": 4, "reference_unstable": False, "observed_reference_moves": [4], "seed_samples": [{"seed": 2040, "reference_move": 4, "teacher_value": 0.42}]},
            {"id": "capture_available-017", "canonical_state": module.canonical_state_key(suite_rows[1]["state"]), "state": suite_rows[1]["state"], "reference_move": 3, "reference_unstable": False, "observed_reference_moves": [3], "seed_samples": [{"seed": 2040, "reference_move": 3, "teacher_value": 0.46}]},
            {"id": "capture_available-018", "canonical_state": module.canonical_state_key(suite_rows[2]["state"]), "state": suite_rows[2]["state"], "reference_move": 4, "reference_unstable": False, "observed_reference_moves": [4], "seed_samples": [{"seed": 2040, "reference_move": 4, "teacher_value": 0.41}]},
        ]

        selected = module.select_opening_capture_family_rows(suite_rows=suite_rows, reference_rows=reference_rows)

        self.assertEqual(["capture_available-016", "capture_available-017"], [row["id"] for row in selected])
        self.assertEqual([4, 3], [row["reference_move"] for row in selected])
        self.assertEqual(["opening_capture_family", "opening_capture_family"], [row["replay_role"] for row in selected])

    def test_selects_only_stable_high_imbalance_rows_from_requested_ids(self):
        from ml.alphazero_lite import build_tactical_stable_failure_family_replay as module

        suite_rows = [
            {"id": "high_imbalance-001", "state": {"player_pits": [4, 4, 4, 4, 0, 5], "opponent_pits": [0, 1, 6, 6, 6, 6], "player_store": 1, "opponent_store": 1, "current_player": 0}, "side_to_move": 0, "legal_moves": [0, 1, 2, 3, 5], "phase": "midgame", "bucket": "high_imbalance", "tags": ["high_imbalance"], "source": "generated"},
            {"id": "high_imbalance-002", "state": {"player_pits": [5, 4, 4, 4, 4, 0], "opponent_pits": [5, 5, 0, 5, 5, 5], "player_store": 1, "opponent_store": 1, "current_player": 0}, "side_to_move": 0, "legal_moves": [0, 1, 2, 3, 4], "phase": "midgame", "bucket": "high_imbalance", "tags": ["high_imbalance"], "source": "generated"},
            {"id": "high_imbalance-003", "state": {"player_pits": [3, 5, 5, 5, 0, 5], "opponent_pits": [0, 2, 6, 6, 6, 5], "player_store": 2, "opponent_store": 1, "current_player": 0}, "side_to_move": 0, "legal_moves": [0, 1, 2, 3, 5], "phase": "midgame", "bucket": "high_imbalance", "tags": ["high_imbalance"], "source": "generated"},
        ]
        reference_rows = [
            {"id": "high_imbalance-001", "canonical_state": module.canonical_state_key(suite_rows[0]["state"]), "state": suite_rows[0]["state"], "reference_move": 2, "reference_unstable": False, "observed_reference_moves": [2], "seed_samples": [{"seed": 2040, "reference_move": 2, "teacher_value": 0.57}]},
            {"id": "high_imbalance-002", "canonical_state": module.canonical_state_key(suite_rows[1]["state"]), "state": suite_rows[1]["state"], "reference_move": None, "reference_unstable": True, "observed_reference_moves": [2, 4], "seed_samples": [{"seed": 2040, "reference_move": 2, "teacher_value": 0.57}, {"seed": 3040, "reference_move": 4, "teacher_value": 0.51}]},
            {"id": "high_imbalance-003", "canonical_state": module.canonical_state_key(suite_rows[2]["state"]), "state": suite_rows[2]["state"], "reference_move": 1, "reference_unstable": False, "observed_reference_moves": [1], "seed_samples": [{"seed": 2040, "reference_move": 1, "teacher_value": 0.61}]},
        ]
        ...
        self.assertEqual(["high_imbalance_stable"], [row["replay_role"] for row in selected])
    ...
    def test_build_dataset_treats_empty_high_imbalance_ids_as_select_none(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary_out_path = tmp_path / "tactical_stable_failure_family_replay_summary.json"
        ...
        self.assertEqual([], [row["id"] for row in rows if row["replay_role"] == "high_imbalance_stable"])
        ...
        reference_rows = [
            {
                ...
                "seed_samples": [{"seed": 2040, "reference_move": True, "teacher_value": 0.42}],
            }
        ]

        selected = module.select_opening_capture_family_rows(suite_rows=[suite_row], reference_rows=reference_rows)
    ...
    def test_build_dataset_rejects_missing_stable_failure_family_targets(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            source_rows = [
                self.preservation_capture_row("preserve-capture-a", priority=7.0, variant="gain_4_5"),
                self.preservation_capture_row("preserve-capture-b", priority=6.0, variant="gain_2_3"),
            ]
            for bucket in ("high_imbalance", "high_value_swing", "starvation_pressure"):
            ...
            suite_rows = [
                {"id": "capture_available-016", "state": {"player_pits": [5, 1, 5, 5, 5, 0], "opponent_pits": [1, 6, 0, 7, 6, 5], "player_store": 1, "opponent_store": 1, "current_player": 0}, "side_to_move": 0, "legal_moves": [0, 1, 2, 3, 4], "phase": "opening", "bucket": "capture_available", "tags": ["capture_available"], "source": "generated"},
                {"id": "high_imbalance-001", "state": {"player_pits": [4, 4, 4, 4, 0, 5], "opponent_pits": [0, 1, 6, 6, 6, 6], "player_store": 1, "opponent_store": 1, "current_player": 0}, "side_to_move": 0, "legal_moves": [0, 1, 2, 3, 5], "phase": "midgame", "bucket": "high_imbalance", "tags": ["high_imbalance"], "source": "generated"},
            ]
            suite_path.write_text(json.dumps(suite_rows), encoding="utf-8")
            reference_payload = {
                "schema": "azlite_forensic_references_v1",
                "reference": {"policy_simulations": 1200, "value_simulations": 1800, "sample_seeds": [2040]},
                "rows": [
                    {"id": "capture_available-016", "canonical_state": module.canonical_state_key(suite_rows[0]["state"]), "state": suite_rows[0]["state"], "reference_move": None, "reference_unstable": True, "observed_reference_moves": [3, 4], "seed_samples": [{"seed": 2040, "reference_move": 3, "teacher_value": 0.42}, {"seed": 3040, "reference_move": 4, "teacher_value": 0.41}]},
                    {"id": "high_imbalance-001", "canonical_state": module.canonical_state_key(suite_rows[1]["state"]), "state": suite_rows[1]["state"], "reference_move": None, "reference_unstable": True, "observed_reference_moves": [1, 2], "seed_samples": [{"seed": 2040, "reference_move": 2, "teacher_value": 0.57}, {"seed": 3040, "reference_move": 1, "teacher_value": 0.51}]},
                ],
            }
            reference_path.write_text(json.dumps(reference_payload), encoding="utf-8")
            summary_out_path = tmp_path / "stable_failure_replay_summary.json"

            with self.assertRaisesRegex(ValueError, "nearby_preservation does not exceed prior 6"):
        ...
        self.assertEqual(3, summary["shortfalls_by_role"]["high_imbalance_stable"])
        self.assertIn("nearby_preservation does not exceed prior 6", summary["invalid_reasons"])
    ...
    def test_build_dataset_writes_explicit_stable_failure_roles(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            reference_payload = {
                ...
                "rows": [
                    {"id": "capture_available-016", "canonical_state": module.canonical_state_key(suite_rows[0]["state"]), "state": suite_rows[0]["state"], "reference_move": 4, "reference_unstable": False, "observed_reference_moves": [4], "seed_samples": [{"seed": 2040, "reference_move": 4, "teacher_value": 0.42}]},
                    {"id": "high_imbalance-001", "canonical_state": module.canonical_state_key(suite_rows[1]["state"]), "state": suite_rows[1]["state"], "reference_move": 2, "reference_unstable": False, "observed_reference_moves": [2], "seed_samples": [{"seed": 2040, "reference_move": 2, "teacher_value": 0.57}]},
                ],
            }
            reference_path.write_text(json.dumps(reference_payload), encoding="utf-8")
            summary_out_path = tmp_path / "stable_failure_replay_summary.json"
            ...
            summary = json.loads(summary_out_path.read_text(encoding="utf-8"))
            persisted_rows = [json.loads(line) for line in out_path.read_text(encoding="utf-8").splitlines()]
        ...
        self.assertEqual(
            ["capture_protection", "capture_preservation", "capture_preservation", "opening_capture_family", "high_imbalance_stable"] + ["nearby_preservation"] * 6,
    ...
    def test_build_dataset_records_shortfalls_when_source_rows_are_undersized(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            tactical_replay_path.write_text(
                "\n".join(
                    [
                        json.dumps(self.preservation_capture_row("preserve-capture-a", priority=7.0, variant="gain_4_5")),
                        json.dumps(self.nearby_row("high_value_swing", 0, priority=5.0)),
                    ]
                ) + "\n",
                encoding="utf-8",
            )
    ...
    def test_build_dataset_records_capture_preservation_shortfall_without_early_failure(self):
        from ml.alphazero_lite import build_tactical_stable_failure_family_replay as module
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for bucket, count in (("high_imbalance", 4), ("high_value_swing", 6), ("starvation_pressure", 2)):
                for index in range(count):
                    source_rows.append(self.nearby_row(bucket, index, priority=50 - index))
    ...
    def test_build_dataset_records_capture_preservation_duplicate_shape_collapse_without_rejecting_run(self):
        from ml.alphazero_lite import build_tactical_stable_failure_family_replay as module
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            source_rows = [
                self.preservation_capture_row("preserve-capture-a", priority=9.0, variant="gain_4_5"),
                self.preservation_capture_row("preserve-capture-b", priority=8.0, variant="gain_2_3"),
                self.preservation_capture_row("preserve-capture-c", priority=7.0, variant="gain_4_5"),
                self.preservation_capture_row("preserve-capture-d", priority=6.0, variant="gain_2_3"),
            ]
            for bucket, count in (("high_imbalance", 2), ("high_value_swing", 4), ("starvation_pressure", 2)):
                for index in range(count):
                    source_rows.append(self.nearby_row(bucket, index, priority=50 - index))
        ...
        self.assertNotIn("capture_preservation shape diversity shortfall 2", summary["invalid_reasons"])

    def test_build_dataset_records_nearby_zero_bucket_shortfall_without_early_failure(self):
        from ml.alphazero_lite import build_tactical_stable_failure_family_replay as module
    ...
    def test_build_dataset_rejects_full_nearby_count_when_bucket_missing(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "nearby_preservation missing bucket participation: high_imbalance"):
                module.build_stable_failure_family_replay_dataset(
                    tactical_replay_path=tactical_replay_path,
                    suite_path=suite_path,
                    reference_path=reference_path,
    ...
    def test_build_dataset_returns_rebalanced_23_row_mix_with_summary_diagnostics(self):
        ...
        self.assertEqual(
            ...
            [row["id"] for row in rows if row["replay_role"] == "high_imbalance_stable"],
        )
        self.assertEqual(
            {
                "capture_protection": [row["id"] for row in rows if row["replay_role"] == "capture_protection"],
                "capture_preservation": [row["canonical_state"] for row in rows if row["replay_role"] == "capture_preservation"],
                "opening_capture_family": [row["id"] for row in rows if row["replay_role"] == "opening_capture_family"],
                "high_imbalance_stable": [row["id"] for row in rows if row["replay_role"] == "high_imbalance_stable"],
                "nearby_preservation": [row["canonical_state"] for row in rows if row["replay_role"] == "nearby_preservation"],
            },
        ...
        nearby_rows = [row for row in rows if row["replay_role"] == "nearby_preservation"]
        ...
        self.assertEqual(
            ...
            sorted(
                ...
                if row["bucket"] == "high_value_swing" and row["canonical_state"] in {"high_value_swing-2", "high_value_swing-3"}
            ),
        )

    def test_build_dataset_writes_summary_before_raising_when_opening_family_remains_dominant(self):
        from ml.alphazero_lite import build_tactical_stable_failure_family_replay as module
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "opening_capture_family remains dominant|nearby_preservation does not exceed prior 6"):
                module.build_stable_failure_family_replay_dataset(
                    tactical_replay_path=tactical_replay_path,
                    suite_path=suite_path,
                    reference_path=reference_path,
            ...
            self.assertIn("opening_capture_family remains dominant", summary_payload["invalid_reasons"])
            self.assertIn("nearby_preservation does not exceed prior 6", summary_payload["invalid_reasons"])
