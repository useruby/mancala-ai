...
class SuperhumanStrengthExperimentTest(unittest.TestCase):
    def test_non_dry_run_keeps_stdout_machine_parseable(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (repo_root / "ml/alphazero_lite/pipeline.py").write_text(
                pipeline_stub, encoding="utf-8"
            )
            (
                repo_root / "ml/alphazero_lite/build_superhuman_strength_dataset.py"
            ).write_text(
                "#!/usr/bin/env python3\n"
                "import json, sys\n"
            ...
            fixture_path = (
                repo_root
                / "ml/alphazero_lite/fixtures/superhuman_strength_games_2026_04_06.json"
            )
            fixture_path.write_text("[]", encoding="utf-8")
            (repo_root / "model-artifact/current/weights.json").write_text(
                "{}", encoding="utf-8"
            )
            phase_config = {
                "run_id": "stub",
            ...
            ).write_text(json.dumps(phase_config), encoding="utf-8")
            (
                repo_root
            ...
            ).write_text(json.dumps({"steps": []}), encoding="utf-8")
    ...
    def test_dry_run_writes_lane_manifests(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(
                str(report["curated_dataset_path"]).startswith(str(output_root))
            )
            ...
            self.assertTrue(
                any(
                    "aggressive_v3_superhuman_search_control.json" in part
                    for part in lane_a
                )
            )
            ...
            self.assertTrue(
                any("ml/alphazero_lite/pipeline.py" in part for part in lane_b_stage1)
            )
            self.assertTrue(
                any("ml/alphazero_lite/pipeline.py" in part for part in lane_b_stage2)
            )
            ...
            self.assertIn(
                report["curated_dataset_path"],
                stage1_train[stage1_train.index("--data-files") + 1],
            )
            self.assertIn(
                report["curated_dataset_path"],
            ...
            self.assertTrue(
                stage1_train[stage1_train.index("--init-checkpoint") + 1].endswith(
                    "current_init_checkpoint.npz"
                )
            )
            self.assertIn("--init-checkpoint", stage2_train)

    def test_direct_launcher_prefers_workspace_root_venv_in_worktree_without_env_override(
        self,
    ):
        source_repo_root = Path(__file__).resolve().parents[2]
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                ...
                Path(env["WORKSPACE_PYTHON_MARKER"])
                .read_text(encoding="utf-8")
                .strip(),
            )
    ...
    def test_superhuman_strength_experiment_lane_b_stage1_uses_opponent_pool(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            pool_path = Path(
                stage1_self_play[stage1_self_play.index("--opponent-pool-config") + 1]
            )
    ...
    def test_superhuman_strength_experiment_accepts_explicit_opponent_checkpoint(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = json.loads(result.stdout)
            pool = json.loads(
                Path(report["opponent_pool_path"]).read_text(encoding="utf-8")
            )

            self.assertIn(str(extra_model.resolve()), pool["checkpoints"])
            self.assertIn(
                str(extra_model.resolve()), report["opponent_checkpoint_inputs"]
            )

    def test_superhuman_strength_experiment_adds_fallback_init_checkpoint_when_only_explicit_opponent_exists(
        self,
    ):
        source_repo_root = Path(__file__).resolve().parents[2]
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fallback_checkpoint = (
                output_root / "explicit-plus-fallback-test/current_init_checkpoint.npz"
            )
    ...
    def test_superhuman_strength_experiment_deduplicates_explicit_and_discovered_checkpoints(
        self,
    ):
        source_repo_root = Path(__file__).resolve().parents[2]
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                pool["checkpoints"].count(str(current_model_path.resolve())), 1
            )

    def test_superhuman_strength_experiment_deduplicates_explicit_checkpoint_inputs(
        self,
    ):
        source_repo_root = Path(__file__).resolve().parents[2]
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [str(explicit_model.resolve())], report["opponent_checkpoint_inputs"]
            )
    ...
    def test_superhuman_strength_experiment_keeps_stage2_self_play_without_opponent_pool(
        self,
    ):
        source_repo_root = Path(__file__).resolve().parents[2]
        with tempfile.TemporaryDirectory() as tmp:
