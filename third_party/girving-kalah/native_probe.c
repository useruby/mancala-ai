...
static void board(position p, int x) { int i; printf("\"pits\":["); for(i=0;i<6;i++)printf("%s%d",i?",":"",p.a[i]);for(i=0;i<6;i++)printf(",%d",p.a[i+7]);printf("],\"stores\":[%d,%d],\"player\":%d,\"extra_turn\":%s,\"terminal\":%s,\"final_margin\":%d",p.a[6],p.a[13],p.s,x?"true":"false",p.w>=0?"true":"false",p.a[6]-p.a[13]); }
static void apply(const char *s) { position p;int a,x;readp(s,&p);if(!integer(s,"\"action\"",&a)||a<0||a>=6||!bin(p,a))exit(2);x=move(&p,a);printf("{\"operation\":\"apply\",");board(p,x);puts("}"); }
static void solveone(const char *s) { position p;readp(s,&p);printf("{\"operation\":\"solve\",");board(p,0);printf(",\"exact_value\":%d}\n",value(p)); }
static void label(const char *s) { position p,c;int a,x,v,best,first=1;readp(s,&p);best=p.s?10000:-10000;printf("{\"operation\":\"label\",");board(p,0);printf(",\"action_values\":{");for(a=0;a<6;a++)if(bin(p,a)){c=p;x=move(&c,a);v=c.w>=0?c.a[6]-c.a[13]:value(c);if((!p.s&&v>best)||(p.s&&v<best))best=v;printf("%s\"%d\":%d",first?"":",",a,v);first=0;}printf("},\"exact_value\":%d,\"optimal_actions\":[",best);first=1;for(a=0;a<6;a++)if(bin(p,a)){c=p;x=move(&c,a);v=c.w>=0?c.a[6]-c.a[13]:value(c);if(v==best){printf("%s%d",first?"":",",a);first=0;}}printf("],\"action_margins\":{");first=1;for(a=0;a<6;a++)if(bin(p,a)){c=p;x=move(&c,a);v=c.w>=0?c.a[6]-c.a[13]:value(c);printf("%s\"%d\":%d",first?"":",",a,best-v);first=0;}puts("}}"); }
int main(void) { char s[4096];init_hash(16,0);while(fgets(s,sizeof(s),stdin)){if(strstr(s,"apply"))apply(s);else if(strstr(s,"solve"))solveone(s);else if(strstr(s,"label"))label(s);else return 2;fflush(stdout);}close_hash();return 0; }
